<template>
  <div class="min-h-full bg-surface2 transition-colors duration-300">
    ...
    <header class="bg-surface border-b border-light-c px-4 py-3 sticky top-0 z-30">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <div>
            <h1 class="font-bold text-base-c leading-none text-sm sm:text-base">活動消息管理</h1>
            <p class="text-xs text-hint-c mt-0.5 hidden sm:block">News & Events</p>
          </div>
        </div>
        <button v-if="perm.can('staff.news.edit')" @click="openModal(null)"
    ...
    <div class="max-w-7xl mx-auto px-3 sm:px-6 py-4">
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-3">
        <div v-for="item in newsList" :key="item.id"
             class="bg-surface rounded-2xl border border-light-c shadow-sm p-4 flex gap-4 items-start">

          <!-- 封面圖 -->
          <div class="flex-shrink-0 w-28 h-28 lg:w-36 lg:h-36 rounded-xl overflow-hidden bg-surface2">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-start justify-between gap-2">
              <div class="min-w-0">
                <p class="font-bold text-base text-base-c truncate">{{ item.title }}</p>
                <div class="flex flex-wrap items-center gap-1.5 mt-1">
                  <span class="text-sm text-hint-c">{{ item.date }}</span>
                  <span v-for="tag in item.tags" :key="tag"
                        class="px-2 py-0.5 rounded-full text-xs bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400">
                    {{ tag }}
                  </span>
                </div>
                <p class="text-sm text-hint-c mt-1.5 line-clamp-2">{{ stripHtml(item.content) }}</p>
                <p v-if="item.attachments?.length" class="text-sm text-hint-c mt-1">
                  📎 {{ item.attachments.length }} 個附件
                </p>
              </div>
              <div class="flex gap-1.5 flex-shrink-0">
                <button v-if="perm.can('staff.news.edit')" @click="openModal(item)"
                ...
                <button v-if="perm.can('staff.news.edit')" @click="confirmDelete(item)"
    ...
    <div v-if="modal.show"
      ...
      <div class="bg-surface rounded-b-3xl sm:rounded-2xl shadow-xl w-full sm:max-w-2xl max-h-screen sm:max-h-[92vh] overflow-y-auto">
        ...
        <div class="px-5 py-4 space-y-4">
          ...
          <!-- 標題 -->
          <div>
            <label class="text-xs font-semibold text-muted-c block mb-1">標題 *</label>
            <input v-model="form.title" placeholder="活動消息標題"
                   class="w-full px-3 py-2 text-sm rounded-xl border border-light-c bg-surface text-base-c outline-none focus:ring-2 focus:ring-sky-400" />
          </div>

          <!-- 日期 + 分類標籤 -->
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="text-xs font-semibold text-muted-c block mb-1">日期</label>
              <input v-model="form.date" type="date"
                     class="w-full px-3 py-2 text-sm rounded-xl border border-light-c bg-surface text-base-c outline-none focus:ring-2 focus:ring-sky-400" />
            </div>
            <div>
              <label class="text-xs font-semibold text-muted-c block mb-1">分類標籤</label>
              <div class="flex gap-1.5">
                <input v-model="tagInput" placeholder="輸入後按 Enter"
                       @keydown.enter.prevent="addTag"
                       class="flex-1 px-3 py-2 text-sm rounded-xl border border-light-c bg-surface text-base-c outline-none focus:ring-2 focus:ring-sky-400" />
                <button @click="addTag"
          ...
          <div>
            ...
            <div v-if="form.coverPreview || form.coverUrl"
                 class="relative w-full h-40 rounded-xl overflow-hidden mb-2 border border-light-c bg-surface2">
            ...
            <div @click="coverInputRef?.click()"
                 class="border-2 border-dashed border-base rounded-xl p-4 text-center cursor-pointer hover:border-sky-400 transition-colors">
          ...
          <!-- 內文 (Tiptap) -->
          <div>
            <label class="text-xs font-semibold text-muted-c block mb-1">內文</label>
            <!-- 工具列 -->
            <div class="flex flex-wrap items-center gap-0.5 px-2 py-1.5 border border-light-c border-b-0 rounded-t-xl bg-surface2">
              <button type="button" @click="editor?.chain().focus().toggleHeading({ level: 2 }).run()"
                      :class="editor?.isActive('heading', { level: 2 }) ? 'bg-sky-100 text-sky-700' : 'text-hint-c hover:bg-surface2'"
                      class="px-2 py-1 rounded text-xs font-bold transition-colors">H2</button>
              <button type="button" @click="editor?.chain().focus().toggleHeading({ level: 3 }).run()"
                      :class="editor?.isActive('heading', { level: 3 }) ? 'bg-sky-100 text-sky-700' : 'text-hint-c hover:bg-surface2'"
                      class="px-2 py-1 rounded text-xs font-bold transition-colors">H3</button>
              <div class="w-px h-4 bg-surface2 mx-1"></div>
              <button type="button" @click="editor?.chain().focus().toggleBold().run()"
                      :class="editor?.isActive('bold') ? 'bg-sky-100 text-sky-700' : 'text-hint-c hover:bg-surface2'"
                      class="px-2 py-1 rounded text-xs font-bold transition-colors" title="粗體">B</button>
              <button type="button" @click="editor?.chain().focus().toggleItalic().run()"
                      :class="editor?.isActive('italic') ? 'bg-sky-100 text-sky-700' : 'text-hint-c hover:bg-surface2'"
                      class="px-2 py-1 rounded text-xs italic font-semibold transition-colors" title="斜體">I</button>
              <button type="button" @click="editor?.chain().focus().toggleUnderline().run()"
                      :class="editor?.isActive('underline') ? 'bg-sky-100 text-sky-700' : 'text-hint-c hover:bg-surface2'"
                      class="px-2 py-1 rounded text-xs underline font-semibold transition-colors" title="底線">U</button>
              <button type="button" @click="editor?.chain().focus().toggleStrike().run()"
                      :class="editor?.isActive('strike') ? 'bg-sky-100 text-sky-700' : 'text-hint-c hover:bg-surface2'"
              ...
              <button type="button" @click="editor?.chain().focus().setTextAlign('left').run()"
                      :class="editor?.isActive({ textAlign: 'left' }) ? 'bg-sky-100 text-sky-700' : 'text-hint-c hover:bg-surface2'"
              ...
              <button type="button" @click="editor?.chain().focus().setTextAlign('center').run()"
                      :class="editor?.isActive({ textAlign: 'center' }) ? 'bg-sky-100 text-sky-700' : 'text-hint-c hover:bg-surface2'"
              ...
              <button type="button" @click="editor?.chain().focus().setTextAlign('right').run()"
                      :class="editor?.isActive({ textAlign: 'right' }) ? 'bg-sky-100 text-sky-700' : 'text-hint-c hover:bg-surface2'"
              ...
              <button type="button" @click="editor?.chain().focus().toggleBulletList().run()"
                      :class="editor?.isActive('bulletList') ? 'bg-sky-100 text-sky-700' : 'text-hint-c hover:bg-surface2'"
              ...
              <button type="button" @click="editor?.chain().focus().toggleOrderedList().run()"
                      :class="editor?.isActive('orderedList') ? 'bg-sky-100 text-sky-700' : 'text-hint-c hover:bg-surface2'"
              ...
              <button type="button" @click="editor?.chain().focus().setHorizontalRule().run()"
                      class="p-1.5 rounded text-hint-c hover:bg-surface2 transition-colors" title="分隔線">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button type="button" @click="editor?.chain().focus().undo().run()"
                      class="p-1.5 rounded text-hint-c hover:bg-surface2 transition-colors" title="復原">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/></svg>
              </button>
              <button type="button" @click="editor?.chain().focus().redo().run()"
                      class="p-1.5 rounded text-hint-c hover:bg-surface2 transition-colors" title="取消復原">
          ...
          <!-- 附件 -->
          <div>
            <label class="text-xs font-semibold text-muted-c block mb-1">附件</label>
            <div v-if="form.attachments?.length" class="space-y-1.5 mb-2">
              <div v-for="(att, idx) in form.attachments" :key="idx"
                   class="flex items-center justify-between px-3 py-2 bg-surface2 rounded-xl text-sm">
                <span class="text-muted-c truncate flex-1 mr-2">📎 {{ att.name }}</span>
            ...
            <div @click="attachInputRef?.click()"
                 class="border-2 border-dashed border-base rounded-xl p-4 text-center cursor-pointer hover:border-sky-400 transition-colors">
              <p class="text-sm text-hint-c">點擊上傳附件（可多選）</p>
