...
const refresh = async () => {
  ...
  try {
    ...
    if (dates.length > 0) {
      ...
      const [prevDates, nextDates] = await Promise.all([
        findDatesWithData(fp, -1, 1),
        findDatesWithData(ln, 1, 1)
      ])
      hasPrev.value = prevDates.length > 0
      hasNext.value = nextDates.length > 0
    } else {
      hasPrev.value = false
      hasNext.value = false
    }
...
// ── 格子（槽）管理：新增 / 刪除 ─────────────────────────────────────
const nextSlotNumber = (type) => {
  const slots = slotsByType(type)
  return slots.length > 0 ? Math.max(...slots) + 1 : 1
}

const addNewSlot = async (type) => {
  await addItemToSlot(type, nextSlotNumber(type))
  await fetchMarkedDates()
}

const confirmDeleteSlot = async (type, slot) => {
  const items = itemsByTypeAndSlot(type, slot)
...
// ── 還原成預設格子配置 ───────────────────────────────────────────────
// 注意：預設格數的權威定義在後端 /holy/menu/init/{date}（切換日期時也會呼叫這支）。
// 這裡必須呼叫同一支 API，而不是自己另外兜一套數字，否則切換日期回來時
// 後端 init 會依照它自己的模板把格子補回去，等於「還原」被打回原狀。
  ...
  <ClientOnly>
    <div class="min-h-full bg-surface2 transition-colors duration-300">
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-1">
              ...
              >
                ...
                <div
                  v-else
                  class="w-4 h-4 border-2 border-orange-500 border-t-transparent rounded-full animate-spin flex-shrink-0"
                />
              </button>
            </div>
          </div>
      ...
      <div class="max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
        <div
          :class="isEditMode ? 'flex flex-col lg:flex-row' : 'flex flex-col'"
          class="gap-4 items-start"
        >
          <!-- ── 左欄：日曆（僅編輯模式）── -->
          <div
            v-if="isEditMode"
            class="w-full lg:w-72 xl:w-80 flex-shrink-0"
          >
              ...
              <div class="flex items-center justify-between mt-3 pt-3 border-t border-light-c">
                ...
                <div class="flex items-center gap-2">
                  <button
                    ...
                    class="text-xs text-hint-c hover:text-orange-600 transition-colors disabled:opacity-50"
                    @click="confirmRestoreDefaults"
                  >
                    還原預設
                  </button>
                  <button
                    v-if="selectedDate && dateStatus[selectedDate]"
                  ...
                  <button
                    class="text-sm text-orange-700 dark:text-orange-400 hover:text-orange-800 font-medium"
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <span
                      v-if="dateStatus[day.date] === 'complete'"
                    ...
                    >
                      ...
                      >
                        <div v-if="itemsByTypeForDate(day.date, section.type).length > 0">
                          <span
                            :class="section.badge"
                            class="text-xs mb-1 inline-block"
                          >{{ section.label }}</span>
                          <div class="space-y-1.5">
                            <div
                              v-for="item in itemsByTypeForDate(day.date, section.type)"
                              :key="item.id"
                              class="rounded-xl overflow-hidden border border-light-c bg-surface2"
                            >
                              <template v-if="item.images && item.images.length > 0">
                                <img
                                  v-if="!imgErrors.has(item.images[0])"
                              ...
                              <div class="px-2 py-1.5">
                                <div class="flex items-center gap-1 flex-wrap">
                                  <span
                                    v-if="item.isFirst"
                                    class="text-xs px-1.5 py-0.5 rounded-full bg-orange-500 text-white font-medium flex-shrink-0"
                                  >現在</span>
                                  <span
                                    v-else
                                    class="text-xs px-1.5 py-0.5 rounded-full bg-surface2 text-hint-c font-medium flex-shrink-0"
                                  >已換</span>
                                  <span
                                    v-if="item.dietType"
                                    :class="DIET_BADGE[item.dietType]"
                                    class="text-xs px-1.5 py-0.5 rounded-full font-medium flex-shrink-0"
                                  >{{ item.dietType }}</span>
                                  <p class="font-semibold text-base-c text-sm leading-snug">
                                    {{ item.name }}
                                  </p>
                                </div>
                                <p
                                ...
                                <p
                                  v-if="item.note"
                                  class="text-xs text-hint-c italic mt-0.5"
                                >
                                  {{ item.note }}
                                </p>
                              </div>
              ...
              <div class="space-y-5">
                ...
                >
                  <div class="flex items-center justify-between mb-2">
                    ...
                    <button
                      class="flex items-center gap-1 text-xs text-orange-600 dark:text-orange-400 hover:text-orange-700 font-medium px-2 py-1 rounded-lg hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors"
                      @click="addNewSlot(section.type)"
                    >
                      <svg
                        class="w-3.5 h-3.5"
                        fill="none"
                  ...
                  <div class="space-y-3">
                    <div
                      v-for="slot in slotsByType(section.type)"
                      :key="`${section.type}-${slot}`"
                      class="bg-surface rounded-2xl border border-light-c shadow-sm overflow-hidden"
                    >
                      <div class="flex items-center justify-between px-3 pt-2">
                        <span class="text-xs text-hint-c">格 {{ slot }}</span>
                        <button
                          class="p-1 text-hint-c hover:text-red-400 transition-colors"
                          title="刪除此格子"
                          @click="confirmDeleteSlot(section.type, slot)"
                        >
                          <svg
                            class="w-3.5 h-3.5"
                            fill="none"
                      ...
                      <div
                        v-for="(item, itemIdx) in itemsByTypeAndSlot(section.type, slot)"
                        :key="item.id"
                      >
                        <div
                          v-if="itemIdx > 0"
                          class="mx-3 border-t border-dashed border-light-c"
                        />
                        <div class="flex gap-3 p-3">
                          <div
                            class="flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden relative"
                          ...
                          >
                            <template v-if="item.images && item.images.length > 0">
                              ...
                              <div
                                v-else
                                class="w-full h-full flex items-center justify-center text-hint-c text-xs text-center p-1"
                              >
                                無法載入
                              </div>
                            </template>
                          ...
                          <div class="flex-1 min-w-0">
                            <div class="flex items-start gap-2 mb-1">
                              <select
                                v-if="section.hasDiet"
                                v-model="item.dietType"
                              ...
                              <div class="flex items-center gap-1 flex-shrink-0">
                                <button
                                  ...
                                  @click="showNote[item.id] = !showNote[item.id]"
                                >
                                  <svg
                                    class="w-3.5 h-3.5"
                                    fill="none"
                            ...
                            <div class="flex flex-wrap gap-1 mt-1.5 items-center">
                              <span
                                v-for="(ing, ingIdx) in item.ingredients"
                                :key="ingIdx"
                              ...
                              >
                                {{ ing }}
                                <button
                                  class="text-green-300 hover:text-red-400 leading-none ml-0.5"
                                  @click="item.ingredients.splice(ingIdx, 1); autoSave(item)"
                                >×</button>
                              </span>
                              <input
                            ...
                            <input
                              v-if="item.note || showNote[item.id]"
                              v-model="item.note"
                              placeholder="備註…"
                      ...
                      <div class="px-4 pb-3">
                        <button
                          ...
                          @click="addItemToSlot(section.type, slot)"
                        >
                          <svg
                            class="w-3.5 h-3.5"
                            fill="none"
        ...
        <div
          ...
          <div class="flex items-center justify-between mb-3">
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
              ...
              <button
                class="p-2 text-hint-c hover:text-muted-c"
                @click="suggestModal.show = false"
              >
                <svg
                  class="w-5 h-5"
                  fill="none"
          ...
          <div class="space-y-3">
            ...
            <div v-if="suggestModal.data['dish']">
              ...
              <div
                v-if="suggestModal.data['dish'].length > 0"
                class="grid grid-cols-3 gap-2"
              >
                <div
                  v-for="(item, idx) in suggestModal.data['dish']"
                  :key="idx"
                  class="rounded-xl overflow-hidden border border-light-c bg-surface2"
                >
                  <div
                  ...
                  <div
                    v-else
                    class="aspect-square bg-surface2 flex items-center justify-center text-hint-c text-xs"
                  >
                    無圖
                  </div>
                  <div class="px-2 py-1">
                    <span
                      v-if="item.dietType"
                      :class="DIET_BADGE[item.dietType]"
                      class="text-xs px-1 py-0.5 rounded-full font-medium"
                    >{{ item.dietType }}</span>
                    <p
                      v-if="item.name"
                      class="text-xs font-semibold text-base-c mt-0.5"
                    >
                      {{ item.name }}
                    </p>
                  </div>
              ...
              <p
                v-else
                class="text-xs text-hint-c py-2 px-3 rounded-xl bg-surface2 border border-dashed border-light-c"
              >
                歷史紀錄不足
              </p>
            </div>
            ...
            <div>
              <div class="grid grid-cols-3 gap-2">
                <div
                  v-for="(item, idx) in [...(suggestModal.data['soup'] || []), ...(suggestModal.data['tea'] || [])]"
                  :key="idx"
                  class="rounded-xl overflow-hidden border border-light-c bg-surface2"
                >
                  <div
                  ...
                  <div class="px-2 py-1">
                    ...
                    <span
                      v-if="item._category"
                      class="text-xs text-hint-c ml-1"
                    >{{ item._category }}</span>
                    <p
                      v-if="item.name"
                      class="text-xs font-semibold text-base-c mt-0.5"
          ...
          <button
            class="mt-5 w-full px-4 py-2.5 text-sm bg-surface2 text-muted-c rounded-xl hover:bg-surface2 transition-colors"
            @click="suggestModal.show = false"
          >
            關閉
          </button>
      ...
      >
        <div
          ...
          <div class="mb-4">
            ...
            <p
              v-else
              class="text-sm text-hint-c py-4 text-center border border-dashed border-light-c rounded-xl"
            >
              尚無圖片
            </p>
          </div>
          ...
          >
            <svg
              class="w-8 h-8 mx-auto mb-2 text-hint-c"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <button
            class="mt-4 w-full px-4 py-2.5 text-sm bg-surface2 text-muted-c rounded-xl hover:bg-surface2 transition-colors"
            @click="imageModal.show = false"
          >
            關閉
          </button>
