<script setup>
  ...
  const PERIOD_BASE = () => commonStore.data.main_url + '/holy/booking/period'
  ...
  // 卡片目前只顯示今天～週日（見 WEEK_DATES_FROM_TODAY 的說明），標題範圍也跟著顯示今天起算，
  // 避免寫著「週一～週日」但實際上週一～昨天沒有卡片，看起來對不上
  const weekRangeLabel = computed(() => {
    if (!weekSummary.value?.weekEnd) return ''
    return `${fmtMD(todayStr)} - ${fmtMD(weekSummary.value.weekEnd)}`
  })
  ...
  const PERIOD_COLORS = {
    amber: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border-amber-200 dark:border-amber-800/40',
    orange: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400 border-orange-200 dark:border-orange-800/40',
    indigo: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 border-indigo-200 dark:border-indigo-800/40',
    purple: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400 border-purple-200 dark:border-purple-800/40',
    teal: 'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400 border-teal-200 dark:border-teal-800/40'
  }
  const periodColorClass = (colorKey) => PERIOD_COLORS[colorKey] || PERIOD_COLORS.amber
  ...
  // 未落在任何時段區間內的歸為「未分類」並排在最後
  function groupBookingByPeriod(items, rules) {
    const map = new Map()
    const ensure = (period) => {
      ...
      if (!map.has(key)) {
        map.set(key, {
          ...
          items: [], rules: [], count: 0, guestTotal: 0, meat: 0, fullVeg: 0, eggVeg: 0, spiceVeg: 0
        })
      }
      return map.get(key)
    }
    const addQty = (g, obj) => {
      g.meat += Number(obj.meatQty) || 0
      g.fullVeg += Number(obj.fullVegQty) || 0
      g.eggVeg += Number(obj.eggVegQty) || 0
      g.spiceVeg += Number(obj.spiceVegQty) || 0
    }
    for (const item of items) {
      const g = ensure(findPeriod(item.time))
      g.items.push(item)
      g.count += 1
      g.guestTotal += itemGuestTotal(item)
      addQty(g, item)
    }
    for (const rule of rules) {
      const g = ensure(findPeriod(rule.time))
      g.rules.push(rule)
      g.guestTotal += (Number(rule.meatQty) || 0) + (Number(rule.fullVegQty) || 0) + (Number(rule.eggVegQty) || 0) + (Number(rule.spiceVegQty) || 0)
      addQty(g, rule)
    }
  ...
  // 目前後端 /holy/home/week 只回傳「今天起到週日」的資料（週一到昨天沒有），
  // 這裡先在前端把已經過去的日子隱藏，只顯示今天～週日，避免出現一堆空白的舊日期卡片；
  // 之後後端補齊整週資料後，這裡要記得改回 CAL_WEEK_DATES（顯示週一～週日）
  const WEEK_DATES_FROM_TODAY = CAL_WEEK_DATES.filter(date => date >= todayStr)
  const weekDayCards = computed(() => {
    if (viewMode.value !== 'week') return []
    return WEEK_DATES_FROM_TODAY.map((date) => {
      ...
      return {
        ...
        soybean: {
          items: sItems,
          breakdown: buildSoymilkVolumeBreakdown(sItems),
          activeCount: sItems.filter(i => i.status !== '已取消').length,
          pickedCount: sItems.filter(i => i.status === '已取貨').length
        }
      }
    })
  })
  ...
  // 依容量彙總多筆豆製品訂單的豆漿數量，跟後台 soybean-orders.vue 的分組標頭一樣，
  // 回傳 [{volume: 800, qty: 9}, {volume: 1000, qty: 2}, ...]，依容量由小到大排序。
  // 已取消的訂單不算；舊資料沒有 soymilkBreakdown 只有 soymilkQty 時，比照後台當作 800ml。
  function buildSoymilkVolumeBreakdown(items) {
    const map = new Map()
    for (const item of items) {
      ...
      if (list.length) {
        for (const b of list) {
          ...
          if (!vol || !qty) continue
          map.set(vol, (map.get(vol) || 0) + qty)
        }
      } else if (item.soymilkQty) {
        map.set(800, (map.get(800) || 0) + (Number(item.soymilkQty) || 0))
      }
    }
    return Array.from(map.entries())
      .sort(([a], [b]) => a - b)
      .map(([volume, qty]) => ({volume, qty}))
  }

  // 當天（最近出貨日）豆漿依容量彙總，例如 800ml×9、1000ml×2、1700ml×2
  const soybeanVolumeBreakdown = computed(() => buildSoymilkVolumeBreakdown(soybeanOrders.value))
  // 豆漿總容量（毫升）：由容量彙總再加總，已取消的不算
  const soybeanSoymilkTotalMl = computed(() =>
    soybeanVolumeBreakdown.value.reduce((s, b) => s + b.volume * b.qty, 0))
  // 豆漿總袋數：有容量明細就用明細加總（排除已取消），跟上面的容量彙總對得起來；沒有明細才用後端給的數字
  const soybeanSoymilkBags = computed(() => {
    const bags = soybeanVolumeBreakdown.value.reduce((s, b) => s + b.qty, 0)
    return bags || soybeanSoymilk.value
  })
  // 已取貨進度（不含已取消）
  const soybeanActiveCount = computed(() => soybeanOrders.value.filter(i => i.status !== '已取消').length)
  const soybeanPickedCount = computed(() => soybeanOrders.value.filter(i => i.status === '已取貨').length)
  ...
  // 豆製品訂單狀態顏色：跟後台 soybean-orders.vue 同一套色系，各狀態差距拉大；
  // 已取貨用實心綠最顯眼，已取消灰色加刪除線
  const soybeanStatusClass = (status) => {
    switch (status) {
      case '待確認':
        return 'bg-amber-100 text-amber-800 ring-1 ring-inset ring-amber-400 dark:bg-amber-900/40 dark:text-amber-300 dark:ring-amber-600'
      case '已確認':
        return 'bg-sky-100 text-sky-800 ring-1 ring-inset ring-sky-400 dark:bg-sky-900/40 dark:text-sky-300 dark:ring-sky-600'
      case '已付款':
        return 'bg-violet-100 text-violet-800 ring-1 ring-inset ring-violet-400 dark:bg-violet-900/40 dark:text-violet-300 dark:ring-violet-600'
      case '已取貨':
        return 'bg-emerald-600 text-white font-bold shadow-sm dark:bg-emerald-500 dark:text-emerald-950'
      case '已取消':
  ...
  // 豆製品訂單整列底色：已取貨淺綠底，已取消變淡
  const soybeanRowClass = (status) => {
    if (status === '已取貨') return 'bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900 rounded-lg px-2 py-1'
    if (status === '已取消') return 'opacity-50'
    return ''
  }
  ...
  // ── 設定區塊：手機／電腦推播通知（就算沒開著這個分頁也能收到新訂單/每日提醒）───
  // 只是「加到主畫面」還不夠，一定要使用者實際按過下面的開關、在系統彈窗按允許，才算訂閱成功
  const {
    isSupported: pushSupported,
    subscribe: pushSubscribe,
    unsubscribe: pushUnsubscribe,
    isSubscribed: pushIsSubscribed,
  } = usePushNotification(() => commonStore.data.main_url)

  const pushEnabled = ref(false)       // 這台裝置目前是否已訂閱
  const pushLoading = ref(false)       // 開啟/關閉訂閱進行中
  const pushError = ref('')            // 訂閱失敗時顯示的錯誤訊息
  const pushSupportedValue = ref(true) // 此瀏覽器是否支援推播（iOS < 16.4 或非 PWA 模式下會是 false）
  const pushTesting = ref(false)       // 發送測試推播進行中
  const pushTestMsg = ref('')          // 測試推播結果訊息

  async function initPushState() {
    pushSupportedValue.value = pushSupported()
    if (pushSupportedValue.value) {
      pushEnabled.value = await pushIsSubscribed()
    }
  }
  ...
  async function togglePushSubscribe() {
    ...
    try {
      ...
      pushError.value = e?.message || '設定失敗，請重試'
    } finally {
  ...
  async function sendTestPush() {
    ...
    try {
      const res = await fetch(`${commonStore.data.main_url}/holy/push/test/send`, {
        method: 'POST',
        credentials: 'include',
      })
      pushTestMsg.value = res.ok ? '已送出，等幾秒看手機有沒有跳通知' : '發送失敗，請確認後端設定'
    } catch (e) {
      pushTestMsg.value = '發送失敗，請確認後端設定'
    } finally {
      pushTesting.value = false
      setTimeout(() => (pushTestMsg.value = ''), 4000)
    }
  }
  ...
  onMounted(() => {
    ...
    fetchPeriods() // 訂位時段設定，跟今日概況/房務狀況各自獨立拉，不影響彼此載入中狀態
    fetchToday().then(() => {
      syncKnownItems(daySummary.value)
      connectStream()
      startPolling()
    ...
    initPushState() // 確認這台裝置目前是否已訂閱推播，初始化設定區塊的開關狀態
  })
            ...
            <div class="min-h-0 overflow-hidden">
              ...
              <div
                ...
                <span
                  ...
                  style="font-size:clamp(11px, calc(11px + 0.4vw), 14px)"
                >
                  手機／電腦推播通知（就算沒開著這個網頁也能收到新訂單、每日提醒）
                </span>
                <button
                  class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors flex-shrink-0"
                  ...
                  :disabled="pushLoading || !pushSupportedValue"
                  :title="pushEnabled ? '點一下關閉，這台就不會再收到推播' : '點一下開啟，會跳出瀏覽器的通知權限請求'"
                  @click="togglePushSubscribe"
                >
                  <span
                    class="inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform"
                    :class="pushEnabled ? 'translate-x-4' : 'translate-x-1'"
                  />
                </button>
                <span
                  class="text-hint-c"
                  style="font-size:clamp(11px, calc(11px + 0.4vw), 14px)"
                >
                  {{ pushLoading ? '設定中...' : (pushEnabled ? '已開啟' : '已關閉') }}
                </span>
              </div>
              <div
                v-if="!pushSupportedValue"
                class="px-4 py-2 bg-surface2/40 border-t border-light-c/60 text-red-600 dark:text-red-400"
                style="font-size:clamp(10px, calc(10px + 0.35vw), 13px)"
              >
                這個瀏覽器不支援推播通知。iPhone 請先「加入主畫面」，並從主畫面圖示（不是 Safari 分頁）打開才行。
              </div>
              <div
                v-if="pushError"
                class="px-4 py-2 bg-surface2/40 border-t border-light-c/60 text-red-600 dark:text-red-400"
              ...
              >
                <button
                  type="button"
                  class="px-3 py-1.5 text-xs rounded-lg border border-light-c text-muted-c hover:bg-surface2 transition-colors disabled:opacity-50"
                  :disabled="pushTesting"
                  @click="sendTestPush"
                >
                  {{ pushTesting ? '發送中...' : '發送測試通知' }}
                </button>
                <span
                  v-if="pushTestMsg"
                  class="text-hint-c"
                  ...
                  >
                    ...
                    <div class="divide-y divide-base">
                      <div
                        ...
                        :key="'recur-' + rule.id"
                        class="py-1 first:pt-0 flex items-center gap-1.5 flex-wrap"
                      >
                        <span
            ...
            >
              ...
              <template v-if="!soybeanCollapsed && soybeanOrders.length > 0">
                ...
                <div
                  ...
                  style="font-size:clamp(11px, calc(11px + 0.45vw), 15px)"
                >
                  <div v-if="soybeanSoymilkBags > 0">
                    🥛 豆漿 {{ soybeanSoymilkBags }} 袋（共 {{ formatVolume(soybeanSoymilkTotalMl) }}）
                  </div>
                  <!-- 依容量彙總：800ml×9、1000ml×2、1700ml×2 -->
                  <div
                    v-if="soybeanVolumeBreakdown.length"
                    class="flex flex-wrap gap-1.5 py-0.5"
                  >
                    <span
                      v-for="b in soybeanVolumeBreakdown"
                      :key="b.volume"
                      class="inline-flex items-baseline gap-0.5 rounded-lg px-2 py-0.5 bg-amber-50 ring-1 ring-inset ring-amber-300 text-amber-800 dark:bg-amber-900/30 dark:ring-amber-700 dark:text-amber-300"
                    >
                      <span class="font-semibold">{{ b.volume }}ml</span>
                      <span class="opacity-70">×</span>
                      <span class="font-black" style="font-size:1.15em">{{ b.qty }}</span>
                    </span>
                  </div>
                  ...
                  <div
                    v-if="soybeanActiveCount > 0"
                    class="flex items-center gap-1 font-semibold"
                    :class="soybeanPickedCount === soybeanActiveCount ? 'text-emerald-600 dark:text-emerald-400' : 'text-hint-c'"
                  >
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/>
                    </svg>
                    已取貨 {{ soybeanPickedCount }}/{{ soybeanActiveCount }}
                  </div>
                </div>
                <div
                  ...
                  <div
                    ...
                    class="py-1.5 first:pt-0"
                  >
                    <div :class="soybeanRowClass(item.status)">
                      <div class="flex items-center gap-2 flex-wrap">
                    <span
                      class="flex-1 min-w-0 truncate font-semibold text-base-c"
                      style="font-size:clamp(12px, calc(12px + 0.45vw), 17px)"
                    >{{ item.name || '未填姓名' }}</span>
                        <span
                          class="flex-shrink-0 rounded-full px-1.5 py-0.5 font-medium"
                          style="font-size:clamp(9px, calc(9px + 0.45vw), 13px)"
                          :class="soybeanStatusClass(item.status)"
                        ><svg
                          v-if="item.status === '已取貨'"
                          class="inline-block w-3 h-3 -mt-0.5 mr-0.5"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        ><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>{{ item.status }}</span>
                      </div>
                      <div
                        class="text-hint-c mt-0.5"
                        style="font-size:clamp(11px, calc(11px + 0.45vw), 15px)"
              ...
              >
                ...
                >
                  ...
                  >
                    <!-- 當天豆漿依容量彙總＋已取貨進度 -->
                    <div
                      v-if="day.soybean.breakdown.length || day.soybean.activeCount"
                      class="flex flex-wrap items-center gap-1 pb-1"
                      style="font-size:clamp(10px, calc(10px + 0.4vw), 13px)"
                    >
                      <span
                        v-for="b in day.soybean.breakdown"
                        :key="b.volume"
                        class="inline-flex items-baseline gap-0.5 rounded-md px-1.5 py-0.5 bg-amber-50 ring-1 ring-inset ring-amber-300 text-amber-800 dark:bg-amber-900/30 dark:ring-amber-700 dark:text-amber-300"
                      >
                        <span class="font-semibold">{{ b.volume }}ml</span>
                        <span class="opacity-70">×</span>
                        <span class="font-black">{{ b.qty }}</span>
                      </span>
                      <span
                        v-if="day.soybean.activeCount"
                        class="ml-auto font-semibold whitespace-nowrap"
                        :class="day.soybean.pickedCount === day.soybean.activeCount ? 'text-emerald-600 dark:text-emerald-400' : 'text-hint-c'"
                      >✓ {{ day.soybean.pickedCount }}/{{ day.soybean.activeCount }}</span>
                    </div>
                    <div
                      v-for="item in day.soybean.items"
                      :key="item.id"
                      :class="soybeanRowClass(item.status)"
                    >
                      <div class="flex items-center gap-2 flex-wrap">
                      <span
                        class="flex-1 min-w-0 truncate font-medium text-base-c"
