...
const appGroups = [
  {
    ...
    items: [
      ...
      {
        to: '/staff/personnel/phone-directory',
        icon: '📞',
        label: '電話簿',
  ...
  },
  {
    ...
    items: [
      {
        to: '/staff/print/table-card-print',
        icon: '🪧',
        label: '桌牌',
        key: 'staff.table-card-print',
        bg: 'bg-purple-500'
      },
      {
        to: '/staff/print/herbs-label-print',
        icon: '🏷️',
        label: '花園 QRCode',
        key: 'staff.herbs-label-print',
        bg: 'bg-purple-600'
      },
    ]
  },
  ...
  },
  {
    ...
    items: [
      {
        to: '/staff/order/booking-orders',
        icon: '🪑',
        label: '訂位管理',
        key: 'staff.booking-orders',
        bg: 'bg-green-600'
      },
      {to: '/staff/order/lunch-orders', icon: '🍱', label: '便當訂單', key: 'staff.lunch-orders', bg: 'bg-green-700'},
      {
        to: '/staff/order/soybean-orders',
        icon: '🥛',
        label: '豆漿訂單',
        key: 'staff.soybean-orders',
        bg: 'bg-green-800'
      },
      {
        to: '/staff/order/black-cat-orders',
        icon: '🚚',
        label: '黑貓貨單',
        key: 'staff.black-cat-orders',
        bg: 'bg-teal-600'
      },
    ]
  },
    ...
    if (gRes?.ok) {
      ...
      gEvents = (gData.items || []).map(item => {
        ...
        return {
          ...
          owner: item.organizer?.displayName || '', room: item.location || '',
...
<template>
  <div class="min-h-screen bg-surface2 transition-colors">
    ...
    <div class="max-w-2xl mx-auto px-3 sm:px-4 py-4 space-y-4">
      ...
      <!-- ── App 功能列表 ── -->
      <div v-if="!permLoaded" class="flex justify-center py-8">
