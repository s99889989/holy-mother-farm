...
<template>
  <div class="p-4 max-w-screen-xl mx-auto text-sm text-base-c">
    <!-- 標題 -->
    <div class="text-base font-bold text-muted-c dark:text-hint-c mb-3">
      黑貓貨單管理
    </div>
    ...
    <div class="flex gap-2 mb-2 items-center text-sm flex-wrap">
      ...
      <select
        v-model="limit"
        class="border border-base rounded px-2 py-1 bg-surface text-base-c"
        @change="page=1; refreshList()"
      >
        <option :value="10">
          10
        </option>
      ...
      <table class="w-full border-collapse text-sm">
        <thead class="bg-teal-600 dark:bg-teal-800 text-white">
          <tr>
            ...
            <th class="border border-teal-700 dark:border-teal-900 px-2 py-2 w-12 text-center whitespace-nowrap">
              編輯
            </th>
        ...
        <tbody class="divide-y divide-base">
          ...
          >
            ...
            <td class="border border-light-c px-3 py-1 font-mono text-blue-600 dark:text-blue-400 whitespace-nowrap">
              {{ row.tracking_no }}
            </td>
            ...
            <td
              class="border border-light-c px-3 py-1 whitespace-nowrap max-w-[180px] overflow-hidden text-ellipsis"
            ...
            <td
              class="border border-light-c px-3 py-1 whitespace-nowrap max-w-[120px] overflow-hidden text-ellipsis"
              :title="row.production_name"
            >
              {{ row.production_name }}
            </td>
            <td class="border border-light-c px-3 py-1 whitespace-nowrap">
    ...
    <div class="mb-2 text-muted-c dark:text-hint-c">
      目前契客代號：<strong class="text-base-c">{{ form.sender_code }}</strong>
    </div>

    <!-- ════════════════════════ 託運單表單 ════════════════════════ -->
    <div class="border border-base rounded-md overflow-hidden mb-4">
      <div class="bg-teal-500 dark:bg-teal-700 text-white px-4 py-2 font-bold">
        託運單資料
      </div>
      <div class="p-4 bg-pink-50 dark:bg-zinc-900 space-y-4">
        ...
        <div class="flex flex-wrap gap-3 items-center bg-surface border border-base rounded px-3 py-2">
          ...
          <select
            v-model="form.sender_code"
            class="border border-base rounded px-2 py-1 bg-surface text-base-c"
          >
            <option
              v-for="a in (accounts as any[])"
              :key="a.login"
              :value="a.login"
            >
              {{ a.login }}
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div class="space-y-4">
            ...
            <fieldset class="border border-base rounded p-3 bg-surface">
              <legend class="font-bold text-pink-700 dark:text-pink-400 px-1">
                收件人
              </legend>
              <div class="grid grid-cols-3 gap-2 mt-2">
                <div>
                  ...
                  <input
                    ...
                    type="text"
                    class="border border-base rounded px-2 py-1 w-full bg-surface text-base-c"
                  >
                </div>
                <div>
              ...
              <div class="mt-2">
                ...
                <input
                  ...
                  type="text"
                  class="border border-base rounded px-2 py-1 w-1/2 bg-surface text-base-c"
                >
              </div>
            </fieldset>
            ...
            <fieldset class="border border-base rounded p-3 bg-surface">
              <div>
                ...
                <div class="flex gap-2 mt-1">
                  <select
                    v-model="form.production_kind"
                    class="border border-base rounded px-2 py-1 w-36 bg-surface text-base-c"
                  >
                    <option
                      v-for="p in (productnames as any[])"
                      :key="p.product_id"
                  ...
                  <input
                    ...
                    type="text"
                    class="border border-base rounded px-2 py-1 flex-1 bg-surface text-base-c"
                  >
                </div>
                <p class="text-red-500 dark:text-red-400 text-xs mt-1">
                  ※品名內容不得為文件、標單、明信片、郵簡、信函、資料、發票、訴訟文件、公文、身分證明文件、報名表、申請書、帳單、訂單、報價單、繳費單、通知單、保險單、文書、檢體
                </p>
              </div>
            ...
            <fieldset class="border border-base rounded p-3 bg-surface">
              ...
              <div class="grid grid-cols-2 gap-3 mt-2">
                <div>
                  ...
                  <input
                    ...
                    class="border border-base rounded px-2 py-1 w-full bg-surface text-base-c placeholder:text-hint-c dark:placeholder:text-hint-c"
                  >
                </div>
                <div>
              ...
              <div class="mt-3">
                ...
                <div class="flex gap-4">
                  <label
                    v-for="t in deliverTimes"
                    :key="t.value"
                    class="flex items-center gap-1 cursor-pointer text-muted-c"
                  >
                    <input
              ...
              <div class="mt-3">
                ...
                <div class="flex gap-4">
                  ...
                  <label class="flex items-center gap-1 cursor-pointer text-muted-c">
                    <input
                      v-model="form.precision_instrument"
                      type="checkbox"
                      true-value="yes"
                      false-value="no"
                    >精密儀器
        ...
        <div
          v-if="formError"
          class="bg-red-100 dark:bg-red-900/40 border border-red-400 dark:border-red-700 text-red-700 dark:text-red-300 px-4 py-2 rounded"
        >
          {{ formError }}
        </div>
        <div
          v-if="formSuccess"
          class="bg-green-100 dark:bg-green-900/40 border border-green-400 dark:border-green-700 text-green-700 dark:text-green-300 px-4 py-2 rounded"
        >
          {{ formSuccess }}
        </div>
