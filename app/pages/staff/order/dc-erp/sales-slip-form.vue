...
<template>
  <div class="p-2 sm:p-4">
      ...
      <div class="space-y-3 p-2 sm:p-4">
        <div class="flex items-center justify-end gap-3">
          ...
          <NuxtLink
            ...
            class="text-xs text-muted-c hover:underline"
          >
            返回列表
          </NuxtLink>
        </div>
        ...
        <template v-else>
          <p
            v-if="errorMessage"
            class="rounded-lg border border-red-200 bg-red-50 p-2 text-sm text-red-600"
          >
            {{ errorMessage }}
          </p>
          ...
          <div class="rounded-xl border border-light-c bg-surface text-sm">
            ...
            >
              <div class="flex flex-wrap items-center gap-x-2 gap-y-1">
                ...
                <span
                  v-if="!showHeaderSection"
                  class="text-xs text-muted-c"
                >
            ...
            <div
              v-show="showHeaderSection"
              class="space-y-2 border-t border-light-c p-3 pt-2"
            >
              <div
                v-if="!isNew"
              ...
              <div class="flex flex-wrap items-center gap-2">
                ...
                <select
                  v-model="header.workPlaceID"
                  class="rounded border border-light-c bg-surface px-2 py-1"
                >
                  <option
                    v-for="opt in options.workPlace"
                    :key="opt.value"
                    :value="opt.value"
                  >
                    {{ opt.label }}
              ...
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span
                  v-if="firmLookupState === 'loading'"
                  class="text-xs text-hint-c"
                >查詢中…</span>
                <span
                  v-else-if="firmLookupState === 'found'"
                  class="text-sm text-green-700"
                >{{ header.firmName }}</span>
                <span
                  v-else-if="firmLookupState === 'notfound'"
                  class="text-xs text-red-600"
                >查無此客戶代號</span>
                <span
                  v-else-if="firmLookupState === 'error'"
                  class="text-xs text-red-600"
                >查詢失敗</span>

                <label class="ml-2 text-muted-c">取訂貨單：</label>
                <input
                  v-model="header.relationCode"
                  type="text"
                  placeholder="訂貨單號"
                  class="w-32 rounded border border-light-c bg-surface px-2 py-1"
                >
              </div>
              ...
              <div class="flex flex-wrap items-center gap-2">
                ...
                <select
                  v-model="header.payWay"
                  class="rounded border border-light-c bg-surface px-2 py-1"
                >
                  <option
                    v-for="opt in options.payWay"
                    :key="opt.value"
                    :value="opt.value"
                  >
                    {{ opt.label }}
                ...
                <select
                  v-model="header.taxInputType"
                  class="rounded border border-light-c bg-surface px-2 py-1"
                >
                  <option
                    v-for="opt in options.taxInputType"
                    :key="opt.value"
                    :value="opt.value"
                  >
                    {{ opt.label }}
              ...
              <div class="flex flex-wrap items-center gap-2">
                ...
                <input
                  ...
                  type="text"
                  class="w-full sm:w-72 rounded border border-light-c bg-surface px-2 py-1"
                >
              </div>
              ...
              <div class="flex flex-wrap items-center gap-2">
                ...
                <input
                  ...
                  type="text"
                  class="w-full sm:w-96 rounded border border-light-c bg-surface px-2 py-1"
                >
              </div>
              ...
              <div class="flex flex-wrap items-center gap-2 text-xs text-hint-c">
                ...
                <span
                  v-if="header.signState"
                  class="ml-2"
                >簽核狀態：{{ header.signState }}</span>
              </div>
            </div>
          </div>
            ...
            >
              ...
              >
                <div class="relative aspect-[4/3] bg-surface2">
                  ...
                  <span
                    v-else
                    class="flex h-full w-full items-center justify-center text-xs text-hint-c"
                  >無圖</span>
                  <div class="absolute right-1.5 top-1.5 flex flex-col gap-0.5">
                    <button
                      type="button"
                      class="rounded bg-surface/90 px-1.5 text-muted-c hover:bg-surface disabled:opacity-30"
                      title="上移"
                      :disabled="index === 0"
                  ...
                  <div class="flex items-center justify-between">
                    <div>
                      ...
                      <div class="mt-0.5 flex items-center gap-1">
                        <button
                          type="button"
                          class="rounded border border-light-c px-2 text-muted-c hover:bg-surface2"
                          @click="row.originalNum = Math.max(0, (Number(row.originalNum) || 0) - 1)"
                        >
                          −
                        </button>
                        <input
                          v-model.number="row.originalNum"
                          type="number"
                          step="any"
                          class="w-14 appearance-none rounded border border-light-c bg-surface px-1 py-1 text-center [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                        >
                        <button
                          type="button"
                          class="rounded border border-light-c px-2 text-muted-c hover:bg-surface2"
                          @click="row.originalNum = (Number(row.originalNum) || 0) + 1"
                        >
                          ＋
                        </button>
                      </div>
                    ...
                    <div class="text-right">
                      ...
                      <div class="mt-0.5 font-medium text-base-c">
                        {{ row.price }}
                      </div>
                    </div>
                  </div>
              ...
              <p
                v-if="!details.length"
                class="col-span-full p-6 text-center text-hint-c"
              >
                尚無明細，請按「新增商品」
              </p>
              <div
                v-if="details.length"
                class="col-span-full flex items-center justify-between rounded-lg bg-surface2 px-3 py-2 font-medium"
              >
              ...
              <table class="w-full text-sm">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="px-2 py-1.5">
                      <div class="flex items-center justify-center gap-1">
                        ...
                        <input
                          ...
                          type="number"
                          step="any"
                          class="w-14 appearance-none rounded border border-light-c bg-surface px-1 py-0.5 text-center [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                        >
                        <button
                          type="button"
                          class="rounded border border-light-c px-2 text-muted-c hover:bg-surface2"
                    ...
                    <td class="px-2 py-1.5 text-right">
                      {{ ((Number(row.originalNum) || 0) * (Number(row.price) || 0)).toLocaleString() }}
                    </td>
                    <td class="px-2 py-1.5">
                      <button
                        class="text-xs text-red-600 hover:underline"
                  ...
                  <tr v-if="!details.length">
                    <td
                      colspan="8"
                      class="px-2 py-6 text-center text-hint-c"
                    >
                      尚無明細，請按「新增商品」
                    </td>
                  </tr>
                ...
                <tfoot v-if="details.length">
                  <tr class="border-t border-light-c bg-surface2 font-medium">
                    <td
                      colspan="6"
                      class="px-2 py-2 text-right text-muted-c"
                    >
                      合計
                    </td>
    ...
    >
      <div class="flex max-h-[80vh] w-full max-w-2xl flex-col rounded-xl bg-surface p-4">
        ...
        <div class="mb-2 flex items-center gap-2">
          <select
            v-model="firmCategory"
            class="rounded border border-light-c bg-surface px-2 py-1 text-sm"
          >
            <option
              v-if="!firmCategoryOptions.length"
              value="不拘"
            >
              不拘
            </option>
            <option
              v-for="opt in firmCategoryOptions"
              :key="opt.value"
              :value="opt.value"
            >
              {{ opt.label }}
        ...
        <div
          v-if="firmTotalPages > 1"
          class="mt-2 flex items-center justify-center gap-2 text-xs text-muted-c"
        >
          <button
            class="rounded border border-light-c px-2 py-1 disabled:opacity-40"
            :disabled="firmPage <= 1"
            @click="searchFirms(firmPage - 1)"
          >
            上一頁
          </button>
          ...
          <button
            class="rounded border border-light-c px-2 py-1 disabled:opacity-40"
            :disabled="firmPage >= firmTotalPages"
            @click="searchFirms(firmPage + 1)"
          >
            下一頁
          </button>
      ...
      <div class="flex max-h-[85vh] w-full max-w-4xl flex-col rounded-xl bg-surface p-4">
        <div class="mb-2 flex items-center justify-between">
          <div class="text-sm font-bold text-base-c">
            選擇商品
            <span
              v-if="productFirmName"
              class="ml-2 text-xs font-normal text-hint-c"
            >{{ productFirmName }}</span>
          </div>
        ...
        <div class="mb-2 space-y-2 rounded-lg border border-light-c bg-surface2 p-2 text-sm">
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              <option
                v-for="c in productClassOptions"
                :key="c"
                :value="c"
              >
                {{ c }}
          ...
          <div
            v-show="productFilterExpanded"
            class="flex flex-wrap items-center gap-2 border-t border-light-c pt-2"
          >
            <label class="text-muted-c">依欄位：</label>
            <select
              v-model="productWhSearch"
              class="rounded border border-light-c bg-surface px-2 py-1"
            >
              <option
                v-if="!productWhSearchOptions.length"
                value="whatever"
              >
                欄位不拘
              </option>
              <option
                v-for="opt in productWhSearchOptions"
                :key="opt.value"
                :value="opt.value"
              >
                {{ opt.label }}
            ...
            <select
              v-model="productSourceType"
              class="rounded border border-light-c bg-surface px-2 py-1"
            >
              <option
                v-if="!productSourceTypeOptions.length"
                value="0"
              >
                所有商品規格
              </option>
              <option
                v-for="opt in productSourceTypeOptions"
                :key="opt.value"
                :value="opt.value"
              >
                {{ opt.label }}
            ...
            >
              <option
                v-if="!productScheOptions.length"
                value="0"
              >
                不拘
              </option>
              <option
                v-for="opt in productScheOptions"
                :key="opt.value"
                :value="opt.value"
              >
                {{ opt.label }}
            ...
            <input
              v-model="productCorrespondNoKeyword"
              type="text"
              class="w-24 rounded border border-light-c bg-surface px-2 py-1"
            >
          </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto rounded border border-light-c">
          <p
            v-if="productClassSearching"
            class="p-4 text-sm text-hint-c"
          >
            {{ productClassSearchProgress }}
          </p>
          <p
            v-else-if="productSearching"
            class="p-4 text-sm text-hint-c"
          >
            搜尋中…
          </p>
          ...
          <!-- 卡片檢視 -->
          <div
            v-else-if="productViewMode === 'card'"
            class="grid grid-cols-2 gap-2 p-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6"
          >
            <div
            ...
            >
              ...
              <div class="p-2 text-sm">
                <div
                  class="truncate font-medium text-base-c"
                  :title="p.name"
                >
                  {{ p.name }}
                </div>
                ...
                <div
                  v-if="productClassOf(p.code)"
                  class="truncate text-xs text-hint-c"
                  :title="productClassOf(p.code)"
                >
                  {{ productClassOf(p.code) }}
                </div>
              </div>
            </div>
            <p
              v-if="!filteredProductResults.length"
              class="col-span-full py-6 text-center text-hint-c"
            >
              查無資料
            </p>
          </div>
          ...
          >
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-2 py-1.5 text-xs text-muted-c">
                  {{ productClassOf(p.code) }}
                </td>
              </tr>
        ...
        <div class="mt-2 flex items-center justify-between">
          <div
            ...
            class="flex items-center gap-2 text-xs text-muted-c"
          >
            <button
              class="rounded border border-light-c px-2 py-1 disabled:opacity-40"
              :disabled="productPage <= 1"
              @click="searchProducts(productPage - 1)"
            >
              上一頁
            </button>
          ...
          <div class="flex items-center gap-2">
            ...
            <button
              class="rounded bg-green-700 px-4 py-1.5 text-xs font-medium text-white hover:bg-green-800"
    ...
    >
      <div class="max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-surface p-4">
        <div class="mb-3 flex items-center justify-between">
          ...
          <button
            class="text-hint-c hover:text-base-c"
        ...
        <template v-else>
          <table class="w-full text-sm">
            <tbody>
              <tr
                v-for="style in commonPrintStyles"
                :key="style.name"
          ...
          <table
            v-show="printStylesExpanded"
            class="mt-2 w-full text-sm"
          >
            <tbody>
              <tr
                v-for="style in otherPrintStyles"
                :key="style.name"
                class="border-b border-light-c last:border-b-0"
              >
                <td class="px-2 py-2">
    ...
    <form
      ref="printForm"
      method="post"
      action="/api/dc-erp/sales-slip-print"
      target="_blank"
      class="hidden"
    >
      <input
      ...
      <input
        type="hidden"
        name="reportId"
        :value="submitReportId"
      >
      <input
        type="hidden"
        name="reportFormat"
        :value="submitReportFormat"
      >
      <input
        type="hidden"
        name="titleType"
