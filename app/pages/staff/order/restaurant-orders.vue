<script setup>
definePageMeta({layout: 'staff', requiredPermission: 'order.restaurant-orders'})
...
// ── 預約注意事項（可自訂，客戶端訂位頁「📋 預約注意事項」區塊顯示用）────
const NOTICE_BASE = computed(() => `${BASE.value}/notice`)
...
// ── 初始化 ────────────────────────────────────────────────────────
onMounted(async () => {
  await Promise.all([fetchMarkedDates(), fetchSchedule(), fetchRecurring(), fetchPeriods(), fetchArrivalSlots(), fetchHoursSettings(), fetchNotice()])
    ...
    <header class="bg-surface border-b border-light-c px-4 py-3 sticky top-0 z-30">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            class="text-xs flex items-center gap-1 px-2.5 py-1.5 rounded-lg hover-surface2 text-hint-c font-medium transition-colors"
            title="客戶訂位頁下方顯示的「預約注意事項」文字"
            @click="openNoticeSettings"
          >
            📋 <span class="hidden sm:inline">預約注意事項</span>
          </button>
          <NuxtLink
