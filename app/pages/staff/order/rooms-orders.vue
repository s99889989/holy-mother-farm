...
function bookingInSelectedBuilding(b) {
  ...
  if (b.roomId) {
    const r = roomById(b.roomId)
    return r && r.buildingId === ordersBuilding.value
  }
    ...
    if (expanded) {
      ...
      for (const b of sortedMembers) {
        ...
        rows.push({
          ...
          inGroup: true,
          showBuildingHeader,
          buildingName: room ? room.buildingName : ''
        })
      }
    }
  }
  for (const b of singles) rows.push({
    kind: 'booking',
    booking: b,
    inGroup: false,
    showBuildingHeader: false,
    buildingName: roomById(b.roomId)?.buildingName || ''
  })
  return rows
})
...
const roomTypeSummary = computed(() => {
  ...
  for (const r of roomsInSelectedBuilding.value) {
    if (!map.has(r.capacity)) map.set(r.capacity, {
      capacity: r.capacity,
      types: new Set(),
      total: 0,
      active: 0,
...
function addSplitRow() {
  splitRows.value.push({ roomId: '', guests: 1, name: '', notes: '' })
}
...
function eligibleRoomsForSplitRow(idx) {
  ...
  return rooms.value
    .filter(r => r.active)
    .filter(r => r.capacity >= (row.guests || 1))
    .filter(r => !pickedElsewhere.includes(r.id))
    .filter(r => isRoomAvailable(r.id, b.checkIn, b.checkOut, b.id))
    .sort((r1, r2) => r1.capacity - r2.capacity || r1.price - r2.price)
}

// 改了人數、但原本選的房間住不下新人數時，把房間選擇清掉，避免送出住不下的房間
function onSplitRowGuestsChange(idx) {
  const row = splitRows.value[idx]
  if (!row || !row.roomId) return
  const r = roomById(row.roomId)
  if (r && r.capacity < (row.guests || 1)) row.roomId = ''
}
...
async function submitSplit() {
  splitError.value = ''
  if (!assignTarget.value) return
  if (!splitGroupName.value.trim()) {
    splitError.value = '請填寫團體名稱'
    return
  }
  const rows = splitRows.value.filter(r => r.roomId)
  if (rows.length === 0) {
    splitError.value = '請至少分配一間房'
    return
  }
  const roomIds = rows.map(r => r.roomId)
  if (new Set(roomIds).size !== roomIds.length) {
    splitError.value = '同一份表單裡不能選到同一間房兩次'
    return
  }
...
async function submitEditOrder() {
  ...
  try {
    ...
    if (res && res.error) {
      editOrderError.value = res.error
      return
    }
    ...
    if (o.roomId && o.roomId !== editOrderOriginalRoomId.value) {
      ...
      if (assignRes && assignRes.error) {
        editOrderError.value = `其他資料已更新，但房間變更失敗：${assignRes.error}`
        await fetchAll()
        return
      }
    }
    ...
    console.error(e)
    editOrderError.value = '儲存失敗，請稍後再試'
  }
}
...
// 改了人數、但原本選的房間住不下新人數時，把房間選擇清掉，避免送出住不下的房間
function onGroupRowGuestsChange(idx) {
  const row = newGroup.value.rooms[idx]
  if (!row || !row.roomId) return
  const r = roomById(row.roomId)
  if (r && r.capacity < (row.guests || 1)) row.roomId = ''
...
// 改了人數、但原本選的房間住不下新人數時，把房間選擇清掉，避免送出住不下的房間
function onGroupAddGuestsChange() {
  if (!groupAddRoom.value.roomId) return
  const r = roomById(groupAddRoom.value.roomId)
  if (r && r.capacity < (groupAddRoom.value.guests || 1)) groupAddRoom.value.roomId = ''
}
...
const eligibleRoomsForGroupAdd = computed(() => {
  ...
  return rooms.value
    .filter(r => r.active)
    .filter(r => r.capacity >= (groupAddRoom.value.guests || 1))
  ...
  <div class="min-h-full bg-surface2 transition-colors">
              ...
              >
                <div>
                  ...
                  >
                    {{ b.checkIn }} → {{
                      b.checkOut
                    }}{{ b.buildingPref !== 'all' ? '・偏好 ' + buildingNameOf(b.buildingPref) : '' }}
                  </div>
                </div>
                <button
                  class="mini-btn mini-primary"
                ...
                <div v-if="dayTarget.bookings.length">
                  ...
                  >
                    ...
                    <template v-else>
                      ...
                      <div class="flex justify-between py-0.5">
                        <span class="text-hint-c">入住 → 退房</span><span class="text-base-c">{{
                          bk.checkIn
                        }} → {{ bk.checkOut }}</span>
                      </div>
                      <div class="flex justify-between py-1 mt-1 border-t border-light-c">
          ...
          >
            ...
            >
              ...
              >
                ...
                <template
                  v-for="r in b.rooms.filter(rm => rm.active)"
                  :key="r.id"
                >
        ...
        <div v-else-if="tab === 'rooms'">
          ...
          <div class="flex items-center justify-between flex-wrap gap-2 mb-4">
            ...
            <div class="segmented">
              <button
                ...
                :style="viewMode === 'list' ? segActiveStyle : ''"
                @click="viewMode = 'list'"
              >
                列表檢視
              </button>
              <button
                ...
                :style="viewMode === 'floorplan' ? segActiveStyle : ''"
                @click="viewMode = 'floorplan'"
              >
                平面圖檢視
              </button>
              <button
                :class="viewMode === 'shape' ? 'seg-active' : ''"
                :style="viewMode === 'shape' ? segActiveStyle : ''"
                @click="viewMode = 'shape'"
              >
                矩形對應
              </button>
            </div>
          ...
          >
            <div class="flex items-center justify-between gap-2 mb-3 flex-wrap">
              ...
              <div class="segmented">
                <button
                  :class="shapeEditMode === 'assign' ? 'seg-active' : ''"
                  :style="shapeEditMode === 'assign' ? segActiveStyle : ''"
                  @click="shapeEditMode = 'assign'; selectedShapeId = null"
                >
                  指定房間
                </button>
                <button
                  :class="shapeEditMode === 'edit' ? 'seg-active' : ''"
                  :style="shapeEditMode === 'edit' ? segActiveStyle : ''"
                  @click="shapeEditMode = 'edit'"
                >
                  調整位置
                </button>
              </div>
            ...
            >
              <div class="flex items-center justify-between gap-2 mb-2 flex-wrap">
                ...
                >
                  <button
                    class="mini-btn"
                    @click="addShape(grp.id, 'rect')"
                  >
                    + 矩形
                  </button>
                  <button
                    class="mini-btn"
                    @click="addShape(grp.id, 'vline')"
                  >
                    + 垂直線
                  </button>
                  <button
                    class="mini-btn"
                    @click="addShape(grp.id, 'hline')"
                  >
                    + 水平線
                  </button>
                </div>
                ...
                >
                  <g v-if="s.type === 'vline'">
                    ...
                    <template v-if="shapeEditMode === 'edit' && selectedShapeId === s.id">
                      <circle
                        ...
                        r="6"
                        class="shape-handle"
                        @pointerdown.stop="onShapePointerDown($event, grp.id, s, 'resize-vline-1')"
                      />
                      <circle
                        :cx="s.x"
                        :cy="s.y2"
                        r="6"
                        class="shape-handle"
                        @pointerdown.stop="onShapePointerDown($event, grp.id, s, 'resize-vline-2')"
                      />
                    </template>
                  </g>
                  <g v-else-if="s.type === 'hline'">
                    ...
                    <template v-if="shapeEditMode === 'edit' && selectedShapeId === s.id">
                      <circle
                        ...
                        r="6"
                        class="shape-handle"
                        @pointerdown.stop="onShapePointerDown($event, grp.id, s, 'resize-hline-1')"
                      />
                      <circle
                        :cx="s.x2"
                        :cy="s.y"
                        r="6"
                        class="shape-handle"
                        @pointerdown.stop="onShapePointerDown($event, grp.id, s, 'resize-hline-2')"
                      />
                    </template>
                  </g>
                  <g
                  ...
                  >
                    ...
                    <text
                      ...
                      class="shape-label"
                    >{{ assignedRoomFor(grp, s.id) ? assignedRoomFor(grp, s.id).id : s.id.replace('custom_', 'c') }}
                    </text>
                    <rect
          ...
          <div
            v-if="shapeEditMode === 'edit' && selectedShapeBuilding"
            class="shape-panel shape-panel-float"
          >
            <div class="flex items-center justify-between mb-2 gap-2">
              <span
                ...
                style="font-size:13.5px"
              >
                {{ selectedShapeBuilding.name }}・{{
                  selectedShapeInGroup(selectedShapeBuilding.id).id
                }}（{{ shapeTypeLabel[selectedShapeInGroup(selectedShapeBuilding.id).type] }}）
              </span>
              <div class="flex items-center gap-2">
                <button
                  class="mini-btn mini-danger"
                  @click="deleteSelectedShape(selectedShapeBuilding.id)"
                >
                  刪除
                </button>
                <button
            ...
            <div
              v-if="selectedShapeInGroup(selectedShapeBuilding.id).type === 'rect'"
              class="shape-panel-grid"
            >
              <label>X<input
                v-model.number="selectedShapeInGroup(selectedShapeBuilding.id).x"
                type="number"
                @change="commitSelectedShape(selectedShapeBuilding.id)"
              ></label>
              <label>Y<input
                v-model.number="selectedShapeInGroup(selectedShapeBuilding.id).y"
                type="number"
                @change="commitSelectedShape(selectedShapeBuilding.id)"
              ></label>
              <label>寬<input
                v-model.number="selectedShapeInGroup(selectedShapeBuilding.id).w"
                type="number"
                @change="commitSelectedShape(selectedShapeBuilding.id)"
              ></label>
              <label>高<input
                v-model.number="selectedShapeInGroup(selectedShapeBuilding.id).h"
                type="number"
                @change="commitSelectedShape(selectedShapeBuilding.id)"
              ></label>
            </div>
            ...
            <div
              v-if="selectedShapeInGroup(selectedShapeBuilding.id).type === 'rect'"
              class="shape-edge-controls"
            >
              <div
                v-for="edge in EDGES"
                :key="edge.key"
                class="shape-edge-row"
              >
                <span class="shape-edge-label">{{ edge.label }}</span>
                <button
                  class="mini-btn"
                  @click="adjustShapeEdge(selectedShapeBuilding.id, edge.key, -1)"
                >
                  －
                </button>
                <button
            ...
            <div
              v-else-if="selectedShapeInGroup(selectedShapeBuilding.id).type === 'vline'"
              class="shape-panel-grid"
            >
              <label>X<input
                v-model.number="selectedShapeInGroup(selectedShapeBuilding.id).x"
              ...
              ></label>
              <label>Y1<input
                v-model.number="selectedShapeInGroup(selectedShapeBuilding.id).y1"
                type="number"
                @change="commitSelectedShape(selectedShapeBuilding.id)"
              ></label>
              <label>Y2<input
                v-model.number="selectedShapeInGroup(selectedShapeBuilding.id).y2"
                type="number"
                @change="commitSelectedShape(selectedShapeBuilding.id)"
              ></label>
            </div>
            ...
            >
              ...
              ></label>
              <label>X1<input
                v-model.number="selectedShapeInGroup(selectedShapeBuilding.id).x1"
                type="number"
                @change="commitSelectedShape(selectedShapeBuilding.id)"
              ></label>
              <label>X2<input
                v-model.number="selectedShapeInGroup(selectedShapeBuilding.id).x2"
                type="number"
                @change="commitSelectedShape(selectedShapeBuilding.id)"
              ></label>
            </div>
          ...
          <div class="border-t border-light-c pt-2.5 mt-1">
            ...
            >
              ...
              <option
                ...
                :value="r.id"
              >
                {{ r.id }} {{ r.type }}・{{ r.buildingName }}・可住 {{ r.capacity }} 人・NT$ {{
                  r.price.toLocaleString()
                }}/晚
              </option>
            </select>
            ...
            <div class="grid grid-cols-12 gap-2 items-end">
              ...
              <div class="col-span-1">
                ...
                <input
                  ...
                  class="select-input"
                  style="width:100%"
                  @change="onGroupRowGuestsChange(idx)"
                >
              </div>
              <div class="col-span-2">
                <label
    ...
    >
      <div class="bg-surface rounded-2xl shadow-lg w-full max-w-lg p-5 max-h-[85vh] overflow-y-auto">
        ...
        <p
          ...
          style="font-size:12.5px"
        >
          共 {{ groupBookings.length }}
          間房{{ groupBookings[0] && groupBookings[0].phone ? '・' + groupBookings[0].phone : '' }}
        </p>
        ...
        >
          <div class="flex justify-between py-0.5">
            <span class="text-hint-c">房間</span><span class="text-base-c">{{
              bk.roomId ? roomLabel(bk.roomId) : '待指派'
            }}</span>
          </div>
          <div class="flex justify-between py-0.5">
    ...
    >
      <div class="bg-surface rounded-2xl shadow-lg w-full max-w-lg p-5 max-h-[85vh] overflow-y-auto">
        ...
        >
          <div class="flex justify-between py-0.5">
            <span class="text-hint-c">房客</span><span class="text-base-c">{{
              assignTarget.name
            }}（{{ assignTarget.guests }} 人）</span>
          </div>
          <div class="flex justify-between py-0.5">
            <span class="text-hint-c">入住 → 退房</span><span class="text-base-c">{{
              assignTarget.checkIn
            }} → {{ assignTarget.checkOut }}（{{ nights(assignTarget.checkIn, assignTarget.checkOut) }} 晚）</span>
          </div>
          <div
            ...
            >
              <div>
                ...
                <div
                  ...
                  style="font-size:12.5px"
                >
                  {{ r.buildingName }} ・ 可住 {{ r.capacity }} 人 ・ {{ r.bed }} ・ NT$ {{
                    r.price.toLocaleString()
                  }}/晚
                </div>
              </div>
              ...
              <div class="grid grid-cols-12 gap-2 items-end">
                ...
                <div class="col-span-2">
                  ...
                  <input
                    ...
                    class="select-input"
                    style="width:100%"
                    @change="onSplitRowGuestsChange(idx)"
                  >
                </div>
                <div class="col-span-3">
                  <label
      ...
      <div class="bg-surface rounded-2xl shadow-lg w-full max-w-sm p-5">
        ...
        <div v-if="tileTarget.bookings.length">
          ...
          >
            <div
              ...
              class="flex justify-between py-0.5"
            >
              <span class="text-hint-c" /><span class="status-badge bg-emerald-100 text-emerald-700">涵蓋 {{
                viewDate
              }}</span>
            </div>
            <div class="flex justify-between py-0.5">
            ...
            <div class="flex justify-between py-0.5">
              <span class="text-hint-c">入住 → 退房</span><span class="text-base-c">{{ bk.checkIn }} → {{
                bk.checkOut
              }}</span>
            </div>
            <div
        ...
        >
          ...
          <p
            ...
            style="font-size:13px"
          >
            {{ tileTarget.room.type }}・可住 {{ tileTarget.room.capacity }} 人・NT$
            {{ tileTarget.room.price.toLocaleString() }}/晚
          </p>
        </div>
        <div class="flex justify-end mt-4">
          <button
    ...
    <div
      ...
      class="fixed inset-0 bg-black/50 flex items-center justify-center z-30 px-4"
      @mousedown="onBackdropMousedown"
      @click="onBackdropClick($event, () => buildingModal.open = false)"
    >
      <div class="bg-surface rounded-2xl shadow-lg w-full max-w-sm p-5">
        <h2
          class="font-bold text-base-c mb-3"
    ...
    <div
      ...
      class="fixed inset-0 bg-black/50 flex items-center justify-center z-30 px-4"
      @mousedown="onBackdropMousedown"
      @click="onBackdropClick($event, () => roomModal.open = false)"
    >
      <div class="bg-surface rounded-2xl shadow-lg w-full max-w-sm p-5">
        <h2
          class="font-bold text-base-c mb-3"
        ...
        >
          {{
            roomModal.id ? '編輯房間' : '新增房間'
          }}（{{ roomModal.buildingName }}）
        </h2>
    ...
    <div
      ...
      class="fixed inset-0 bg-black/50 flex items-center justify-center z-30 px-4"
      @mousedown="onBackdropMousedown"
      @click="onBackdropClick($event, () => detailTarget = null)"
    >
      <div class="bg-surface rounded-2xl shadow-lg w-full max-w-sm p-5">
        <h2
          class="font-bold text-base-c mb-3"
    ...
    <!-- ===== 矩形對應 Modal ===== -->
    <div
      v-if="shapeAssign.open"
      class="fixed inset-0 bg-black/50 flex items-center justify-center z-30 px-4"
      @mousedown="onBackdropMousedown"
      @click="onBackdropClick($event, () => shapeAssign.open = false)"
    >
      <div class="bg-surface rounded-2xl shadow-lg w-full max-w-sm p-5">
        <h2
          class="font-bold text-base-c mb-3"
        ...
        <p
          ...
          style="font-size:12.5px"
        >
          如果選的房間已經對應到別的矩形，會自動把舊的對應清掉，避免一個矩形同時綁兩間房。
        </p>
        <div class="flex justify-end gap-2 mt-3">
          <button
            class="btn-plain"
            @click="shapeAssign.open = false"
          >
            取消
          </button>
          <button
...
.date-picker-cell-selected .date-picker-dot {
  background: #fff;
}
