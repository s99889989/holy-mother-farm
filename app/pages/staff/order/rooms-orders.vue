      ...
      <template v-else>
        ...
        <div v-else-if="tab === 'orders'">
          <div class="flex items-center justify-between flex-wrap gap-2 mb-4">
            ...
            <div class="segmented">
              <button
                :class="ordersView === 'list' ? 'seg-active' : ''"
                @click="ordersView = 'list'"
              >
                列表檢視
              </button>
              <button
                :class="ordersView === 'floorplan' ? 'seg-active' : ''"
                @click="ordersView = 'floorplan'"
              >
                平面圖檢視
              </button>
            </div>
            ...
            <div class="panel">
              ...
              >
                ...
                <RoomFloorplan
                  :building="grp"
                  :bookings="bookings"
                  :status-resolver="r => ({ cls: orderTileClass(r), label: orderTileLabel(r) })"
                  @select="room => room && openBookingTile(room)"
                />
              </div>
              <div
              ...
              <p
                ...
                style="font-size:11px"
              >
                ＊快樂運動館已依實際牆面繪製平面圖；合力居、愛加倍已依實際平面圖比例定位；懇親房目前無座標資料，暫以走廊示意圖顯示
              </p>
            </div>
          </div>
        </div>
    ...
    >
      <div class="bg-surface rounded-2xl shadow-lg w-full max-w-sm p-5">
        <h2
          ...
          style="font-size:15px"
        >
          {{ tileTarget.room.id }} · {{ buildingNameOf(tileTarget.room.buildingId) }}
        </h2>
        <div v-if="tileTarget.booking">
          ...
          >
            <div class="flex justify-between py-0.5">
              <span class="text-hint-c">訂單編號</span><span class="text-base-c">{{ tileTarget.booking.id }}</span>
            </div>
            <div class="flex justify-between py-0.5">
              <span class="text-hint-c">房客</span><span class="text-base-c">{{ tileTarget.booking.name }}（{{ tileTarget.booking.guests }} 人）</span>
            </div>
            <div class="flex justify-between py-0.5">
              <span class="text-hint-c">電話</span><span class="text-base-c">{{ tileTarget.booking.phone }}</span>
            </div>
            <div class="flex justify-between py-0.5">
              <span class="text-hint-c">入住 → 退房</span><span class="text-base-c">{{ tileTarget.booking.checkIn }} → {{ tileTarget.booking.checkOut }}</span>
            </div>
            <div
              v-if="tileTarget.booking.notes"
              class="flex justify-between py-0.5"
            >
              <span class="text-hint-c">備註</span><span class="text-base-c">{{ tileTarget.booking.notes }}</span>
            </div>
            <div class="flex justify-between py-1 mt-1 border-t border-light-c">
              <span class="text-hint-c">狀態</span><span
              class="status-badge"
              :class="statusClass(tileTarget.booking.status)"
            >{{ statusLabel(tileTarget.booking.status) }}</span>
            </div>
          </div>
          <div class="flex gap-2 flex-wrap">
            <button
              v-if="tileTarget.booking.status === 'pending'"
              class="mini-btn mini-primary"
              @click="quickSetTile('confirmed')"
            >
              確認訂單
            </button>
            <button
              v-if="tileTarget.booking.status === 'pending'"
              class="mini-btn"
              @click="openAssign(tileTarget.booking); tileTarget = null"
            >
              更換房間
            </button>
            <button
              v-if="tileTarget.booking.status === 'confirmed'"
              class="mini-btn"
              @click="quickSetTile('completed')"
            >
              設為已退房
            </button>
            <button
              class="mini-btn"
              @click="quickSetTile('cancelled')"
            >
              取消訂單
            </button>
          </div>
