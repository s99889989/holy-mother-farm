    ...
    <header class="bg-surface border-b border-light-c px-4 py-3 sticky top-0 z-20">
      ...
      <div class="mx-auto mt-1" :class="wideTab ? 'w-full' : 'max-w-5xl'">
        <div class="segmented w-fit">
          ...
          <button :class="tab === 'sheet' ? 'seg-active' : ''" :style="tab === 'sheet' ? segActiveStyle : ''" @click="tab = 'sheet'">訂房總表</button>
    ...
    <div class="mx-auto px-3 sm:px-4 py-4" :class="wideTab ? 'w-full' : 'max-w-5xl'">
      ...
      <template v-else>
            ...
            <table class="w-full">
              ...
              <template v-for="row in orderRows" :key="row.kind === 'groupHeader' ? ('grp_' + row.groupId) : row.booking.id">
                ...
                <template v-if="row.kind !== 'groupHeader'">
                  <tr v-if="row.showBuildingHeader" class="border-t border-light-c" :class="row.inGroup ? 'group-member-row' : ''">
                    <td class="py-1 text-hint-c font-semibold" colspan="10" style="font-size:11.5px">🏠 {{ row.buildingName }}</td>
                  </tr>
                  <tr v-for="b in [row.booking]" :key="b.id" class="border-t border-light-c" :class="row.inGroup ? 'group-member-row' : ''" style="font-size:13.5px">
                    <td class="py-2 text-base-c">{{ b.id }}</td>
                    <td class="py-2">
                      <template v-if="b.roomId">
                        <div v-if="!row.inGroup && row.buildingName" class="text-hint-c" style="font-size:11px">{{ row.buildingName }}</div>
        ...
        <div v-else-if="tab === 'sheet'">
          <div class="flex items-center justify-between flex-wrap gap-2 mb-4">
            <div class="flex items-center gap-2 flex-wrap">
              <input type="text" v-model="sheetKeyword" placeholder="搜尋客戶／團體／房號" class="select-input">
              <span class="text-hint-c" style="font-size:12.5px">快速選月</span>
              <button class="mini-btn" @click="shiftSheetMonth(-1)">◀上月</button>
              <input type="month" v-model="sheetMonthPick" @change="applySheetMonth(sheetMonthPick)" class="select-input">
              <button class="mini-btn" @click="shiftSheetMonth(1)">下月▶</button>
              <button class="mini-btn" @click="resetSheetMonth">本月</button>
              <span class="text-hint-c" style="font-size:12.5px">時段</span>
              <input type="date" v-model="sheetDateFrom" class="select-input">
              <span class="text-hint-c">～</span>
              <input type="date" v-model="sheetDateTo" class="select-input">
              <button v-if="sheetDateFrom || sheetDateTo" class="mini-btn" @click="clearSheetMonth">清除時段</button>
            </div>
            <span class="text-hint-c" style="font-size:13px">共 {{ sheetRows.length }} 筆</span>
          </div>
          ...
          <div class="panel">
            <table class="w-full sheet-table">
              <thead>
              <tr class="text-hint-c text-left" style="font-size:12px">
                <th class="py-1.5 font-semibold sheet-col-num">編號</th>
                <th class="py-1.5 font-semibold sheet-col-date">入退日期</th>
                <th class="py-1.5 font-semibold sheet-col-name">客戶</th>
                <th v-for="bd in buildings" :key="bd.id" class="py-1.5 font-semibold text-center">{{ bd.name }}</th>
              </tr>
              </thead>
              <tbody>
              <tr v-for="(row, idx) in sheetRows" :key="row.key" class="border-t border-light-c sheet-row" :class="row.groupId ? 'group-member-row' : ''" style="font-size:13.5px">
                <td class="py-2 text-hint-c">{{ idx + 1 }}</td>
                <td class="py-2 text-base-c whitespace-nowrap">{{ sheetDateRangeLabel(row.checkIn, row.checkOut) }}</td>
                <td class="py-2">
                  <button class="text-left text-base-c font-semibold" style="text-decoration:underline;text-underline-offset:2px" @click="row.groupId ? openGroupTarget(row.groupId, row.groupName) : openEditOrder(row.members[0])">
                    {{ row.name }}
                  </button>
                  <span v-if="row.groupId" class="status-badge bg-violet-200 text-violet-800" style="margin-left:5px">團體</span>
                </td>
                <td v-for="bd in buildings" :key="bd.id" class="py-2 text-center" style="font-size:13px">
                  <span v-if="row.byBuilding[bd.id].includes('全包')" class="status-badge bg-amber-100 text-amber-700">全包</span>
                  <span v-else-if="row.byBuilding[bd.id].length" class="text-base-c">{{ row.byBuilding[bd.id].join('、') }}</span>
                  <span v-else class="text-hint-c">—</span>
                </td>
              </tr>
              <tr v-if="sheetRows.length === 0"><td :colspan="3 + buildings.length" class="text-center text-hint-c py-6" style="font-size:13.5px">沒有符合條件的訂單</td></tr>
              </tbody>
            </table>
          </div>
        </div>
      ...
      <div class="bg-surface rounded-2xl shadow-lg w-full max-w-lg p-5 max-h-[85vh] overflow-y-auto">
        <div v-if="!groupEditing" class="flex items-start justify-between gap-2 mb-1">
          ...
          <div class="flex gap-2 flex-wrap justify-end">
            ...
            <button class="mini-btn mini-primary" @click="startGroupAdd">+ 新增房間</button>
          </div>
        </div>
        ...
        <div v-if="groupAdding" class="bg-surface2 rounded-lg p-3 mb-3">
          <p class="text-hint-c mb-2" style="font-size:12.5px">會用這個團體原本的入住/退房日期（{{ groupCheckIn }} → {{ groupCheckOut }}）新增一間房，掛在同一個團體底下。</p>
          <div class="grid grid-cols-12 gap-2 items-end">
            <div class="col-span-7">
              <label class="block text-hint-c mb-1" style="font-size:12px">房間</label>
              <select v-model="groupAddRoom.roomId" class="select-input" style="width:100%">
                <option value="">選擇房間</option>
                <option v-for="r in eligibleRoomsForGroupAdd" :key="r.id" :value="r.id">
                  {{ r.id }} {{ r.type }}・{{ r.buildingName }}・可住 {{ r.capacity }} 人
                </option>
              </select>
            </div>
            <div class="col-span-2">
              <label class="block text-hint-c mb-1" style="font-size:12px">人數</label>
              <input type="number" min="1" v-model.number="groupAddRoom.guests" class="select-input" style="width:100%">
            </div>
            <div class="col-span-3">
              <button class="mini-btn" style="width:100%" @click="openRoomPicker('groupAdd', groupCheckIn, groupCheckOut, groupAddRoom.guests, groupBookings.map(b => b.roomId).filter(Boolean))">在平面圖選房間</button>
            </div>
          </div>
          <div class="grid grid-cols-2 gap-2 mt-2">
            <div>
              <label class="block text-hint-c mb-1" style="font-size:12px">聯絡人（留空用團體名稱）</label>
              <input type="text" v-model="groupAddRoom.name" class="select-input" style="width:100%" placeholder="選填">
            </div>
            <div>
              <label class="block text-hint-c mb-1" style="font-size:12px">備註</label>
              <input type="text" v-model="groupAddRoom.notes" class="select-input" style="width:100%" placeholder="選填">
            </div>
          </div>
          <p v-if="groupCheckIn && eligibleRoomsForGroupAdd.length === 0" class="text-hint-c mt-1" style="font-size:12px">這個日期沒有空房可選了（或已經被這個團體用掉）</p>
          <p v-if="groupAddError" class="text-red-500 mt-2" style="font-size:13px">{{ groupAddError }}</p>
          <div class="flex justify-end gap-2 mt-3">
            <button class="btn-plain" @click="groupAdding = false">取消</button>
            <button class="mini-btn mini-primary" :disabled="groupAddSaving" @click="submitAddRoomToGroup">{{ groupAddSaving ? '處理中...' : '加入這間房' }}</button>
          </div>
        </div>
        ...
        <div v-if="groupEditing" class="bg-surface2 rounded-lg p-3 mb-3">
          <div class="flex flex-col gap-2">
            ...
            <div class="grid grid-cols-2 gap-2">
              <div>
                <label class="block text-hint-c mb-1" style="font-size:13px">入住日期</label>
                <input type="date" v-model="groupEditForm.checkIn" class="select-input" style="width:100%">
              </div>
              <div>
                <label class="block text-hint-c mb-1" style="font-size:13px">退房日期</label>
                <input type="date" v-model="groupEditForm.checkOut" class="select-input" style="width:100%">
              </div>
            </div>
          </div>
          ...
          <p class="text-hint-c mt-2" style="font-size:12px">會套用到這個團體底下每一間房的訂單；個別房間的房客姓名/人數請用下面每筆訂單的「編輯」</p>
        </div>
        ...
        <div v-for="bk in groupBookings" :key="bk.id" class="bg-surface2 rounded-lg p-3 mb-2.5" style="font-size:13px">
          ...
          <div class="flex items-center justify-between py-1 mt-1 border-t border-light-c">
            ...
            <div class="flex gap-2">
              ...
              <button class="mini-btn mini-danger" @click="removeRoomFromGroup(bk)">移除</button>
            </div>
          </div>
        </div>
