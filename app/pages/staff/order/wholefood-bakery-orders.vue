<script setup>
  ...
  definePageMeta({ layout: 'staff', requiredPermission: 'order.wholefood-bakery-orders' })

  const commonStore = useCommonStore()
  const BASE = computed(() => commonStore.data.main_url + '/holy/wholefood-bakery')
  ...
  // ── 供應日排程 ────────────────────────────────────────────────────
  const businessDays = ref([2, 3, 4])
  const businessDaysDraft = ref([2, 3, 4])
  ...
  const removeBusinessDaysScheduleEntry = async (effectiveFrom) => {
    if (!confirm(`確定要刪除 ${effectiveFrom} 起生效的供應日排程？`)) return
    try {
  ...
  // ── 分類設定 ────────────────────────────────────────────────────
  // categories：已儲存的分類（含 itemCount）；categoriesDraft：面板編輯中的草稿，originalName 用來讓後端辨識改名
  ...
  function setCategories(list) {
    ...
    categoriesDraft.value = list.map(c => ({ ...c, originalName: c.name }))
  }

  // countsOnly：只更新各分類的品項數，不覆蓋分類面板中尚未儲存的草稿
  const fetchCategories = async (countsOnly = false) => {
    try {
      const res = await fetch(`${BASE.value}/admin/settings/categories`, { credentials: 'include' })
      const data = await res.json()
      ...
      for (const d of categoriesDraft.value) {
        const saved = data.categories.find(c => c.name === d.originalName)
        if (saved) d.itemCount = saved.itemCount
      }
    } catch {
    }
  }
  ...
  function addCategoryRow() {
    categoriesDraft.value.push({ name: '', emoji: '🍞', active: true, originalName: '', itemCount: 0 })
  }

  function removeCategoryRow(idx) {
    const c = categoriesDraft.value[idx]
    // 品項數以「已儲存」的品項為準，品項面板改了分類要先儲存
    if (c.originalName && (c.itemCount || 0) > 0) {
      alert(`「${c.originalName}」底下還有 ${c.itemCount} 個品項，請先在品項設定把它們移到其他分類並儲存`)
      return
    }
    categoriesDraft.value.splice(idx, 1)
  }

  function moveCategoryRow(idx, delta) {
    const to = idx + delta
    if (to < 0 || to >= categoriesDraft.value.length) return
    const list = categoriesDraft.value
    ;[list[idx], list[to]] = [list[to], list[idx]]
  }

  async function saveCategories() {
    const names = categoriesDraft.value.map(c => (c.name || '').trim())
    if (names.some(n => !n)) {
      alert('分類名稱不能為空')
      return
    }
    ...
    try {
      const payload = categoriesDraft.value.map(c => ({
        name: c.name.trim(), emoji: (c.emoji || '').trim(), active: !!c.active, originalName: c.originalName || ''
      }))
      const res = await fetch(`${BASE.value}/admin/settings/categories`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        credentials: 'include', body: JSON.stringify({ categories: payload })
      })
      const data = await res.json()
      if (data.error) {
        alert('儲存失敗：' + data.error)
      ...
      // 改名要同步到品項草稿，避免品項面板還沒儲存的修改被舊分類名稱蓋回去
      const renames = {}
      ...
      setTimeout(() => categoriesSaved.value = false, 2000)
    } catch {
      alert('儲存失敗')
    } finally {
  ...
  // 依分類分組：照分類設定的順序排，分類內保留品項清單順序；未分類／分類不存在的歸入最後的「未分類」
  function groupByCategory(list) {
    const map = {}
    for (const it of list) {
      const known = categories.value.some(c => c.name === it.category)
      const key = known ? it.category : '未分類'
      ;(map[key] ||= []).push(it)
    }
    const groups = categories.value
      .filter(c => map[c.name]?.length)
      .map(c => ({ name: c.name, emoji: c.emoji, active: c.active, items: map[c.name] }))
    if (map['未分類']?.length) groups.push({ name: '未分類', emoji: '', active: true, items: map['未分類'] })
    return groups
  }
  ...
  async function saveItems() {
    ...
    try {
      ...
      fetchCategories(true) // 更新各分類的品項數
    ...
    return Array.from(map.entries())
      ...
      .map(([date, list]) => {
        ...
        const breakdown = Object.entries(qtyMap)
          ...
          .join('、')
        return { date, dateLabel: `${m}/${d}（週${weekDay}）取貨`, orders: list, amount, breakdown }
      })
  })
    ...
    <div class="lg:flex lg:gap-6 lg:items-start">
      ...
      <div class="space-y-3 mb-3 lg:mb-0 lg:w-80 xl:w-96 lg:flex-shrink-0 lg:sticky lg:top-4">
        ...
        <div class="bg-surface border border-light-c rounded-2xl">
          ...
          >
            <span>供應日設定（目前：{{ businessDayOptions.map(o => o.label).join('、') }}）</span>
        ...
        <div class="bg-surface border border-light-c rounded-2xl">
          ...
          >
            <span>分類設定（共 {{ categories.length }} 類）</span>
            <span>{{ categoriesPanelOpen ? '收合 ▲' : '展開 ▼' }}</span>
          </button>
          <div
            v-if="categoriesPanelOpen"
            class="px-4 pb-4 space-y-2"
          >
            <div
              v-for="(cat, idx) in categoriesDraft"
              :key="idx"
              class="flex items-center gap-2 bg-surface2 rounded-lg px-3 py-2"
            ...
            >
              <input
                ...
                placeholder="圖示"
                maxlength="4"
                class="w-11 px-1 py-1 text-center text-sm border border-light-c rounded bg-surface text-base-c"
              >
              <input
                v-model="cat.name"
                placeholder="分類名稱"
                class="flex-1 min-w-[80px] px-2 py-1 text-xs border border-light-c rounded bg-surface text-base-c"
              >
              <span
                class="text-[11px] text-hint-c whitespace-nowrap"
                :title="cat.originalName && cat.originalName !== cat.name ? `原名稱：${cat.originalName}` : ''"
              >
                {{ cat.itemCount || 0 }} 項<span
                v-if="cat.originalName && cat.originalName !== cat.name"
                class="text-amber-700"
              >・改名</span>
              </span>
              <label class="flex items-center gap-1 text-xs text-hint-c whitespace-nowrap">
                <input
                  v-model="cat.active"
                  type="checkbox"
                > 啟用
              ...
              <button
                ...
                :disabled="idx === 0"
                title="上移"
                @click="moveCategoryRow(idx, -1)"
              >
                ▲
              </button>
              <button
                class="w-6 h-6 text-xs border border-light-c rounded bg-surface text-hint-c disabled:opacity-30"
                :disabled="idx === categoriesDraft.length - 1"
                title="下移"
                @click="moveCategoryRow(idx, 1)"
              >
                ▼
            ...
            <p class="text-[11px] text-hint-c leading-relaxed">
              順序即前台分區順序。改名會自動更新所屬品項；停用後整個分類在前台隱藏、不能下單（品項上架狀態不變）；還有品項的分類不能刪除。
            </p>
            <div class="flex items-center gap-2 pt-1">
              ...
              >
                {{ categoriesSaving ? '儲存中…' : (categoriesSaved ? '已儲存 ✓' : '儲存分類設定') }}
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="bg-surface border border-light-c rounded-2xl">
          ...
          >
            <span>品項設定（共 {{ items.length }} 款）</span>
            ...
            >
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <select
                  ...
                  class="w-28 px-2 py-1 text-xs border border-light-c rounded bg-surface text-base-c"
                >
                  <option value="">
                    未分類
                  </option>
      ...
      <div class="lg:flex-1 lg:min-w-0">
        ...
        <div class="flex flex-wrap items-center gap-2 mb-4 md:justify-between">
          ...
          <div class="flex items-center gap-2">
            <!-- 客戶訂購連結 -->
            <a
              href="https://holyfarm.netlify.app/front/order/wholefood-bakery?og=20"
              target="_blank"
          ...
          <div class="bg-surface rounded-2xl shadow-xl w-full max-w-md lg:max-w-lg p-5 max-h-[85vh] overflow-y-auto">
            ...
            <div class="space-y-3">
              ...
              <div>
                ...
                >
                  <p class="text-[11px] font-semibold text-amber-700 mb-1">
                    {{ group.emoji }} {{ group.name }}<span
                    v-if="!group.active"
                    class="ml-1 text-hint-c font-normal"
                  >（前台停用中）</span>
                  </p>
