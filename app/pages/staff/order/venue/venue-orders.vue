            ...
            <table v-else class="w-full" style="font-size:13px">
              <tbody>
                <tr v-for="b in upcomingBookings" :key="b.id" class="border-t border-light-c">
                  <td class="py-2">{{ b.date }} {{ b.startTime }}–{{ b.endTime }}</td>
              ...
              <tbody>
                <template v-for="row in visibleOrderRows" :key="row.key">
                  ...
                  <tr v-else :class="row.isGroupMember ? 'group-member-row' : ''" class="border-t border-light-c">
                    <td class="py-2">{{ row.date }} {{ row.startTime }}–{{ row.endTime }}</td>
              ...
              <div v-for="(cell, i) in calendarCells" :key="i"
                ...
                <template v-if="cell">
                  ...
                  <div v-for="b in cell.bookings.slice(0, 3)" :key="b.key" class="cal-pill" :style="{ background: b.isGroup ? '#7c3aed' : statusColor(b.status) }">
                    {{ b.isGroup ? `${b.groupName || '團體'}（${b.members.length}）` : `${b.startTime} ${venueLabel(b.venueId)}` }}
                  </div>
          ...
          <div v-if="dayTarget" class="panel" style="width:320px;flex-shrink:0">
            ...
            <div v-for="b in dayBookings" :key="b.key" class="border border-light-c rounded-lg p-2.5 mb-2">
              <div class="flex items-center justify-between">
                <span class="font-bold" style="font-size:13px">{{ b.startTime }}–{{ b.endTime }}</span>
              ...
              <template v-for="v in activeVenues" :key="v.id">
                ...
                <div class="gantt-room-track" :style="{ gridColumn: `2 / span ${ganttHours.length}` }">
                  <div v-for="b in ganttBarsForVenue(v.id)" :key="b.key"
                       class="gantt-bar" :style="b.style" :title="`${b.name} ${b.startTime}-${b.endTime}`"
      ...
      <div v-if="newOrderOpen" class="modal-mask" @click.self="newOrderOpen = false">
        <div class="modal-box">
          ...
          <div class="grid grid-cols-3 gap-2 mb-2">
            <input v-model="newOrder.date" type="date" class="select-input">
            <input v-model="newOrder.startTime" type="time" class="select-input">
            <input v-model="newOrder.endTime" type="time" class="select-input">
          </div>
          <div class="mb-2">
    ...
    <Teleport to="body">
      <div v-if="editOrderOpen" class="modal-mask" @click.self="editOrderOpen = false">
        <div class="modal-box">
          ...
          <div class="grid grid-cols-3 gap-2 mb-2">
            <input v-model="editOrder.date" type="date" class="select-input">
            <input v-model="editOrder.startTime" type="time" class="select-input">
            <input v-model="editOrder.endTime" type="time" class="select-input">
          </div>
          <div class="mb-2">
    ...
    <Teleport to="body">
      <div v-if="assignOpen" class="modal-mask" @click.self="assignOpen = false">
        <div class="modal-box">
          ...
          <div class="mb-2 text-hint-c" style="font-size:12.5px">{{ assignTarget?.date }} {{ assignTarget?.startTime }}–{{ assignTarget?.endTime }} ・ {{ assignTarget?.guests }} 人</div>
      ...
      <div v-if="newGroupOpen" class="modal-mask" @click.self="newGroupOpen = false">
        <div class="modal-box" style="max-width:640px">
          ...
          <div class="grid grid-cols-3 gap-2 mb-3">
            <input v-model="newGroup.date" type="date" class="select-input">
            <input v-model="newGroup.startTime" type="time" class="select-input">
            <input v-model="newGroup.endTime" type="time" class="select-input">
          </div>
        ...
        <div class="modal-box" style="max-width:600px">
          ...
          <div v-for="b in groupManageTarget" :key="b.id" class="border border-light-c rounded-lg p-2.5 mb-2 flex items-center justify-between gap-2">
            <div>
              ...
              <div class="text-hint-c" style="font-size:11.5px">{{ b.date }} {{ b.startTime }}–{{ b.endTime }} ・ {{ b.guests }} 人</div>
            </div>
...
<script setup>
definePageMeta({ layout: 'staff', requiredPermission: 'order.venue-orders' })
...
const bookings = ref([])   // [{id,venueId,name,phone,email,guests,date,startTime,endTime,venuePref,notes,groupId,groupName,status,createdAt}]
...
const todaysBookings = computed(() => bookings.value.filter(b => b.date === todayStr && b.status !== 'cancelled'))
const upcomingBookings = computed(() =>
  bookings.value.filter(b => b.date >= todayStr && ['unassigned', 'pending', 'confirmed'].includes(b.status))
    .sort((a, b) => (a.date + a.startTime).localeCompare(b.date + b.startTime)).slice(0, 10)
)
...
const filteredBookings = computed(() => {
  ...
  return bookings.value.filter(b => {
    ...
    if (ordersDateFrom.value && b.date < ordersDateFrom.value) return false
    if (ordersDateTo.value && b.date > ordersDateTo.value) return false
    if (kw) {
...
const orderRows = computed(() => {
  ...
  singles.sort((a, b) => (a.date + a.startTime).localeCompare(b.date + b.startTime))
...
function bookingsOnDate(dateStr) {
  const dayBs = bookings.value.filter(b => b.date === dateStr && b.status !== 'cancelled')
  const groups = new Map()
  const result = []
  for (const b of dayBs) {
    if (b.groupId) {
      if (!groups.has(b.groupId)) { groups.set(b.groupId, { key: 'g_' + b.groupId, isGroup: true, groupId: b.groupId, groupName: b.groupName, members: [], status: b.status, startTime: b.startTime }); result.push(groups.get(b.groupId)) }
      groups.get(b.groupId).members.push(b)
    } else {
      result.push({ ...b, key: b.id })
    }
  }
...
function ganttBarsForVenue(venueId) {
  ...
  return bookings.value
    .filter(b => b.venueId === venueId && b.date === ganttDate.value && b.status !== 'cancelled')
    .map(b => {
      const startMin = Math.max(timeToMinutes(b.startTime) - GANTT_START * 60, 0)
      const endMin = Math.min(timeToMinutes(b.endTime) - GANTT_START * 60, spanMin)
...
function overviewBookingsFor(venueId) {
  return bookings.value.filter(b => b.venueId === venueId && b.date === overviewDate.value && b.status !== 'cancelled')
...
async function refreshOccupied(date, startTime, endTime, excludeId) {
  pickerOccupied.value = {}
  if (!date || !startTime || !endTime) return
  try {
    const qs = new URLSearchParams({ date, startTime, endTime, excludeId: excludeId || '' })
...
const newOrder = reactive({ venueId: '', date: '', startTime: '', endTime: '', guests: 1, name: '', phone: '', email: '', notes: '' })
function openNewOrder() {
  Object.assign(newOrder, { venueId: '', date: '', startTime: '', endTime: '', guests: 1, name: '', phone: '', email: '', notes: '' })
  newOrderError.value = ''
  newOrderOpen.value = true
}
watch(() => [newOrder.date, newOrder.startTime, newOrder.endTime], () => {
  if (newOrderOpen.value) refreshOccupied(newOrder.date, newOrder.startTime, newOrder.endTime, '')
})
async function submitNewOrder() {
  newOrderError.value = ''
  if (!newOrder.date || !newOrder.startTime || !newOrder.endTime) { newOrderError.value = '請填寫日期與起訖時間'; return }
...
const editOrder = reactive({ id: '', venueId: '', date: '', startTime: '', endTime: '', guests: 1, name: '', phone: '', email: '', notes: '' })
...
function openEditOrder(b) {
  Object.assign(editOrder, { id: b.id, venueId: b.venueId, date: b.date, startTime: b.startTime, endTime: b.endTime, guests: b.guests, name: b.name, phone: b.phone, email: b.email, notes: b.notes })
  editOrderError.value = ''
  editOrderOpen.value = true
  refreshOccupied(b.date, b.startTime, b.endTime, b.id)
}
watch(() => [editOrder.date, editOrder.startTime, editOrder.endTime], () => {
  if (editOrderOpen.value) refreshOccupied(editOrder.date, editOrder.startTime, editOrder.endTime, editOrder.id)
})
async function submitEditOrder() {
  editOrderError.value = ''
  if (!editOrder.date || !editOrder.startTime || !editOrder.endTime) { editOrderError.value = '請填寫日期與起訖時間'; return }
...
const newGroup = reactive({ groupName: '', phone: '', date: '', startTime: '', endTime: '', rows: [{ venueId: '', guests: 1, notes: '' }] })
function openNewGroup() {
  Object.assign(newGroup, { groupName: '', phone: '', date: '', startTime: '', endTime: '', rows: [{ venueId: '', guests: 1, notes: '' }] })
  newGroupError.value = ''
...
watch(() => [newGroup.date, newGroup.startTime, newGroup.endTime], () => {
  if (newGroupOpen.value) refreshOccupied(newGroup.date, newGroup.startTime, newGroup.endTime, '')
})
async function submitNewGroup() {
  newGroupError.value = ''
  if (!newGroup.date || !newGroup.startTime || !newGroup.endTime) { newGroupError.value = '請填寫日期與起訖時間'; return }
  ...
  try {
    ...
    const first = await (await fetch(`${BOOKINGS_BASE()}/request`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ date: newGroup.date, startTime: newGroup.startTime, endTime: newGroup.endTime, guests: rows[0].guests, name: newGroup.groupName.trim(), phone: newGroup.phone.trim(), notes: rows[0].notes }),
    })).json()
