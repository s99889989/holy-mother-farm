<script setup>
  ...
  // ── 24 小時制時間選擇（比照訂位頁面，避免原生 time input 出現上午/下午） ──
  const HOUR_OPTIONS = Array.from({ length: 24 }, (_, i) => String(i).padStart(2, '0'))
  const MINUTE_OPTIONS = Array.from({ length: 12 }, (_, i) => String(i * 5).padStart(2, '0'))
  // 給定 reactive 物件與欄位名稱（值為 "HH:mm" 字串），回傳可分別綁定「時」「分」的 computed；欄位允許空字串（未填時間）
  const timePart = (obj, key, part) => computed({
    get: () => {
      if (!obj[key]) return ''
      const [h, m] = obj[key].split(':')
      return part === 'h' ? h : m
    },
    set: (v) => {
      const [h, m] = (obj[key] || '00:00').split(':')
      obj[key] = part === 'h' ? `${v}:${m || '00'}` : `${h || '00'}:${v}`
    }
  })
  ...
  // 月曆格子上點活動：手機版一律先開側板（避免小螢幕直接跳一堆詳情 modal），
  // 電腦版才維持原本「點了直接開詳情」
  function handleEventClick(ev, cell) {
    if (isMobileViewport.value) {
      openDayPanel(cell)
      return
    }
    if (isDetailClickable(ev)) openEventDetail(ev)
  }
  ...
  <div class="min-h-full bg-surface2 dark:bg-[#15171c] transition-colors lg:flex lg:items-start">
          ...
          >
                ...
                <template v-if="cell.day">
                  ...
                  >
                    <div
                      ...
                      @click.stop="handleEventClick(ev, cell)"
            ...
            >
              <div
                ...
                @click.stop="handleEventClick(b.ev, week[b.startCol])"
    ...
    >
      <div class="bg-surface dark:bg-[#15171c] rounded-2xl shadow-xl w-full sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div class="px-5 py-4 border-t border-light-c dark:border-[#2a2e37] flex flex-wrap gap-2 justify-end sticky bottom-0 bg-surface dark:bg-[#15171c]">
          <button
            class="px-2.5 py-1 text-[11px] whitespace-nowrap bg-surface2 dark:bg-[#1c1f26] text-muted-c rounded-xl hover:bg-surface2 transition-colors"
            @click="formModal.show = false"
          >
            取消
          </button>
          <button
            :disabled="saving"
            class="px-2.5 py-1 text-[11px] whitespace-nowrap bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 disabled:bg-indigo-300 transition-colors flex items-center gap-1.5"
            @click="saveForm"
          >
            <div
              v-if="saving"
    ...
    >
      <div class="bg-surface dark:bg-[#15171c] rounded-2xl shadow-xl w-full sm:max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div class="px-5 py-4 border-t border-light-c dark:border-[#2a2e37] flex flex-wrap gap-2 justify-end">
          <button
            class="px-2.5 py-1 text-[11px] whitespace-nowrap bg-surface2 dark:bg-[#1c1f26] text-muted-c rounded-xl hover:bg-surface2 transition-colors"
            @click="closeClearMonthModal"
          >
          ...
          <button
            ...
            class="px-2.5 py-1 text-[11px] whitespace-nowrap bg-red-500 text-white rounded-xl hover:bg-red-600 disabled:bg-red-200 disabled:cursor-not-allowed transition-colors flex items-center gap-1.5"
    ...
    >
      <div class="bg-surface dark:bg-[#15171c] rounded-2xl shadow-xl w-full sm:max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div class="px-5 py-4 border-t border-light-c dark:border-[#2a2e37] flex flex-wrap gap-2 justify-end sticky bottom-0 bg-surface dark:bg-[#15171c]">
          <button
            class="px-2.5 py-1 text-[11px] whitespace-nowrap border border-red-200 dark:border-red-900/50 text-red-500 dark:text-red-400 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
            @click="deleteGoogleFromDetail"
          >
            刪除
          </button>
          <button
            class="px-2.5 py-1 text-[11px] whitespace-nowrap bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors"
          ...
          <a
            ...
            target="_blank"
            rel="noopener"
            class="px-2.5 py-1 text-[11px] whitespace-nowrap bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors flex items-center gap-1.5"
          >
            <svg
              class="w-3.5 h-3.5"
              fill="none"
    ...
    >
      <div class="bg-surface dark:bg-[#15171c] rounded-2xl shadow-xl w-full sm:max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div class="px-5 py-4 border-t border-light-c dark:border-[#2a2e37] flex flex-wrap gap-2 justify-end sticky bottom-0 bg-surface dark:bg-[#15171c]">
          ...
          <button
            class="px-2.5 py-1 text-[11px] whitespace-nowrap bg-teal-600 text-white rounded-xl hover:bg-teal-700 transition-colors"
      ...
      <div class="bg-surface dark:bg-[#15171c] rounded-2xl shadow-xl w-full sm:max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div class="px-5 py-4 border-t border-light-c dark:border-[#2a2e37] flex flex-wrap gap-2 justify-end sticky bottom-0 bg-surface dark:bg-[#15171c]">
          ...
          <a
            ...
            class="px-2.5 py-1 text-[11px] whitespace-nowrap bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors"
          >
            前往管理
          </a>
        </div>
      ...
      <div class="bg-surface dark:bg-[#15171c] rounded-2xl shadow-xl w-full sm:max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div class="px-5 py-4 border-t border-light-c dark:border-[#2a2e37] flex flex-wrap gap-2 justify-end sticky bottom-0 bg-surface dark:bg-[#15171c]">
          ...
          <a
            ...
            class="px-2.5 py-1 text-[11px] whitespace-nowrap bg-lime-600 text-white rounded-xl hover:bg-lime-700 transition-colors"
          >
            前往豆漿管理
          </a>
        </div>
        ...
        <div class="px-5 py-4 border-t border-light-c dark:border-[#2a2e37] flex flex-wrap gap-2 justify-end sticky bottom-0 bg-surface dark:bg-[#15171c]">
          ...
          <template v-if="!txtResult">
            <button
              ...
              class="px-2.5 py-1 text-[11px] whitespace-nowrap bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 disabled:opacity-50 transition-colors"
              @click="parseTxt"
            >
              解析預覽
            </button>
          ...
          <template v-else>
            <button
              class="px-2.5 py-1 text-[11px] whitespace-nowrap border border-light-c dark:border-[#2a2e37] text-muted-c rounded-xl hover:bg-surface2 transition-colors"
            ...
            <button
              ...
              class="px-2.5 py-1 text-[11px] whitespace-nowrap bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 disabled:opacity-50 transition-colors flex items-center gap-1.5"
