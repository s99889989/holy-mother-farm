<template>
      ...
      <header class="bg-surface border-b border-light-c px-4 py-3 sticky top-0 z-30">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <div>
              <h1 class="font-bold text-base-c leading-none text-sm sm:text-base">工作管理</h1>
 ...
 : 'text-hint-c dark:text-hint-c hover-surface2'"
      ...
      <div class="max-w-6xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
        ...
        <div v-if="activeTab === 'overview'">
          ...
          <div class="flex flex-wrap items-center gap-3 mb-4">
            <div class="flex items-center gap-2">
              <span class="text-lg font-bold text-base-c">{{ todayLabel }}</span>
            ...
            <div class="flex flex-wrap gap-1.5">
              <span v-for="s in staffList" :key="s.id"
                    :class="isOnLeave(s.id) ? 'bg-surface2 text-hint-c line-through' : 'bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300'"
          ...
          <div class="grid grid-cols-2 gap-3 mb-5">
            <div class="bg-surface rounded-2xl border border-light-c p-4 shadow-sm">
              <div class="text-xs text-hint-c mb-1">必要工作</div>
              <div class="flex items-end gap-2">
                <span class="text-2xl font-bold text-base-c">
                  {{ summary.requiredDone }} <span class="text-base font-normal text-hint-c">/ {{ summary.requiredTotal }}</span>
                </span>
              </div>
              ...
              <div class="text-xs text-hint-c mt-1">{{ reqPct }}% 完成</div>
            </div>
            <div class="bg-surface rounded-2xl border border-light-c p-4 shadow-sm">
              <div class="text-xs text-hint-c mb-1">選做工作</div>
              <div class="flex items-end gap-2">
                <span class="text-2xl font-bold text-base-c">
                  {{ summary.optionalDone }} <span class="text-base font-normal text-hint-c">/ {{ summary.optionalTotal }}</span>
                </span>
              </div>
              <div class="mt-2 h-1.5 bg-surface2 rounded-full overflow-hidden">
              ...
              <div class="text-xs text-hint-c mt-1">{{ optPct }}% 完成</div>
            </div>
          </div>

          <!-- 工作清單 -->
          <div v-if="loadingOverview" class="text-center py-10 text-hint-c">載入中...</div>
          <div v-else>
            <!-- 必要 -->
            <div class="mb-2 text-xs font-semibold text-hint-c uppercase tracking-wide">必要工作</div>
            <div class="space-y-2 mb-5">
              <div v-for="group in requiredGroups" :key="group.taskId"
                ...
                <!-- 項目標題列 -->
                <div class="flex items-center justify-between px-4 py-3 border-b border-light-c cursor-pointer"
                     @click="toggleGroup(group.taskId)">
                  <div class="flex items-center gap-2">
                    <span :class="groupStatusClass(group)" class="w-2.5 h-2.5 rounded-full flex-shrink-0"></span>
                    <span class="font-semibold text-base-c text-sm">{{ group.taskName }}</span>
                ...
                <!-- 步驟明細 -->
                <div v-if="expandedGroups.has(group.taskId)" class="divide-y divide-base">
                  <div v-for="r in group.records" :key="r.id"
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2">
                        <span class="text-xs text-hint-c w-5">{{ r.stepOrder }}</span>
                        <span class="text-sm text-base-c">{{ r.stepName }}</span>
                        <span v-if="r.assigneeOnLeave" class="text-xs bg-surface2 text-hint-c px-1.5 py-0.5 rounded">休假</span>
                      </div>
                    </div>
              ...
              <div v-if="requiredGroups.length === 0" class="text-center py-6 text-hint-c text-sm">今日無必要工作</div>
            </div>

            <!-- 選做 -->
            <div class="mb-2 text-xs font-semibold text-hint-c uppercase tracking-wide">選做工作</div>
            <div class="space-y-2">
              ...
              <div v-if="optionalGroups.length === 0" class="text-center py-6 text-hint-c text-sm">今日無選做工作</div>
            </div>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'tasks'">
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-2">
              <label class="text-sm text-hint-c flex items-center gap-1.5 cursor-pointer select-none">
          ...
          <div v-if="loadingTasks" class="text-center py-10 text-hint-c">載入中...</div>
          <div v-else class="space-y-3">
            <div v-for="task in tasks" :key="task.id"
                ...
                <div class="flex items-start gap-3 flex-1 min-w-0">
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                      <span class="font-semibold text-base-c">{{ task.name }}</span>
                      ...
                      <span class="text-xs bg-surface2 text-hint-c px-2 py-0.5 rounded-full">
                        {{ repeatTypeLabel(task) }}
                      </span>
                      <span v-if="task.estMinutes" class="text-xs text-hint-c">⏱ {{ task.estMinutes }} 分</span>
                      <span v-if="task.active === 0" class="text-xs bg-red-100 text-red-500 px-2 py-0.5 rounded-full">已停用</span>
                    </div>
                    <p v-if="task.description" class="text-xs text-hint-c mt-1">{{ task.description }}</p>
                  </div>
                </div>
                <div class="flex items-center gap-1 flex-shrink-0 ml-2">
                  <button @click="openTaskModal(task)"
                          class="p-1.5 text-hint-c hover:text-teal-600 hover:bg-teal-50 dark:hover:bg-teal-900/20 rounded-lg transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                    </svg>
                  </button>
                  <button @click="toggleTask(task)"
                          :class="task.active ? 'text-hint-c hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20' : 'text-hint-c hover:text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20'"
                  ...
                  <button @click="toggleSteps(task.id)"
                          class="p-1.5 text-hint-c hover:text-muted-c hover-surface2 rounded-lg transition-colors">
              ...
              <div v-if="expandedSteps.has(task.id)"
                   class="border-t border-light-c px-4 py-4">

                <!-- 工具列 -->
                <div class="flex items-center justify-between mb-3">
                  <span class="text-xs font-medium text-hint-c">流程步驟</span>
                  <div class="flex items-center gap-2">
                    <span class="text-xs text-hint-c">快速指派：</span>
                    <select @change="quickAssignAll(task, $event.target.value); $event.target.value = ''"
                            class="text-xs border border-light-c rounded-lg px-2 py-1 bg-surface text-base-c">
                ...
                <!-- 流程圖容器（可垂直滾動） -->
                <div class="overflow-y-auto overflow-x-auto max-h-[600px] rounded-xl bg-surface2/50 border border-light-c p-4">
                  <div class="flex flex-col items-center min-w-[320px]">
                    ...
                    <template v-for="(groupSteps, gIdx) in groupedSteps(task.steps)" :key="'g'+gIdx">
                        ...
                        <div class="flex items-stretch justify-center gap-0"
                          ...
                          <div v-for="(step, sIdx) in groupSteps" :key="step.id"
                            ...
                            <div class="flow-node group/node relative">
                              ...
                              <div class="font-medium text-sm text-base-c text-center leading-snug">{{ step.name }}</div>
                              <div class="flex items-center justify-center gap-1.5 mt-1.5 flex-wrap">
                                <span v-if="step.estMinutes" class="text-xs text-hint-c">⏱{{ step.estMinutes }}分</span>
                                <div class="flex -space-x-1">
                                  <div v-for="cid in step.assigneeIds" :key="cid" class="relative group/av">
                                    <img v-if="staffMap[cid]?.picture" :src="staffMap[cid].picture" class="w-5 h-5 rounded-full border-2 border-white dark:border-base object-cover"/>
                                    <div v-else class="w-5 h-5 rounded-full bg-teal-600 border-2 border-white dark:border-base flex items-center justify-center text-white text-xs font-bold">{{ (staffMap[cid]?.name||'?').charAt(0) }}</div>
                                    <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-2 py-0.5 text-white text-xs rounded-lg whitespace-nowrap opacity-0 group-hover/av:opacity-100 pointer-events-none z-20 shadow-lg">{{ staffMap[cid]?.name||cid }}</div>
                                  </div>
                                </div>
                              </div>
                      ...
                      <div v-else-if="groupSteps.length === 1" class="flex flex-col items-center">
                        ...
                        <div class="flow-node group/node relative">
                          ...
                          <div class="font-medium text-sm text-base-c text-center leading-snug">{{ groupSteps[0].name }}</div>
                          <div class="flex items-center justify-center gap-1.5 mt-1.5 flex-wrap">
                            <span v-if="groupSteps[0].estMinutes" class="text-xs text-hint-c">⏱{{ groupSteps[0].estMinutes }}分</span>
                            <div class="flex -space-x-1">
                    ...
                    <div v-if="!task.steps || task.steps.length === 0"
                         class="text-xs text-hint-c py-6">尚無步驟，點上方「＋」新增</div>
            ...
            <div v-if="tasks.length === 0" class="text-center py-10 text-hint-c">尚無工作項目</div>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'schedule'">
          <!-- 月份切換 -->
          <div class="flex items-center justify-between mb-4">
            <button @click="prevMonth" class="p-2 hover-surface2 rounded-xl transition-colors">
              <svg class="w-5 h-5 text-hint-c dark:text-hint-c" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
              </svg>
            </button>
            <span class="text-base font-semibold text-muted-c">{{ scheduleYearMonth }}</span>
            <button @click="nextMonth" class="p-2 hover-surface2 rounded-xl transition-colors">
              <svg class="w-5 h-5 text-hint-c dark:text-hint-c" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="overflow-x-auto">
              <table class="w-full text-sm">
                ...
                <tr class="border-b border-light-c">
                  <th class="text-left px-4 py-3 text-xs font-medium text-hint-c w-24 sticky left-0 bg-surface">員工</th>
                  <th v-for="day in scheduleDays" :key="day.date"
                      class="px-1 py-2 text-center min-w-8"
                      :class="day.isSun ? 'text-red-400' : 'text-hint-c'">
                ...
                <tr v-for="staff in staffList" :key="staff.id">
                  <td class="px-4 py-2 sticky left-0 bg-surface">
                    <div class="flex items-center gap-1.5">
                      <img v-if="staff.picture" :src="staff.picture" class="w-6 h-6 rounded-full" />
                      <span class="text-xs font-medium text-base-c truncate max-w-16">{{ staff.name }}</span>
                    </div>
                  </td>
 ...
 : day.isSun ? 'bg-surface2 /50 text-hint-c'
 : 'hover:bg-teal-50 dark:hover:bg-teal-900/20 text-base-c'"
          ...
          <p class="text-xs text-hint-c mt-2 text-center">點擊格子切換排休，紅色「休」表示排休，「日」為週日預設休息</p>
        </div>
        ...
        <div v-if="activeTab === 'history'">
          ...
          <div class="bg-surface rounded-2xl border border-light-c shadow-sm p-4 mb-4">
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div>
                <label class="text-xs text-hint-c mb-1 block">開始日期</label>
                <input type="date" v-model="histFilter.from"
                       class="w-full text-sm border border-light-c rounded-lg px-2 py-1.5 bg-surface text-base-c" />
              </div>
              <div>
                <label class="text-xs text-hint-c mb-1 block">結束日期</label>
                <input type="date" v-model="histFilter.to"
                       class="w-full text-sm border border-light-c rounded-lg px-2 py-1.5 bg-surface text-base-c" />
              </div>
              <div>
                <label class="text-xs text-hint-c mb-1 block">員工</label>
              ...
              <div>
                <label class="text-xs text-hint-c mb-1 block">工作項目</label>
          ...
          <div v-else class="bg-surface rounded-2xl border border-light-c shadow-sm overflow-hidden">
            <div class="overflow-x-auto">
              <table class="w-full text-sm">
                <thead class="border-b border-light-c">
                <tr>
                  <th class="text-left px-4 py-3 text-xs font-medium text-hint-c">日期</th>
                  <th class="text-left px-4 py-3 text-xs font-medium text-hint-c">工作</th>
                  <th class="text-left px-4 py-3 text-xs font-medium text-hint-c">步驟</th>
                  <th class="text-left px-4 py-3 text-xs font-medium text-hint-c">執行人</th>
                  <th class="text-left px-4 py-3 text-xs font-medium text-hint-c">狀態</th>
                  <th class="text-left px-4 py-3 text-xs font-medium text-hint-c">完成時間</th>
                </tr>
                </thead>
                <tbody class="divide-y divide-base">
                <tr v-for="r in historyRecords" :key="r.id" class="hover-surface2/30">
                  <td class="px-4 py-2.5 text-muted-c whitespace-nowrap">{{ r.date }}</td>
                  <td class="px-4 py-2.5">
                    <span class="text-base-c font-medium">{{ r.taskName }}</span>
                    <span :class="r.priority === 'required' ? 'text-teal-600' : 'text-amber-600'"
                          class="ml-1.5 text-xs">{{ r.priority === 'required' ? '必' : '選' }}</span>
                  </td>
                  <td class="px-4 py-2.5 text-hint-c">{{ r.stepName }}</td>
                  <td class="px-4 py-2.5 text-muted-c">
                  ...
                  <td class="px-4 py-2.5 text-hint-c text-xs whitespace-nowrap">{{ r.doneAt ? r.doneAt.slice(0, 16) : '—' }}</td>
                </tr>
                <tr v-if="historyRecords.length === 0">
                  <td colspan="6" class="text-center py-10 text-hint-c">無紀錄</td>
                </tr>
                </tbody>
              </table>
            </div>
      ...
      <div v-if="taskModal.show"
        ...
        <div class="bg-surface rounded-2xl w-full max-w-md shadow-2xl">
          <div class="flex items-center justify-between px-5 py-4 border-b border-light-c">
            <h3 class="font-bold text-base-c">{{ taskModal.id ? '編輯項目' : '新增項目' }}</h3>
            <button @click="taskModal.show = false" class="text-hint-c hover:text-muted-c">
          ...
          <div class="px-5 py-4 space-y-4">
            <div>
              <label class="text-xs font-medium text-hint-c mb-1 block">項目名稱 *</label>
              <input v-model="taskModal.name" type="text" placeholder="例：餐廳拖地"
                     class="w-full border border-light-c rounded-xl px-3 py-2 text-sm bg-surface text-base-c focus:outline-none focus:ring-2 focus:ring-teal-500" />
            </div>
            <div>
              <label class="text-xs font-medium text-hint-c mb-1 block">說明</label>
              <textarea v-model="taskModal.description" rows="2" placeholder="選填"
                        class="w-full border border-light-c rounded-xl px-3 py-2 text-sm bg-surface text-base-c focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none"></textarea>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-xs font-medium text-hint-c mb-1 block">重複週期</label>
              ...
              <div>
                <label class="text-xs font-medium text-hint-c mb-1 block">優先度</label>
            ...
            <!-- 每週指定天 -->
            <div v-if="taskModal.repeatType === 'weekly'" class="bg-surface2 /50 rounded-xl p-3">
              <label class="text-xs font-medium text-hint-c mb-2 block">每週哪幾天執行</label>
 ...
 : 'bg-surface text-muted-c border-base'"
            ...
            <div>
              <label class="text-xs font-medium text-hint-c mb-1 block">預估時間（分鐘）</label>
      ...
      <div v-if="insertModal.show"
        ...
        <div class="bg-surface rounded-2xl w-full max-w-xs shadow-2xl">
          <div class="flex items-center justify-between px-5 py-4 border-b border-light-c">
            <h3 class="font-bold text-base-c text-sm">新增步驟</h3>
            <button @click="insertModal.show = false" class="text-hint-c hover:text-muted-c">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
              </svg>
            </button>
          ...
          <div class="px-5 py-4 space-y-2">
            <p class="text-xs text-hint-c mb-3">
              要在「{{ insertModal.afterStepName || '開始' }}」之後插入什麼？
            </p>
            <!-- 單一步驟 -->
            <button @click="doInsert('single')"
                    class="w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 border-light-c hover:border-teal-500 hover:bg-teal-50 dark:hover:bg-teal-900/20 transition-all group">
              <div class="w-8 h-8 rounded-lg bg-surface2 group-hover:bg-teal-100 dark:group-hover:bg-teal-900/40 flex items-center justify-center transition-colors">
                <svg class="w-4 h-4 text-hint-c group-hover:text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                </svg>
              </div>
              <div class="text-left">
                <div class="text-sm font-medium text-base-c group-hover:text-teal-700 dark:group-hover:text-teal-300">單一步驟</div>
                <div class="text-xs text-hint-c">接在前一步驟完成後才能做</div>
              </div>
            </button>
            <!-- 平行步驟 -->
            <button @click="doInsert('parallel')"
                    class="w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 border-light-c hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all group">
              <div class="w-8 h-8 rounded-lg bg-surface2 group-hover:bg-indigo-100 dark:group-hover:bg-indigo-900/40 flex items-center justify-center transition-colors">
                <svg class="w-4 h-4 text-hint-c group-hover:text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h7"/>
                </svg>
              </div>
              <div class="text-left">
                <div class="text-sm font-medium text-base-c group-hover:text-indigo-700 dark:group-hover:text-indigo-300">與上方步驟同時進行</div>
                <div class="text-xs text-hint-c">加入同一個平行群組</div>
              </div>
            </button>
          </div>
        </div>
      ...
      <div v-if="stepModal.show"
        ...
        <div class="bg-surface rounded-2xl w-full max-w-md shadow-2xl">
          <div class="flex items-center justify-between px-5 py-4 border-b border-light-c">
            <div>
              <h3 class="font-bold text-base-c">{{ stepModal.id ? '編輯步驟' : '新增步驟' }}</h3>
              <p class="text-xs text-hint-c mt-0.5">{{ stepModal.taskName }}</p>
            </div>
            <button @click="stepModal.show = false" class="text-hint-c hover:text-muted-c">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
              </svg>
            </button>
          ...
          <div class="px-5 py-4 space-y-4">
            <div>
              <label class="text-xs font-medium text-hint-c mb-1 block">步驟名稱 *</label>
              <input v-model="stepModal.name" type="text" placeholder="例：用拖把清潔地板"
                     class="w-full border border-light-c rounded-xl px-3 py-2 text-sm bg-surface text-base-c focus:outline-none focus:ring-2 focus:ring-teal-500" />
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-xs font-medium text-hint-c mb-1 block">順序</label>
            ...
            <div>
              <label class="text-xs font-medium text-hint-c mb-1 block">備註提示</label>
              <input v-model="stepModal.note" type="text" placeholder="選填，給執行者的提示"
                     class="w-full border border-light-c rounded-xl px-3 py-2 text-sm bg-surface text-base-c focus:outline-none focus:ring-2 focus:ring-teal-500" />
            </div>
            <!-- 步驟說明圖片 -->
            <div>
              <label class="text-xs font-medium text-hint-c mb-1 block">說明圖片</label>
              <div v-if="stepModal.imageUrl" class="relative inline-block mb-2">
                <img :src="fixImgUrl(stepModal.imageUrl)" class="h-24 w-auto rounded-xl object-cover border border-light-c cursor-pointer"
              ...
              <label class="flex items-center gap-2 cursor-pointer">
                ...
                <span :class="uploadingImg ? 'opacity-50' : ''"
                      class="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border border-light-c text-muted-c hover-surface2 transition-colors">
            ...
            <!-- 平行群組 -->
            <div class="bg-surface2 /50 rounded-xl p-3">
              <label class="flex items-center gap-2 cursor-pointer select-none mb-1">
                <input type="checkbox" v-model="stepModal.isParallel"
                       class="rounded border-base text-teal-600 focus:ring-teal-500"
                       @change="onParallelChange" />
                <span class="text-sm font-medium text-base-c">與其他步驟同時進行（平行）</span>
              </label>
              <div v-if="stepModal.isParallel" class="mt-2">
                <label class="text-xs text-hint-c mb-1 block">平行群組編號（相同編號的步驟可同時進行）</label>
                <input v-model.number="stepModal.parallelGroup" type="number" min="1"
                       class="w-full border border-light-c rounded-xl px-3 py-2 text-sm bg-surface text-base-c focus:outline-none focus:ring-2 focus:ring-teal-500" />
                <p class="text-xs text-hint-c mt-1">例：烘培和香藥草都設群組 2，寄送設群組 3，則兩者完成後才能寄送</p>
              </div>
            </div>
            <div>
              <label class="text-xs font-medium text-hint-c mb-2 block">負責人</label>
              <div class="space-y-1.5 max-h-40 overflow-y-auto">
                <label v-for="s in staffList" :key="s.id"
                  ...
                  <span class="text-sm text-base-c group-hover:text-teal-600 transition-colors">{{ s.name }}</span>
                </label>
              </div>
            </div>
          </div>
...
function statusDot(s) {
  return { pending: 'bg-surface2', in_progress: 'bg-amber-400', done: 'bg-teal-500', skipped: 'bg-surface2' }[s] || 'bg-surface2'
}
function statusTextClass(s) {
  return { pending: 'text-hint-c', in_progress: 'text-amber-500', done: 'text-teal-600', skipped: 'text-hint-c' }[s] || 'text-hint-c'
}
function statusBadgeClass(s) {
  return { pending: 'bg-surface2 text-hint-c', in_progress: 'bg-amber-100 dark:bg-amber-900/30 text-amber-600', done: 'bg-teal-100 dark:bg-teal-900/30 text-teal-700', skipped: 'bg-surface2 text-hint-c' }[s] || ''
}
