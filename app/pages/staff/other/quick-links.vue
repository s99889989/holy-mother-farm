<script setup>
  definePageMeta({ layout: 'staff', requiredPermission: 'other.quick-links' })
  ...
  <div class="min-h-full bg-surface2 transition-colors">
    ...
    <div v-if="qrModal.open" class="fixed inset-0 bg-black/50 flex items-center justify-center z-30 px-4" @click.self="closeQrModal">
      <div class="bg-surface rounded-2xl shadow-lg w-full max-w-xs p-5 text-center">
        ...
        <div class="flex items-center justify-center gap-1 mb-3 bg-surface2 rounded-lg p-1">
          <button
            v-for="opt in qrModeOptions" :key="opt.value"
            class="px-3 py-1 rounded-md transition-colors disabled:opacity-50"
            :class="qrModal.mode === opt.value ? 'bg-green-700 text-white font-semibold' : 'text-muted-c hover-surface2'"
            style="font-size:12px"
            :disabled="qrModal.generating"
            @click="setQrMode(opt.value)"
          >
            {{ opt.label }}
          </button>
        </div>
