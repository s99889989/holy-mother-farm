<script setup>
definePageMeta({layout: 'staff', requiredPermission: 'permission.permission-keys'})
...
// ── 搜尋 / 分類篩選 / 檢視模式（樣式參考 customer-management.vue） ──
...
// ── Computed ──────────────────────────────────────────────────────
// 依搜尋文字篩選 key（比對 key 與中文標籤）
const filteredKeyDefs = computed(() => {
  const q = searchText.value.trim().toLowerCase()
  if (!q) return keyDefs.value
  return keyDefs.value.filter(kd =>
    kd.key.toLowerCase().includes(q) || kd.label.toLowerCase().includes(q)
  )
})

const keysBySection = computed(() => {
  const map = {}
  for (const kd of filteredKeyDefs.value) {
...
// 分類篩選：選定分類時只顯示該分類（不影響上面 keysBySection 的搜尋篩選）
const visibleSections = computed(() => filterSection.value
  ? sections.value.filter(s => s.section === filterSection.value)
  : sections.value)
  ...
  <div class="min-h-full bg-surface2 transition-colors duration-300">
    ...
    <header class="bg-surface border-b border-light-c px-4 py-3 sticky top-0 z-30">
        ...
        <div class="flex items-center gap-2">
          ...
          <div>
            ...
            <p class="text-xs text-hint-c mt-0.5 hidden sm:block">
              新增頁面時在這裡新增 key，各群組會自動補上（預設關閉）
            </p>
          </div>
        </div>
        <span class="text-xs text-hint-c">共 {{ keyDefs.length }} 個 key・{{ sections.length }} 個分類</span>
      </div>
      ...
      <div class="flex flex-wrap gap-2">
        <input
          v-model="searchText"
          placeholder="搜尋 key 名稱、標籤…"
          class="flex-1 min-w-40 px-3 py-1.5 text-sm rounded-lg border border-light-c bg-surface text-base-c outline-none focus:ring-2 focus:ring-violet-400"
        >
        <select
          v-model="filterSection"
          class="px-3 py-1.5 text-sm rounded-lg border border-light-c bg-surface text-base-c outline-none focus:ring-2 focus:ring-violet-400"
        >
          <option value="">
            全部分類
          </option>
    ...
    <div class="max-w-full px-3 sm:px-4 lg:px-6 py-4">
      ...
      <div
        v-if="!loading"
        class="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2 mb-3"
      >
        <div class="flex items-center gap-1 p-0.5 rounded-lg border border-light-c bg-surface">
          <button
            ...
            :class="viewMode === 'card' ? 'bg-violet-600 text-white' : 'text-muted-c hover-surface2'"
            @click="viewMode = 'card'"
          >
          ...
          <button
            type="button"
            class="flex items-center gap-1 px-2.5 py-1 text-xs rounded-md transition-colors"
            :class="viewMode === 'list' ? 'bg-violet-600 text-white' : 'text-muted-c hover-surface2'"
            @click="viewMode = 'list'"
          >
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div
        v-else
        class="max-w-7xl mx-auto bg-surface rounded-2xl border border-light-c shadow-sm overflow-hidden"
      >
        <div
        ...
        >
          <!-- 分區標頭（可收合） -->
          <div
            class="px-4 py-2 bg-surface2 flex items-center justify-between cursor-pointer hover-surface2 transition-colors"
            @click="toggleSectionCollapse(sec.section)"
          >
            <div class="flex items-center gap-2">
              <svg
                class="w-3.5 h-3.5 text-hint-c transition-transform flex-shrink-0"
                :class="collapsedSections[sec.section] ? '-rotate-90' : ''"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <button
              ...
              @click.stop="openCreateKey(sec.section)"
            >
              <span class="text-sm leading-none">+</span> 新增
            </button>
          </div>

          <div v-show="!collapsedSections[sec.section]">
            <!-- 空分類提示 -->
            <div
              v-if="!(keysBySection[sec.section] || []).length"
              class="px-4 py-6 text-center text-hint-c text-xs"
            >
            ...
            <div
              ...
              class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 p-3"
            >
              <div
                v-for="(kd, idx) in (keysBySection[sec.section] || [])"
                :key="kd.key"
              ...
              >
                ...
                <div class="flex flex-col flex-shrink-0">
                  <button
                    ...
                    :disabled="idx === 0 || !!searchText.trim()"
                    title="上移"
                  ...
                  <button
                    class="text-hint-c hover:text-violet-600 disabled:opacity-20 disabled:pointer-events-none leading-none px-0.5"
                    :disabled="idx === (keysBySection[sec.section] || []).length - 1 || !!searchText.trim()"
                    title="下移"
                    @click="moveKey(sec.section, idx, 1)"
                  >
                    <svg
                ...
                <div class="min-w-0 flex-1">
                  <p
                    class="text-sm font-medium text-base-c truncate"
                    :title="kd.label"
                  >
                    {{ kd.label }}
                  </p>
                  <p class="text-xs text-hint-c font-mono mt-0.5 break-all">
                    {{ kd.key }}
                  </p>
                </div>
            ...
            >
              <div
                v-for="(kd, idx) in (keysBySection[sec.section] || [])"
                :key="kd.key"
                class="flex items-center gap-2 px-3 py-2 hover-surface2 transition-colors"
              >
                <!-- 排序上下鍵 -->
                <div class="flex flex-col flex-shrink-0">
                  <button
                ...
                <div class="min-w-0 flex-1 flex items-center gap-3">
                  <span
                    ...
                    :title="kd.label"
                  >{{ kd.label }}</span>
                  <span class="text-xs text-hint-c font-mono truncate">{{ kd.key }}</span>
                </div>
                <div class="flex items-center gap-1.5 flex-shrink-0">
                  <button
                    class="px-2 py-1 text-xs border border-light-c text-muted-c rounded-lg hover-surface2 transition-colors"
        ...
        <div
          v-if="keyDefs.length === 0 && sections.length === 0"
          class="px-4 py-12 text-center text-hint-c text-sm"
        >
      ...
      <div class="bg-surface rounded-2xl shadow-xl w-full max-w-md p-6 my-auto">
        <div class="flex items-center justify-between mb-5">
          <h3 class="font-bold text-base-c">
            {{ keyModal.isCreate ? '新增 Permission Key' : '編輯 Key：' + keyModal.originalKey }}
          </h3>
          <button
            class="text-hint-c hover:text-muted-c p-1"
            @click="keyModal.open = false"
          >
            <svg
              class="w-5 h-5"
              fill="none"
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <div class="flex gap-2">
              ...
              <input
                v-if="keyModal.data.section === '__custom__'"
                v-model="keyModal.customSection"
                placeholder="輸入新分區名稱"
                class="flex-1 px-3 py-2 text-sm rounded-xl border border-light-c bg-surface text-base-c outline-none focus:ring-2 focus:ring-violet-400"
              >
            </div>
          ...
          <div>
            ...
            <input
              v-model="keyModal.data.key"
              placeholder="例如：staff.pos-analysis"
          ...
          <div>
            ...
            <input
              v-model="keyModal.data.label"
              placeholder="例如：POS 分析"
          ...
          <p
            v-if="keyModal.isCreate"
            class="text-xs text-hint-c bg-surface2 rounded-xl px-3 py-2"
          >
    ...
    >
      <div class="bg-surface rounded-2xl shadow-xl w-full max-w-md p-6 my-auto">
        ...
        <p class="text-xs text-hint-c mb-4">
          拖曳把手或用上下鍵調整分類順序；刪除分類時，內部 key 會自動移到「其他」。
        </p>
        ...
        <div class="space-y-2 mb-4 max-h-[50vh] overflow-y-auto pr-0.5">
          ...
          >
            ...
            <span
              v-else
              class="flex-1 min-w-0 text-sm text-base-c truncate"
            >{{ sec.section }}</span>
