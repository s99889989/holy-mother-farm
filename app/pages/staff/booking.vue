...
const recurBookingRules = computed(() => dayRecurRules.value.filter(r => r.type !== 'lunch'))
const recurLunchRules = computed(() => dayRecurRules.value.filter(r => r.type === 'lunch'))
const recurBookingTotal = computed(() => recurBookingRules.value.reduce((s, r) => s + (Number(r.guests) || 0), 0))
const recurLunchTotal = computed(() => recurLunchRules.value.reduce((s, r) => s + (Number(r.guests) || 0), 0))
    ...
    <div class="max-w-2xl mx-auto px-3 sm:px-4 py-4">
          ...
          <div v-if="calOpen" class="border-t border-stone-100 dark:border-stone-700 px-4 pb-4 pt-3">
            <div class="flex items-center justify-between mb-3">
              <button @click="prevMonth"
                ...
                <svg class="w-4 h-4 text-stone-500 dark:text-stone-300" fill="none" stroke="currentColor"
                     viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-7 mb-1">
              <div v-for="w in ['日','一','二','三','四','五','六']" :key="w"
                   class="text-center text-stone-400 dark:text-stone-500 font-medium py-1" style="font-size:12px">{{
                  w
                }}
              </div>
            </div>
            ...
            <div class="flex justify-end mt-2 pt-2 border-t border-stone-100 dark:border-stone-700">
              <button @click="pickDate(todayStr)" class="text-green-700 dark:text-green-400 font-medium"
                      style="font-size:13px">今天
              </button>
            </div>
          </div>
        </Transition>
      ...
      <div v-if="!loading && (bookings.length > 0 || lunchOrders.length > 0 || dayRecurRules.length > 0)"
        ...
        <div v-if="bookings.length > 0 || recurBookingRules.length > 0"
          ...
          <p class="font-semibold text-green-700 dark:text-green-400 mb-1" style="font-size:11px">🪑 訂位
            {{ bookings.length }} 筆・{{ bookingTotal + recurBookingTotal }} 人
          ...
          <div class="space-y-0.5" style="font-size:11px; color: inherit">
            <div v-if="bookingMeat  > 0" class="text-green-700 dark:text-green-300">🍖 葷 <span
              class="font-semibold">{{ bookingMeat }}</span></div>
            <div v-if="bookingFull  > 0" class="text-green-700 dark:text-green-300">🌿 全素 <span class="font-semibold">{{
                bookingFull
              }}</span></div>
            <div v-if="bookingEgg   > 0" class="text-green-700 dark:text-green-300">🥚 蛋奶素 <span
              class="font-semibold">{{ bookingEgg }}</span></div>
            <div v-if="bookingSpice > 0" class="text-green-700 dark:text-green-300">🧄 五辛素 <span
              class="font-semibold">{{ bookingSpice }}</span></div>
          </div>
        </div>
        <!-- 便當統計 -->
        <div v-if="lunchOrders.length > 0"
          ...
          <p class="font-semibold text-orange-700 dark:text-orange-400 mb-1" style="font-size:11px">🍱 便當 {{
              lunchTotal
            }} 個</p>
          <div class="space-y-0.5" style="font-size:11px">
            <div v-if="lunchMeat  > 0" class="text-orange-700 dark:text-orange-300">🍖 葷 <span
              class="font-semibold">{{ lunchMeat }}</span></div>
            <div v-if="lunchFull  > 0" class="text-orange-700 dark:text-orange-300">🌿 全素 <span class="font-semibold">{{
                lunchFull
              }}</span></div>
            <div v-if="lunchEgg   > 0" class="text-orange-700 dark:text-orange-300">🥚 蛋奶素 <span
              class="font-semibold">{{ lunchEgg }}</span></div>
            <div v-if="lunchSpice > 0" class="text-orange-700 dark:text-orange-300">🧄 五辛素 <span
              class="font-semibold">{{ lunchSpice }}</span></div>
          </div>
        </div>
      </div>
          ...
          <div class="space-y-2">
            <div v-for="b in bookings" :key="b.id"
              ...
              <div class="flex items-stretch">
                <!-- 時間色塊 -->
                <div
                  class="w-14 flex-shrink-0 bg-stone-50 dark:bg-zinc-700 flex flex-col items-center justify-center border-r border-stone-200 dark:border-stone-700 py-3">
                  <span class="text-stone-400 uppercase tracking-wide" style="font-size:9px">TIME</span>
                  <span class="font-black text-stone-700 dark:text-stone-100 leading-tight mt-0.5"
                        style="font-size:14px">{{ b.time }}</span>
                </div>
                <!-- 內容 -->
                <div class="flex-1 px-3 py-2.5 min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    <span class="font-bold text-stone-800 dark:text-stone-100" style="font-size:14px">{{
                        b.name
                      }}</span>
                  ...
                  <div class="flex flex-wrap gap-x-2 gap-y-1 mt-1">
                    <span v-if="b.meatQty   > 0"
                          class="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 px-1.5 py-0.5 rounded font-medium"
                          style="font-size:11px">🍖 葷 {{ b.meatQty }}</span>
                    <span v-if="b.fullVegQty > 0"
                          class="bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 px-1.5 py-0.5 rounded font-medium"
                          style="font-size:11px">🌿 全素 {{ b.fullVegQty }}</span>
                    <span v-if="b.eggVegQty  > 0"
                          class="bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 px-1.5 py-0.5 rounded font-medium"
                          style="font-size:11px">🥚 蛋奶素 {{ b.eggVegQty }}</span>
                    <span v-if="b.spiceVegQty> 0"
                          class="bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 px-1.5 py-0.5 rounded font-medium"
                          style="font-size:11px">🧄 五辛素 {{ b.spiceVegQty }}</span>
                  </div>
        ...
        <div v-if="lunchOrders.length > 0" class="mb-4">
          ...
          <div class="space-y-2">
            <div v-for="o in lunchOrders" :key="o.id"
              ...
              <div class="flex items-stretch">
                <div
                  class="w-14 flex-shrink-0 bg-orange-50 dark:bg-orange-900/20 flex flex-col items-center justify-center border-r border-orange-100 dark:border-orange-800/30 py-3">
                  <span class="text-orange-400 uppercase tracking-wide" style="font-size:9px">取餐</span>
                  <span class="font-black text-orange-700 dark:text-orange-300 leading-tight mt-0.5 text-center"
                        style="font-size:13px">{{ o.time }}</span>
                </div>
                <div class="flex-1 px-3 py-2.5 min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    <span class="font-bold text-stone-800 dark:text-stone-100" style="font-size:14px">{{
                  ...
                  <div class="flex flex-wrap gap-x-2 gap-y-1 mt-1">
                    <span v-if="o.meatQty   > 0"
                          class="bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400 px-1.5 py-0.5 rounded-full font-medium"
                          style="font-size:11px">🍖 葷 {{ o.meatQty }}</span>
                    <span v-if="o.fullVegQty > 0"
                          class="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 px-1.5 py-0.5 rounded-full font-medium"
                          style="font-size:11px">🌿 全素 {{ o.fullVegQty }}</span>
                    <span v-if="o.eggVegQty  > 0"
                          class="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 px-1.5 py-0.5 rounded-full font-medium"
                          style="font-size:11px">🥚 蛋奶素 {{ o.eggVegQty }}</span>
                    <span v-if="o.spiceVegQty> 0"
                          class="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 px-1.5 py-0.5 rounded-full font-medium"
                          style="font-size:11px">🧄 五辛素 {{ o.spiceVegQty }}</span>
                  </div>
                  <div class="flex flex-wrap gap-x-3 mt-1 text-stone-400 dark:text-stone-500" style="font-size:11px">
                    <span>📞 {{ o.phone }}</span>
        ...
        <div v-if="dayRecurRules.length > 0" class="mb-4">
          ...
          <div class="space-y-2">
            <div v-for="r in dayRecurRules" :key="r.id"
              ...
              <div class="flex items-stretch">
                <div
                  class="w-14 flex-shrink-0 bg-indigo-50 dark:bg-indigo-900/20 flex flex-col items-center justify-center border-r border-indigo-100 dark:border-indigo-800/30 py-3">
                  <span class="text-indigo-400 uppercase tracking-wide" style="font-size:9px">包月</span>
                  <span class="font-black text-indigo-600 dark:text-indigo-300 leading-tight mt-0.5 text-center"
                        style="font-size:13px">{{ r.time }}</span>
                </div>
                <div class="flex-1 px-3 py-2.5 min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    <span class="font-bold text-stone-800 dark:text-stone-100" style="font-size:14px">{{
...
<style scoped>
.cal-enter-active, .cal-leave-active {
  transition: opacity 0.2s, transform 0.2s;
}

.cal-enter-from, .cal-leave-to {
  opacity: 0;
