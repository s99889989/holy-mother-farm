...
const recurLunchTotal   = computed(() => recurLunchRules.value.reduce((s,r) =>
  s+(Number(r.meatQty)||0)+(Number(r.fullVegQty)||0)+(Number(r.eggVegQty)||0)+(Number(r.spiceVegQty)||0), 0))
  ...
  <div class="min-h-screen bg-stone-50 dark:bg-zinc-900 transition-colors">
    ...
    <header class="bg-white dark:bg-zinc-900 border-b border-stone-200 dark:border-stone-700 px-4 py-3 sticky top-14 z-20">
      <div class="max-w-2xl mx-auto flex items-center gap-2">
        <div class="w-8 h-8 rounded-lg bg-green-800 flex items-center justify-center text-white flex-shrink-0" style="font-size:14px">田</div>
        <div>
          <h1 class="font-bold text-stone-800 dark:text-stone-100 leading-none" style="font-size:15px">訂位記錄</h1>
    ...
    <div class="max-w-2xl mx-auto px-3 sm:px-4 py-4">
      ...
      <div class="bg-white dark:bg-zinc-800 rounded-2xl border border-stone-200 dark:border-stone-700 shadow-sm mb-4 overflow-hidden">
        <button class="w-full flex items-center justify-between px-4 py-3 hover:bg-stone-50 dark:hover:bg-zinc-700/50 transition-colors"
          ...
          <div class="flex items-center gap-2">
            ...
            <span v-if="hasMark(selectedDate)" class="w-2 h-2 rounded-full bg-green-500 inline-block"></span>
          </div>
        ...
        <Transition name="cal">
          <div v-if="calOpen" class="border-t border-stone-100 dark:border-stone-700 px-4 pb-4 pt-3">
            <div class="flex items-center justify-between mb-3">
              ...
              <span class="font-semibold text-stone-700 dark:text-stone-100" style="font-size:14px">{{ calYear }}年 {{ calMonth }}月</span>
            ...
            <div class="grid grid-cols-7 mb-1">
              <div v-for="w in ['日','一','二','三','四','五','六']" :key="w"
                   class="text-center text-stone-400 dark:text-stone-500 font-medium py-1" style="font-size:12px">{{ w }}</div>
            </div>
            <div class="grid grid-cols-7 gap-1">
              <div v-for="(day, idx) in calendarDays" :key="idx"
                ...
                <span>{{ day.label }}</span>
                <div v-if="day.date && hasMark(day.date)" class="absolute bottom-1 flex gap-0.5">
                  <span v-if="markedDates.includes(day.date)"
                        class="w-1.5 h-1.5 rounded-full"
                        :class="day.date === selectedDate ? 'bg-white' : 'bg-red-400'"></span>
                  <span v-if="lunchMarkedDates.includes(day.date)"
                        class="w-1.5 h-1.5 rounded-full"
                        :class="day.date === selectedDate ? 'bg-white' : 'bg-orange-400'"></span>
                </div>
              </div>
            </div>
            <div class="flex justify-end mt-2 pt-2 border-t border-stone-100 dark:border-stone-700">
              <button @click="pickDate(todayStr)" class="text-green-700 dark:text-green-400 font-medium" style="font-size:13px">今天</button>
            </div>
          </div>
        </Transition>
      </div>
      ...
      <!-- ── 當日統計列 ── -->
      <div v-if="!loading && (bookings.length > 0 || lunchOrders.length > 0 || dayRecurRules.length > 0)"
           class="grid grid-cols-2 gap-2 mb-4">
        <!-- 訂位統計 -->
        <div v-if="bookings.length > 0 || recurBookingRules.length > 0"
             class="bg-green-50 dark:bg-green-900/20 rounded-2xl border border-green-200 dark:border-green-800 px-3 py-2.5">
          <p class="font-semibold text-green-700 dark:text-green-400 mb-1" style="font-size:11px">🪑 訂位 {{ bookings.length }} 筆・{{ bookingTotal + recurBookingTotal }} 人
            <span v-if="recurBookingTotal > 0" class="opacity-70">（含包月 {{ recurBookingTotal }} 人）</span>
          </p>
          <div class="space-y-0.5" style="font-size:11px; color: inherit">
            <div v-if="bookingMeat  + recurBookingMeat  > 0" class="text-green-700 dark:text-green-300">🍖 葷 <span class="font-semibold">{{ bookingMeat + recurBookingMeat }}</span></div>
            <div v-if="bookingFull  + recurBookingFull  > 0" class="text-green-700 dark:text-green-300">🌿 全素 <span class="font-semibold">{{ bookingFull + recurBookingFull }}</span></div>
            <div v-if="bookingEgg   + recurBookingEgg   > 0" class="text-green-700 dark:text-green-300">🥚 蛋奶素 <span class="font-semibold">{{ bookingEgg + recurBookingEgg }}</span></div>
            <div v-if="bookingSpice + recurBookingSpice > 0" class="text-green-700 dark:text-green-300">🧄 五辛素 <span class="font-semibold">{{ bookingSpice + recurBookingSpice }}</span></div>
          </div>
        </div>
        <!-- 便當統計 -->
        <div v-if="lunchOrders.length > 0"
          ...
          <p class="font-semibold text-orange-700 dark:text-orange-400 mb-1" style="font-size:11px">🍱 便當 {{ lunchTotal }} 個</p>
          <div class="space-y-0.5" style="font-size:11px">
            <div v-if="lunchMeat  > 0" class="text-orange-700 dark:text-orange-300">🍖 葷 <span class="font-semibold">{{ lunchMeat }}</span></div>
            <div v-if="lunchFull  > 0" class="text-orange-700 dark:text-orange-300">🌿 全素 <span class="font-semibold">{{ lunchFull }}</span></div>
            <div v-if="lunchEgg   > 0" class="text-orange-700 dark:text-orange-300">🥚 蛋奶素 <span class="font-semibold">{{ lunchEgg }}</span></div>
            <div v-if="lunchSpice > 0" class="text-orange-700 dark:text-orange-300">🧄 五辛素 <span class="font-semibold">{{ lunchSpice }}</span></div>
          </div>
        </div>
      </div>
      ...
      <template v-else>
        ...
        <div v-if="bookings.length > 0" class="mb-4">
          <p class="font-semibold text-stone-400 dark:text-stone-500 uppercase tracking-widest flex items-center gap-1.5 mb-2" style="font-size:11px">
            ...
            <div v-for="b in bookings" :key="b.id"
              ...
              <div class="flex items-stretch">
                ...
                <div class="w-14 flex-shrink-0 bg-stone-50 dark:bg-zinc-700 flex flex-col items-center justify-center border-r border-stone-200 dark:border-stone-700 py-3">
                  <span class="text-stone-400 uppercase tracking-wide" style="font-size:9px">TIME</span>
                  <span class="font-black text-stone-700 dark:text-stone-100 leading-tight mt-0.5" style="font-size:14px">{{ b.time }}</span>
                </div>
                <!-- 內容 -->
                <div class="flex-1 px-3 py-2.5 min-w-0">
                  ...
                  <div class="flex flex-wrap gap-x-2 gap-y-1 mt-1">
                    ...
                    <span v-if="b.fullVegQty > 0" class="bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 px-1.5 py-0.5 rounded font-medium" style="font-size:11px">🌿 全素 {{ b.fullVegQty }}</span>
                    <span v-if="b.eggVegQty  > 0" class="bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 px-1.5 py-0.5 rounded font-medium" style="font-size:11px">🥚 蛋奶素 {{ b.eggVegQty }}</span>
                    <span v-if="b.spiceVegQty> 0" class="bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 px-1.5 py-0.5 rounded font-medium" style="font-size:11px">🧄 五辛素 {{ b.spiceVegQty }}</span>
                  </div>
                  <div class="flex flex-wrap gap-x-3 mt-1 text-stone-400 dark:text-stone-500" style="font-size:11px">
                    <span>📞 {{ b.phone }}</span>
        ...
        <!-- ── 便當列表 ── -->
        <div v-if="lunchOrders.length > 0" class="mb-4">
          <p class="font-semibold text-stone-400 dark:text-stone-500 uppercase tracking-widest flex items-center gap-1.5 mb-2" style="font-size:11px">
            <span class="w-2 h-2 rounded-full bg-orange-400"></span> 便當
          </p>
          <div class="space-y-2">
            <div v-for="o in lunchOrders" :key="o.id"
                 class="bg-white dark:bg-zinc-800 rounded-xl border border-stone-200 dark:border-stone-700 shadow-sm overflow-hidden">
              <div class="flex items-stretch">
                <div class="w-14 flex-shrink-0 bg-orange-50 dark:bg-orange-900/20 flex flex-col items-center justify-center border-r border-orange-100 dark:border-orange-800/30 py-3">
                  <span class="text-orange-400 uppercase tracking-wide" style="font-size:9px">取餐</span>
                  <span class="font-black text-orange-700 dark:text-orange-300 leading-tight mt-0.5 text-center" style="font-size:13px">{{ o.time }}</span>
                </div>
                <div class="flex-1 px-3 py-2.5 min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                  ...
                  <div class="flex flex-wrap gap-x-2 gap-y-1 mt-1">
                    ...
                    <span v-if="o.fullVegQty > 0" class="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 px-1.5 py-0.5 rounded-full font-medium" style="font-size:11px">🌿 全素 {{ o.fullVegQty }}</span>
                    <span v-if="o.eggVegQty  > 0" class="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 px-1.5 py-0.5 rounded-full font-medium" style="font-size:11px">🥚 蛋奶素 {{ o.eggVegQty }}</span>
                    <span v-if="o.spiceVegQty> 0" class="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 px-1.5 py-0.5 rounded-full font-medium" style="font-size:11px">🧄 五辛素 {{ o.spiceVegQty }}</span>
                  </div>
                  <div class="flex flex-wrap gap-x-3 mt-1 text-stone-400 dark:text-stone-500" style="font-size:11px">
                    <span>📞 {{ o.phone }}</span>
        ...
        <!-- ── 當月預定 ── -->
        <div v-if="dayRecurRules.length > 0" class="mb-4">
          <p class="font-semibold text-stone-400 dark:text-stone-500 uppercase tracking-widest flex items-center gap-1.5 mb-2" style="font-size:11px">
            <span class="w-2 h-2 rounded-full bg-indigo-400"></span> 當月預定
          </p>
          <div class="space-y-2">
            <div v-for="r in dayRecurRules" :key="r.id"
              ...
              <div class="flex items-stretch">
                <div class="w-14 flex-shrink-0 bg-indigo-50 dark:bg-indigo-900/20 flex flex-col items-center justify-center border-r border-indigo-100 dark:border-indigo-800/30 py-3">
                  <span class="text-indigo-400 uppercase tracking-wide" style="font-size:9px">包月</span>
                  <span class="font-black text-indigo-600 dark:text-indigo-300 leading-tight mt-0.5 text-center" style="font-size:13px">{{ r.time }}</span>
                </div>
                <div class="flex-1 px-3 py-2.5 min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    <span class="font-bold text-stone-800 dark:text-stone-100" style="font-size:14px">{{ r.name }}</span>
                    <span :class="r.type === 'lunch'
                  ...
                  <div class="flex flex-wrap gap-x-2 gap-y-1 mt-1" style="font-size:11px">
                    <span v-if="r.meatQty   > 0" class="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 px-1.5 py-0.5 rounded font-medium">🍖 葷 {{ r.meatQty }}</span>
                    <span v-if="r.fullVegQty > 0" class="bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 px-1.5 py-0.5 rounded font-medium">🌿 全素 {{ r.fullVegQty }}</span>
                    <span v-if="r.eggVegQty  > 0" class="bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 px-1.5 py-0.5 rounded font-medium">🥚 蛋奶素 {{ r.eggVegQty }}</span>
                    <span v-if="r.spiceVegQty> 0" class="bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 px-1.5 py-0.5 rounded font-medium">🧄 五辛素 {{ r.spiceVegQty }}</span>

                  </div>
                  <div class="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1 text-stone-400 dark:text-stone-500" style="font-size:11px">
                    <span v-if="r.weekdays && r.weekdays.length > 0" class="flex gap-0.5">
                      <span v-for="dow in [0,1,2,3,4,5,6]" :key="dow"
                            ...
                            class="w-4 h-4 rounded text-center leading-4 font-medium" style="font-size:10px">
                        {{ ['日','一','二','三','四','五','六'][dow] }}
                      </span>
                    </span>
                    <span v-else class="italic">每天</span>
        ...
        <!-- 空狀態 -->
        <div v-if="bookings.length === 0 && lunchOrders.length === 0 && dayRecurRules.length === 0"
