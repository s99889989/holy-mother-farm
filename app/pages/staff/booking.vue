...
<template>
  <div class="min-h-screen bg-stone-50 dark:bg-zinc-900 transition-colors">
    ...
    <div class="max-w-2xl mx-auto px-3 sm:px-4 py-4">
      ...
      <!-- ── 當日統計列 ── -->
      <div v-if="!loading && (bookings.length > 0 || lunchOrders.length > 0)"
        ...
        <div v-if="bookings.length > 0"
          ...
          <p class="font-semibold text-green-700 dark:text-green-400 mb-1" style="font-size:11px">🪑 訂位 {{ bookings.length }} 筆・{{ bookingTotal }} 人</p>
          <div class="space-y-0.5" style="font-size:11px; color: inherit">
            <div v-if="bookingMeat  > 0" class="text-green-700 dark:text-green-300">🍖 葷 <span class="font-semibold">{{ bookingMeat }}</span></div>
            <div v-if="bookingFull  > 0" class="text-green-700 dark:text-green-300">🌿 全素 <span class="font-semibold">{{ bookingFull }}</span></div>
            <div v-if="bookingEgg   > 0" class="text-green-700 dark:text-green-300">🥚 蛋奶素 <span class="font-semibold">{{ bookingEgg }}</span></div>
            <div v-if="bookingSpice > 0" class="text-green-700 dark:text-green-300">🧄 五辛素 <span class="font-semibold">{{ bookingSpice }}</span></div>
          </div>
        </div>
        <!-- 便當統計 -->
      ...
      <template v-else>
        ...
        <!-- 空狀態 -->
        <div v-if="bookings.length === 0 && lunchOrders.length === 0"
