<template>
  <div class="min-h-full bg-surface2 transition-colors duration-300">
    ...
    <header class="bg-surface border-b border-light-c px-4 py-3 sticky top-0 z-30">
      <div class="max-w-6xl mx-auto flex items-center justify-between mb-2">
        <div class="flex items-center gap-2">
          <div class="w-8 h-8 rounded-lg bg-red-700 flex items-center justify-center text-white text-sm font-bold flex-shrink-0">🧯</div>
          <div>
            <h1 class="font-bold text-base-c leading-none text-sm sm:text-base">滅火器巡檢管理</h1>
            <p class="text-xs text-hint-c mt-0.5 hidden sm:block">Fire Extinguisher Registry</p>
          </div>
        </div>
        <div class="flex items-center gap-2">
    ...
    <div class="max-w-6xl mx-auto px-3 sm:px-4 lg:px-6 py-4">
      ...
      <div class="flex flex-wrap items-center gap-2 mb-4">
        ...
        <div class="relative ml-auto w-full sm:w-64">
          ...
          <input
            ...
            class="w-full pl-9 pr-8 py-1.5 text-sm rounded-xl border border-light-c bg-surface2 text-base-c outline-none focus:ring-2 focus:ring-red-500"
          />
      ...
      <p v-if="loading" class="text-hint-c text-center py-10" style="font-size:14px">載入中...</p>

      <template v-else>
        <p v-if="filtered.length === 0" class="bg-surface rounded-2xl border border-light-c px-4 py-10 text-center text-hint-c shadow-sm" style="font-size:14px">
          找不到符合的滅火器
        </p>
        ...
        <div v-else class="hidden md:block bg-surface rounded-2xl border border-light-c shadow-sm overflow-hidden">
          <table class="w-full">
            <thead>
              <tr class="bg-surface2 border-b border-light-c text-hint-c" style="font-size:12px">
            ...
            <tbody class="divide-y divide-base">
              <tr v-for="(item, idx) in filtered" :key="item.id ?? item.code" class="hover-surface2 transition-colors">
                <td class="px-4 py-2.5 text-hint-c" style="font-size:13px">{{ idx + 1 }}</td>
                <td class="px-4 py-2.5 font-mono font-semibold text-base-c" style="font-size:13px">{{ item.code }}</td>
                <td class="px-4 py-2.5 text-muted-c" style="font-size:13px">{{ item.location }}</td>
                <td class="px-4 py-2.5 text-hint-c font-mono" style="font-size:13px">{{ item.batchNo }}</td>
                <td class="px-4 py-2.5 text-right whitespace-nowrap">
                  <button class="text-blue-700 dark:text-blue-400 hover:underline mr-3" style="font-size:13px" @click="openModal(item)">編輯</button>
                  <button class="text-red-700 dark:text-red-400 hover:underline" style="font-size:13px" @click="removeItem(item)">刪除</button>
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div v-if="filtered.length" class="md:hidden space-y-2">
          <div v-for="item in filtered" :key="item.id ?? item.code"
            ...
            <div class="flex items-start justify-between gap-2">
              <div class="min-w-0">
                <p class="font-mono font-semibold text-base-c" style="font-size:14px">{{ item.code }}</p>
                <p class="text-muted-c mt-0.5" style="font-size:13px">{{ item.location }}</p>
                <p class="text-hint-c font-mono mt-0.5" style="font-size:12px">批號 {{ item.batchNo }}</p>
              </div>
              <div class="flex flex-col items-end gap-1 flex-shrink-0">
                <button class="text-blue-700 dark:text-blue-400" style="font-size:13px" @click="openModal(item)">編輯</button>
                <button class="text-red-700 dark:text-red-400" style="font-size:13px" @click="removeItem(item)">刪除</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <transition name="fade">
      <div v-if="modal.show" class="fixed inset-0 z-40 flex items-center justify-center bg-black/40 px-4" @click.self="modal.show = false">
        <div class="bg-surface rounded-2xl border border-light-c shadow-lg w-full max-w-sm p-5">
          <h2 class="font-bold text-base-c mb-4" style="font-size:16px">{{ modal.isNew ? '新增滅火器' : '編輯滅火器' }}</h2>

          <div class="space-y-3">
            <div>
              <label class="text-hint-c block mb-1" style="font-size:12px">編號</label>
              <input v-model="modal.data.code" placeholder="例如 D區1F-A01"
                     class="w-full px-3 py-2 text-sm rounded-lg border border-light-c bg-surface2 text-base-c outline-none focus:ring-2 focus:ring-red-500 font-mono" />
            </div>
            <div>
              <label class="text-hint-c block mb-1" style="font-size:12px">位置</label>
              <input v-model="modal.data.location" placeholder="例如 服務中心"
                     class="w-full px-3 py-2 text-sm rounded-lg border border-light-c bg-surface2 text-base-c outline-none focus:ring-2 focus:ring-red-500" />
            </div>
            <div>
              <label class="text-hint-c block mb-1" style="font-size:12px">批號</label>
          ...
          <div class="flex gap-2 mt-5">
            <button class="flex-1 py-2 rounded-lg border border-light-c text-muted-c hover-surface2 transition-colors" style="font-size:14px" @click="modal.show = false">取消</button>
            <button class="flex-1 py-2 rounded-lg bg-red-700 text-white font-semibold hover:bg-red-800 transition-colors" style="font-size:14px" @click="saveItem">儲存</button>
          </div>
        </div>
      </div>
    </transition>
    ...
    <!-- ── Toast ── -->
    <transition name="fade">
      <div v-if="toast.show" class="fixed bottom-6 left-1/2 -translate-x-1/2 bg-surface border border-light-c shadow-lg rounded-xl px-4 py-2.5 text-base-c z-50 flex items-center gap-2" style="font-size:13px">
