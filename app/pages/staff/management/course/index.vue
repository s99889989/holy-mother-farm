  ...
  <div class="min-h-full" style="background: var(--surface2)">
    <div class="max-w-5xl mx-auto px-4 py-6">
      <div class="flex items-center justify-between mb-6">
        <div>
          <h1 class="text-xl font-bold" style="color: var(--text-base)">課程報名管理</h1>
          <p class="text-sm mt-1" style="color: var(--text-hint)">建立課程、自訂報名表單、管理報名名單</p>
        </div>
        <button
      ...
      <div v-else-if="store.courses.length === 0" class="text-center py-16" style="color: var(--text-hint)">
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          <NuxtLink :to="`/staff/management/course/${course.id}`" class="block">
            <div
          ...
          <div class="p-4 flex-1 flex flex-col gap-2">
            <NuxtLink :to="`/staff/management/course/${course.id}`" class="font-bold" style="color: var(--text-base)">
              {{ course.name || '（未命名課程）' }}
            </NuxtLink>
            <p class="text-xs" style="color: var(--text-hint)">{{ capacityLabel(course) }}</p>
            <p class="text-xs" style="color: var(--text-hint)">{{ deadlineLabel(course) }}</p>
    ...
    <!-- 新增課程 Modal -->
    <div v-if="modal.show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div class="w-full max-w-sm rounded-2xl p-5" style="background: var(--surface)">
        <h2 class="font-bold mb-3" style="color: var(--text-base)">新增課程</h2>
        <input
          v-model="form.name"
          type="text"
          placeholder="課程名稱"
    ...
    <div v-if="showDeleteConfirm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div class="w-full max-w-sm rounded-2xl p-5" style="background: var(--surface)">
        <h2 class="font-bold mb-2" style="color: var(--text-base)">刪除課程</h2>
