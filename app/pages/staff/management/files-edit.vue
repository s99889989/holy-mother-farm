          ...
          <div v-else-if="viewMode === 'grid'"
            ...
            <div v-for="img in filteredImages" :key="imgKey(img)"
              ...
              <div v-if="!selectMode" class="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all rounded-2xl items-center justify-center gap-1.5 opacity-0 group-hover:opacity-100 hidden sm:flex">
                ...
                <button @click.stop="downloadFile(img)" class="p-2 bg-white/90 hover:bg-indigo-50 rounded-xl text-indigo-600 transition-colors shadow-sm"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg></button>
              ...
              <tr v-for="img in filteredImages" :key="imgKey(img)"
                ...
                <td v-if="!selectMode" class="px-4 py-2">
                  <div class="flex items-center justify-center gap-1">
                    ...
                    <button @click.stop="downloadFile(img)" class="p-1.5 text-stone-400 hover:text-indigo-600 transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg></button>
      ...
      <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div class="flex items-center justify-between px-4 py-3 border-b border-stone-200 dark:border-stone-700">
          ...
          <div class="flex items-center gap-2 ml-4">
            ...
            <button @click="downloadFile(previewImg)" class="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-stone-100 dark:bg-zinc-700 text-stone-700 dark:text-stone-200 rounded-lg hover:bg-stone-200 transition-colors">
...
const selectedPath = ref(import.meta.client ? localStorage.getItem('imageLibrary_lastFolder') || '全部' : '全部')
const images = ref([])
...
const isSelected = (img) => {
  void selTick.value;
  return selected.has(imgKey(img))
}
...
onBeforeUnmount(() => {
  window.removeEventListener('mouseup', endLasso)
})
...
const selectFolder = async (path) => {
  ...
  localStorage.setItem('imageLibrary_lastFolder', path);
...
const confirmDeleteFolder = async (path) => {
  ...
  try {
    ...
    if (selectedPath.value === path || selectedPath.value.startsWith(path + '/')) {
      selectedPath.value = '全部';
      localStorage.setItem('imageLibrary_lastFolder', '全部')
    }
    await fetchImages();
    showToast('資料夾已刪除')
  } catch {
...
const doRenameFolder = async () => {
  ...
  try {
    ...
    if (selectedPath.value === oldPath || selectedPath.value.startsWith(oldPath + '/')) {
      ...
      localStorage.setItem('imageLibrary_lastFolder', selectedPath.value)
    }
    await fetchFolders();
    await fetchImages();
    renameFolderModal.show = false;
...
const doMove = async () => {
  ...
  if (moveModal.isBatch) {
    ...
    try {
      ...
      showToast(`已移動 ${targets.length} 個檔案`);
      exitSelectMode()
    } catch {
      showToast('移動失敗')
    }
