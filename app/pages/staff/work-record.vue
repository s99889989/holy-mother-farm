...
const CHECK_ITEMS = [
  { item: '16:30 未使用空間上鎖歸位', detail: '聖堂、文物室（更衣室）、簡報室、招待所（會議室）、A201、A202、A203、樂活教室、講師休息室、心靈教室、運動館B1大禮堂、運動管樂功能教室、競技館、手作教室、好食光餐廳、活動中心、咖啡小舖平台' },
...
function doneCount(record) {
  ...
  for (const s of (record.spaces || [])) {
    const c = spaceSubDone(s); done += c.done; total += c.total
  }
  ...
  <div class="min-h-screen bg-stone-50 dark:bg-zinc-900 transition-colors">
    ...
    <div class="max-w-2xl mx-auto px-3 sm:px-4 py-4">
      ...
      <div v-else class="space-y-3">
        <div v-for="rec in selectedRecords" :key="rec.id" class="bg-white dark:bg-zinc-800 rounded-2xl border border-stone-200 dark:border-stone-700 shadow-sm overflow-hidden">
          <!-- 卡片標題 -->
          ...
          <div v-if="expandedId === rec.id" class="border-t border-stone-100 dark:border-stone-700 px-4 pb-4 pt-3 space-y-4">
              ...
              <div class="space-y-1.5">
                ...
                >
                  ...
                  >
                    ...
                    <span class="flex-1 font-medium text-stone-700 dark:text-stone-200" style="font-size:13px">{{ s.name }}</span>
                    <span v-if="s.note" class="text-stone-400 flex-shrink-0 mr-1" style="font-size:11px">{{ s.note }}</span>
                    <span class="text-stone-400 flex-shrink-0" style="font-size:11px">{{ spaceSubDone(s).done }}/{{ spaceSubDone(s).total }}</span>
                    <svg class="w-3.5 h-3.5 text-stone-400 flex-shrink-0 transition-transform ml-1" :class="isSpaceExpanded(rec.id, si) ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                  </button>
                  <div v-if="isSpaceExpanded(rec.id, si)" class="border-t border-stone-100 dark:border-stone-700 px-3 py-2 space-y-1">
                    <div v-for="(sub, subI) in s.subItems" :key="subI" class="flex items-center gap-2">
                      <span :class="sub.done ? 'text-green-500' : 'text-stone-300 dark:text-stone-600'" style="font-size:12px">{{ sub.done ? '●' : '○' }}</span>
                      <span :class="['flex-1', sub.done ? 'text-stone-400 dark:text-stone-500 line-through' : 'text-stone-600 dark:text-stone-300']" style="font-size:12px">{{ sub.label }}</span>
                    </div>
                    <p v-if="s.remark" class="text-rose-600 dark:text-rose-400 pt-1.5 border-t border-stone-100 dark:border-stone-700 mt-1" style="font-size:12px">⚠ {{ s.remark }}</p>
                  </div>
            ...
            <div v-if="rec.checks?.length">
              ...
              <div class="space-y-1.5">
                ...
                >
                  <span :class="c.done ? 'text-green-600 dark:text-green-400' : 'text-stone-300 dark:text-stone-500'" class="font-bold flex-shrink-0 mt-px" style="font-size:14px">{{ c.done ? '✓' : '○' }}</span>
                  <div class="flex-1 min-w-0">
            ...
            <div class="flex gap-2 pt-1">
              <button class="flex-1 py-2 rounded-xl border border-teal-600 text-teal-700 dark:text-teal-400 font-medium hover:bg-teal-50 dark:hover:bg-teal-900/20 transition-colors" style="font-size:13px" @click="openEditForm(rec)">編輯</button>
              <button class="px-4 py-2 rounded-xl border border-red-300 text-red-500 font-medium hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" style="font-size:13px" @click="deleteRecord(rec.id)">刪除</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="showForm" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 px-0 sm:px-4">
        <div class="bg-white dark:bg-zinc-900 w-full sm:max-w-xl rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92dvh]">
          ...
          <div class="flex items-center justify-between px-4 py-3 border-b border-stone-100 dark:border-stone-700 flex-shrink-0">
            <h2 class="font-bold text-stone-800 dark:text-stone-100" style="font-size:15px">{{ editingId ? '編輯記錄' : '新增執行記錄' }}</h2>
          ...
          <div class="overflow-y-auto flex-1 px-4 py-4 space-y-4">
            ...
            <!-- ▸ 流程提示 -->
            <div class="rounded-xl bg-teal-50 dark:bg-teal-900/20 border border-teal-200 dark:border-teal-800 px-3 py-2.5 flex items-start gap-2">
              <span class="flex-shrink-0 text-teal-600 dark:text-teal-400 font-bold mt-px" style="font-size:13px">▸</span>
              <p class="text-teal-700 dark:text-teal-300 leading-relaxed" style="font-size:12px">
                各空間作業流程：<strong>1330 啟動暖機</strong> → 環境巡視與整理 → 設施清點與紀錄 → <strong>1630 關機上鎖</strong>
              </p>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input v-model="form.date" type="date" class="w-full px-3 py-2 rounded-xl border border-stone-200 dark:border-stone-600 bg-stone-50 dark:bg-zinc-800 text-stone-800 dark:text-stone-100 focus:outline-none focus:ring-2 focus:ring-teal-500" style="font-size:13px" />
              </div>
              <div>
                <label class="block text-stone-500 dark:text-stone-400 font-medium mb-1" style="font-size:12px">星期</label>
                <select v-model="form.weekday" class="w-full px-3 py-2 rounded-xl border border-stone-200 dark:border-stone-600 bg-stone-50 dark:bg-zinc-800 text-stone-800 dark:text-stone-100 focus:outline-none focus:ring-2 focus:ring-teal-500" style="font-size:13px">
            ...
            <div>
              ...
              <input v-model="form.staff" type="text" placeholder="姓名" class="w-full px-3 py-2 rounded-xl border border-stone-200 dark:border-stone-600 bg-stone-50 dark:bg-zinc-800 text-stone-800 dark:text-stone-100 focus:outline-none focus:ring-2 focus:ring-teal-500" style="font-size:13px" />
            </div>

            <div>
              <label class="block text-stone-500 dark:text-stone-400 font-medium mb-1" style="font-size:12px">整體備注</label>
              <textarea v-model="form.note" rows="2" placeholder="可選填整體補充說明…" class="w-full px-3 py-2 rounded-xl border border-stone-200 dark:border-stone-600 bg-stone-50 dark:bg-zinc-800 text-stone-800 dark:text-stone-100 focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none" style="font-size:13px" />
            </div>

            <!-- 各空間例行工作 -->
            <div>
              <div class="flex items-center justify-between mb-2">
                <p class="font-semibold text-stone-600 dark:text-stone-300 flex items-center gap-1.5" style="font-size:13px"><span>🏢</span> 一、各空間例行工作</p>
                <button class="text-teal-600 font-medium hover:underline" style="font-size:12px" @click="addSpace">＋ 新增空間</button>
              </div>
              <div class="space-y-2">
                <div v-for="(s, si) in form.spaces" :key="si" class="rounded-xl border border-stone-200 dark:border-stone-600 overflow-hidden">
                  <!-- 空間標題列 -->
                  <div class="flex items-center gap-2 px-3 py-2 bg-stone-50 dark:bg-zinc-800">
                    <button class="flex items-center gap-1.5 flex-1 min-w-0 text-left" @click="toggleFormSpace(si)">
                      <svg class="w-3.5 h-3.5 text-stone-400 flex-shrink-0 transition-transform" :class="expandedFormSpaceIdx.has(si) ? 'rotate-90' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                      </svg>
                      <input v-model="s.name" type="text" placeholder="空間名稱" class="flex-1 bg-transparent text-stone-800 dark:text-stone-100 focus:outline-none font-medium" style="font-size:13px" @click.stop />
                    </button>
                    <span class="text-stone-400 text-xs flex-shrink-0">{{ s.subItems.filter(x=>x.done).length }}/{{ s.subItems.length }}</span>
                    <button class="text-stone-300 hover:text-red-400 transition-colors flex-shrink-0 ml-1" @click="removeSpace(si)">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                  </div>

                  <!-- 子項清單 -->
                  <div v-if="expandedFormSpaceIdx.has(si)" class="border-t border-stone-100 dark:border-stone-700 px-3 py-2 space-y-1.5">
                    <div v-for="(sub, subI) in s.subItems" :key="subI" class="flex items-center gap-2">
                      <input v-model="sub.done" type="checkbox" class="w-4 h-4 rounded accent-teal-600 flex-shrink-0" @change="syncSpaceDone(s)" />
                      <input v-model="sub.label" type="text" placeholder="工作項目" class="flex-1 text-stone-700 dark:text-stone-300 bg-transparent focus:outline-none" style="font-size:12px" />
                      <button class="text-stone-200 hover:text-red-400 flex-shrink-0 transition-colors" @click="removeSubItem(si, subI)">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                      </button>
                    </div>
                    <button class="text-teal-500 hover:text-teal-700 dark:hover:text-teal-300 mt-1" style="font-size:12px" @click="addSubItem(si)">＋ 新增項目</button>
                    <div class="border-t border-stone-100 dark:border-stone-700 pt-2 mt-1">
                      <input v-model="s.remark" type="text" placeholder="⚠ 異常備注（可空白）" class="w-full bg-transparent text-rose-600 dark:text-rose-400 placeholder-stone-300 dark:placeholder-stone-600 focus:outline-none" style="font-size:12px" />
                    </div>
                  </div>
                  <div v-else-if="s.remark" class="border-t border-stone-100 dark:border-stone-700 px-3 py-1.5">
                    <p class="text-rose-600 dark:text-rose-400" style="font-size:12px">⚠ {{ s.remark }}</p>
            ...
            <!-- 16:30 關門檢查 -->
            <div>
              <p class="font-semibold text-stone-600 dark:text-stone-300 mb-2 flex items-center gap-1.5" style="font-size:13px"><span>🔒</span> 二、16:30 關門檢查</p>
              <div class="space-y-2">
                <div v-for="(c, ci) in form.checks" :key="ci" class="rounded-xl border border-stone-200 dark:border-stone-600 overflow-hidden">
                  <div class="flex items-center gap-2 px-3 py-2 bg-stone-50 dark:bg-zinc-800">
                  ...
                  <div v-if="c.detail" class="px-3 py-1.5 border-t border-stone-100 dark:border-stone-700">
                    <p class="text-stone-400 dark:text-stone-500 leading-relaxed" style="font-size:11px">{{ c.detail }}</p>
                  </div>
                  <div class="px-3 py-2 border-t border-stone-100 dark:border-stone-700">
                    <input v-model="c.remark" type="text" placeholder="⚠ 異常備注（可空白）" class="w-full bg-transparent text-rose-600 dark:text-rose-400 placeholder-stone-300 dark:placeholder-stone-600 focus:outline-none" style="font-size:12px" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="flex gap-2 px-4 py-3 border-t border-stone-100 dark:border-stone-700 flex-shrink-0">
            <button class="flex-1 py-2.5 rounded-xl border border-stone-200 dark:border-stone-600 text-stone-600 dark:text-stone-300 font-medium hover:bg-stone-50 dark:hover:bg-zinc-800 transition-colors" style="font-size:14px" @click="showForm = false">取消</button>
            <button class="flex-1 py-2.5 rounded-xl bg-teal-700 text-white font-semibold hover:bg-teal-800 transition-colors disabled:opacity-50" style="font-size:14px" :disabled="saving" @click="submitForm">
