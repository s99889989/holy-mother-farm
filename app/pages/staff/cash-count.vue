...
<template>
  <div class="min-h-screen bg-stone-50 dark:bg-zinc-900 transition-colors">
    ...
    <div class="max-w-2xl mx-auto px-3 sm:px-4 py-4">
      ...
      <div class="bg-white dark:bg-zinc-800 rounded-2xl border border-stone-200 dark:border-stone-700 shadow-sm mb-4 overflow-hidden">
        ...
        >
          <div class="flex items-center gap-2">
            ...
            >{{ selectedDate }}</span>
            <span
              v-if="recordDates.has(selectedDate)"
          ...
          >
            ...
            <div class="grid grid-cols-7 gap-1">
              ...
              >
                <span>{{ day.label }}</span>
                <span
                  v-if="day.date && recordDates.has(day.date)"
      ...
      <!-- ── 記錄列表 ── -->
      <h2
        class="font-semibold text-stone-700 dark:text-stone-100 mb-3"
        style="font-size:14px"
      >
        {{ selectedDate }} 點鈔記錄
      </h2>
        ...
        >
          ...
          >
            <div class="w-20 flex-shrink-0 bg-green-50 dark:bg-green-900/20 flex flex-col items-center justify-center border-r border-green-100 dark:border-green-800/30 py-3 px-1">
              <span
                class="text-green-600 dark:text-green-400 uppercase tracking-wide mb-0.5"
                style="font-size:10px"
              >總計</span>
              <span
                class="font-black text-green-700 dark:text-green-300 leading-tight text-center"
                style="font-size:15px"
              >
            ...
            <div class="flex-1 px-3 py-2.5 flex items-center justify-between gap-2">
              <div class="flex-1 min-w-0">
                ...
                >
                  <span
                    class="bg-stone-100 dark:bg-zinc-700 text-stone-500 dark:text-stone-300 rounded-full px-2 py-0.5"
                    style="font-size:11px"
                  >{{ r.note }}</span>
                </div>
                <p
                  v-if="r.createdAt"
                  class="text-stone-300 dark:text-stone-600 mt-1"
                  style="font-size:11px"
                >
                  建立 {{ r.createdAt }}
                </p>
              ...
              <svg
                class="w-4 h-4 text-stone-400 flex-shrink-0 transition-transform"
          ...
          <Transition name="expand">
            ...
            >
              ...
              >
                <thead>
                  <tr class="bg-stone-50 dark:bg-zinc-700/50">
                    <th class="text-left px-4 py-2 text-stone-400 font-normal">
                      面額
                    </th>
                    <th class="text-right px-4 py-2 text-stone-400 font-normal">
                      數量
                    </th>
                    <th class="text-right px-4 py-2 text-stone-400 font-normal">
                      小計
                ...
                <tbody>
                  <template
                    v-for="(group, gi) in denomGroups"
                    :key="gi"
                  >
                    <tr v-if="gi > 0">
                      <td
                        colspan="3"
                        class="border-t-2 border-dashed border-stone-100 dark:border-stone-700"
                      />
                    </tr>
                    <template
                      v-for="d in group"
                    ...
                    >
                      <tr
                        v-if="r.items && Number(r.items[d.value]) > 0"
                        class="border-t border-stone-50 dark:border-stone-700/50"
                      >
                        <td class="px-4 py-2">
                          <span
                            class="bg-stone-100 dark:bg-zinc-700 text-stone-600 dark:text-stone-300 rounded px-2 py-0.5"
                            style="font-size:12px;font-weight:500"
                          >{{ d.label }}</span>
                        </td>
                        <td class="px-4 py-2 text-right text-stone-600 dark:text-stone-300">
                          × {{ r.items[d.value] }}
                        </td>
              ...
              <div
                v-if="r.updatedAt && r.updatedAt !== r.createdAt"
                class="px-4 pb-3 text-stone-300 dark:text-stone-600"
                style="font-size:11px"
              >
                更新 {{ r.updatedAt }}
              </div>
    ...
    <Transition name="lb">
      ...
      >
        ...
        >
        <button
          class="absolute top-4 right-4 w-9 h-9 flex items-center justify-center rounded-full bg-white/20 text-white hover:bg-white/30 transition-colors"
          @click="closeLightbox"
        >
          <svg
            class="w-5 h-5"
