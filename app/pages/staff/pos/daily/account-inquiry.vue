<script setup lang="ts">
  definePageMeta({ layout: 'pos', requiredPermission: 'pos.account-inquiry' })
  ...
  // 帳單瀏覽為預設頁籤
  const view = ref<'invoice' | 'check' | 'report' | 'staffMeal' | 'daily' | 'coupon'>('check')
  const search = ref('')
  const page = ref(1)
  ...
  // ------------------------------------------------------------------
  // 消費券生日券明細：跟月報表的「消費券生日券」欄位用同一組資料來源，但這裡列出
  // 當月「每一筆」符合的交易明細（日期、品項名稱、數量、金額、帳單號碼、店別、
  // 付款方式、發票號碼），不像月報表只顯示每日加總金額，方便逐筆核對。
  // 做法：用「單品明細統計表」API（/holy/bk35sql/sales-analysis/item-detail）
  // 以關鍵字「消費券」「生日」查詢，再用帳單號碼比對「帳單瀏覽」(OCHECK) 取得
  // 店別(POSID)/付款方式，比對「發票資料」(INVOICE) 取得對應發票號碼。
  // 純顯示、不快取：每次切換月份都重新比對，確保資料是最新的。
  // ------------------------------------------------------------------
  interface CouponDetailEntry {
    id: string
    seq: number
    date: string // 'M/D'
    itemName: string
  ...
  // 依帳單的 CashAmt/PayAmt1~4 組出「付款方式」文字（跟包月員工頁籤的邏輯共用同一批欄位，
  // 但這裡單純列出帳單實際收到的付款金額，不做差額推算）
  function describeCheckPayMethod(checkRow: Record<string, any> | undefined): string {
    if (!checkRow) return '-'
  ...
  // 用「單品明細統計表」抓品項名稱包含「消費券」「生日」的每一筆交易，比對帳單瀏覽/發票資料，
  // 組出整月消費券生日券使用明細（依日期排序）
  async function fetchCouponDetailRows(year: number, month: number): Promise<CouponDetailEntry[]> {
    const mm = String(month).padStart(2, '0')
    const lastDay = new Date(year, month, 0).getDate()
    const from = `${year}-${mm}-01`
    const to = `${year}-${mm}-${String(lastDay).padStart(2, '0')}`

    const [checkRows, invoiceRows] = await Promise.all([
    ...
    const entries: CouponDetailEntry[] = []
    const seenTxKeys = new Set<string>()

    for (const keyword of COUPON_BIRTHDAY_KEYWORDS) {
      let res: any
      try {
        res = await $fetch<any>(`${apiBase.value}/holy/bk35sql/sales-analysis/item-detail`, {
          credentials: 'include',
          query: { dateFrom: from, dateTo: to, search: keyword }
        })
      } catch {
        continue // 這支 API 還沒上線或查詢失敗時，忽略該關鍵字，不影響其他關鍵字
      }
      if (res?.error) continue

      const groups = res?.groups ?? []
      for (const g of groups) {
        const groupItemName = String(g.itemName ?? '')
        for (const t of g.transactions ?? []) {
          if (!t?.date) continue
          const checkNo = t?.checkNo ? String(t.checkNo) : ''
          const amt = Number(t.amt) || 0
          const qty = Number(t.qty) || 0
          // 資料庫本身觀察到會有完全相同的重複列，用組合鍵去重複，只留第一筆
          const txKey = `${checkNo}|${groupItemName}|${amt}|${qty}|${t.date}`
          ...
          entries.push({
            id: `${checkNo || 'NA'}-${groupItemName}-${entries.length}`,
            seq: 0, // 排序後統一重新編號
            date: dateLabel,
            itemName: String(t?.itemName ?? groupItemName),
            qty,
            amt: Math.abs(amt),
            checkNo: checkNo || '-',
            posidLabel: checkRow?.POSID ? (POSID_LABELS[String(checkRow.POSID)] ?? String(checkRow.POSID)) : '-',
            payMethod: describeCheckPayMethod(checkRow),
            invoiceNo: checkNo ? (invoiceByCheckNo.get(checkNo) ?? 'X') : 'X'
          })
        }
      }
    }

    // 依日期排序（同日再依帳單號碼排序），排序後統一重新編號序號
    entries.sort((a, b) => {
      const [am, ad] = a.date.split('/').map(Number)
      const [bm, bd] = b.date.split('/').map(Number)
      if (isNaN(am) || isNaN(bm) || am !== bm) return (am || 0) - (bm || 0)
  ...
  function switchView(v: 'invoice' | 'check' | 'report' | 'staffMeal' | 'daily' | 'coupon') {
    view.value = v
    if (v === 'report') {
  ...
  // ---------------- 消費券生日券明細頁籤：狀態與月份切換 ----------------
  const couponYear = ref(new Date().getFullYear())
  const couponMonth = ref(new Date().getMonth() + 1)
  const couponEntries = ref<CouponDetailEntry[]>([])
  const couponRocYear = computed(() => couponYear.value - 1911)
  const couponLoading = ref(false)
  const couponError = ref('')
  const couponMonthTotal = computed(() => couponEntries.value.reduce((s, e) => s + e.amt, 0))

  const couponMonthInputValue = computed({
    get: () => `${couponYear.value}-${String(couponMonth.value).padStart(2, '0')}`,
    set: (v: string) => {
      if (!v) return
      const [y, m] = v.split('-').map(Number)
      if (y && m) {
  ...
  function couponPrevMonth() {
    if (couponMonth.value === 1) {
      couponMonth.value = 12
      couponYear.value -= 1
    } else {
      couponMonth.value -= 1
    }
  ...
  function couponNextMonth() {
    if (couponMonth.value === 12) {
      couponMonth.value = 1
      couponYear.value += 1
    } else {
      couponMonth.value += 1
  ...
  function couponThisMonth() {
    couponYear.value = new Date().getFullYear()
    couponMonth.value = new Date().getMonth() + 1
    loadCouponMonth()
  }

  // 載入該月明細：每次切換月份、進入頁籤都直接重新從系統比對，不快取，
  // 確保資料一定是根據目前帳單瀏覽/發票資料最新算出來的
  async function loadCouponMonth() {
    if (dbAttached.value === false) return

    couponLoading.value = true
    couponError.value = ''
    try {
      couponEntries.value = await fetchCouponDetailRows(couponYear.value, couponMonth.value)
    } catch (e: any) {
      couponError.value = e?.message ?? '自動比對失敗，請確認網路或稍後再試'
      couponEntries.value = []
    } finally {
  ...
  // 匯出這個月的消費券生日券明細（Excel）
  async function downloadCouponReport() {
    downloading.value = true
    try {
      const ExcelJS = (await import('exceljs')).default
      const wb = new ExcelJS.Workbook()
      const ws = wb.addWorksheet('消費券生日券明細')

      ws.mergeCells(1, 1, 1, 9)
      const titleCell = ws.getCell(1, 1)
      titleCell.value = `民國 ${couponRocYear.value} 年 ${couponMonth.value} 月　消費券生日券明細`
      titleCell.font = { bold: true, size: 14 }
      titleCell.alignment = { horizontal: 'center', vertical: 'middle' }
      ws.getRow(1).height = 24

      const headers = ['序號', '日期', '品項名稱', '數量', '金額', '帳單號碼', '店別', '付款方式', '發票號碼']
      headers.forEach((h, i) => {
        const cell = ws.getCell(2, i + 1)
        cell.value = h
        cell.font = { bold: true }
      ...
      couponEntries.value.forEach((e, idx) => {
        const r = 3 + idx
        const values = [e.seq, e.date, e.itemName, e.qty, e.amt, e.checkNo, e.posidLabel, e.payMethod, e.invoiceNo]
        values.forEach((v, ci) => {
          const cell = ws.getCell(r, ci + 1)
          cell.value = v as any
      ...
      const totalRow = 3 + couponEntries.value.length
      ws.mergeCells(totalRow, 1, totalRow, 4)
      const totalLabelCell = ws.getCell(totalRow, 1)
      totalLabelCell.value = '合計'
      totalLabelCell.font = { bold: true }
      totalLabelCell.alignment = { horizontal: 'center', vertical: 'middle' }
      const totalAmtCell = ws.getCell(totalRow, 5)
      totalAmtCell.value = couponMonthTotal.value
      totalAmtCell.font = { bold: true }
      totalAmtCell.alignment = { horizontal: 'center', vertical: 'middle' }
      for (let c = 1; c <= 9; c++) ws.getCell(totalRow, c).border = THIN_BORDER as any
      ...
      ws.pageSetup = {
        ...
        fitToHeight: 1,
        margins: { left: 0.4, right: 0.4, top: 0.4, bottom: 0.3, header: 0, footer: 0 },
        printArea: `A1:I${totalRow}`,
        horizontalCentered: true
      } as any

      const buffer = await wb.xlsx.writeBuffer()
      const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
      triggerBlobDownload(blob, `${couponRocYear.value}年${couponMonth.value}月份消費券生日券明細.xlsx`)
    } finally {
      downloading.value = false
    }
  }
  ...
  // ---- 部門別營收：改用 Bk35AccountInquiryController 專屬的 /daily/department-sales 端點，
  // 後端已經直接依 ORDERI.RefKPType join xxCODE（KPTYPE）組好「代碼-名稱」字串，
  // 前端不用再自己查 xxCODE、也不用再用 resolveDeptName 猜測要用 RefKPType 還是 typeNo。
  ...
  async function loadDailyReport() {
    ...
    try {
      ...
      const [deptRes, itemRes, checkRows, invoiceRows] = await Promise.all([
        $fetch<any>(`${apiBase.value}/holy/bk35sql/account-inquiry/daily/department-sales`, {
          credentials: 'include',
      ...
      if (deptRes?.error) throw new Error(deptRes.error)
      if (itemRes?.error) throw new Error(itemRes.error)

      // ---- 部門別營收 ----
      // 後端 /daily/department-sales 已經依 RefKPType join xxCODE 組好「代碼-名稱」，
      // 也已經排除負數金額列（折價/消費券不計入部門營收，在下面「折價項目」區塊單獨呈現），
      // 前端只要直接用就好，不用再自己判斷代碼/查表。
      const categoryTotal = Number(deptRes?.totalAmt) || 0
      const categories: DailyCategoryRow[] = ((deptRes?.rows ?? []) as any[])
    .map(r => ({
        typeName: String(r.name ?? ''),
        amt: Number(r.amt) || 0,
      ...
      // ---- 用去重複後的單品明細重新加總一次當天總營收，跟 department-sales 回傳的總額比對 ----
      // department-sales 是後端已經 SUM 好的數字，前端拿不到逐筆明細沒辦法反過去去重複；
  ...
  <div class="page-wrap">
    <div class="page-header">
      ...
      <div class="tab-switch">
        ...
        <button
          :class="['sw-tab', { active: view === 'coupon' }]"
    ...
    <template v-else>
      ...
      <!-- 消費券生日券明細：自動比對品項明細 + 帳單瀏覽 + 發票資料，列出當月逐筆使用紀錄 -->
      <div
        v-else-if="view === 'coupon'"
        class="report-panel"
      >
        <div class="month-switch">
          <button
            class="btn-ghost small"
            title="上個月"
            @click="couponPrevMonth"
          >
            ‹ 上月
          </button>
          <input
            v-model="couponMonthInputValue"
            type="month"
            class="month-input"
          >
          <button
            class="btn-ghost small"
            title="下個月"
            @click="couponNextMonth"
          >
            下月 ›
          </button>
          <button
            class="btn-ghost small"
            @click="couponThisMonth"
          >
            本月
          </button>
          <span
            v-if="!couponLoading"
            class="record-count-hint"
          >（共 {{ couponEntries.length }} 筆，合計 {{ couponMonthTotal.toLocaleString() }} 元）</span>
        </div>

        <p class="hint-banner">
          逐筆列出當月「單品明細統計表」中品項名稱含「消費券」「生日」的交易紀錄（跟月報表的
          「消費券生日券」欄位同一組資料來源，這裡改成逐筆明細方便核對）。「店別」「付款方式」
          比對帳單瀏覽(OCHECK) 取得，「發票號碼」比對發票資料取得，沒有對應資料時顯示「X」。
        ...
        <div class="download-bar">
          <button
            ...
            @click="loadCouponMonth"
          >
            {{ couponLoading ? '比對中…' : '重新整理' }}
          </button>
          <button
            class="btn-primary small"
            :disabled="downloading || couponEntries.length === 0"
            @click="downloadCouponReport"
          >
            {{ downloading ? '產生中…' : '下載本月明細（Excel）' }}
          </button>
        </div>
        ...
        >
          <table class="data-table report-table staff-meal-table">
            ...
            >
              ...
              <td>{{ entry.qty }}</td>
              <td>{{ entry.amt.toLocaleString() }}</td>
              <td>{{ entry.checkNo }}</td>
              <td>{{ entry.posidLabel }}</td>
              <td>{{ entry.payMethod }}</td>
              <td>{{ entry.invoiceNo }}</td>
            </tr>
            <tr v-if="couponEntries.length === 0">
              <td
                colspan="9"
                class="empty-cell"
              >
      ...
      >
        ...
        >
          ...
          折價項目已經去重複，但「部門別營收」是後端 department-sales 直接算好的總數，前端沒有逐筆資料可以去重複，
