...
<template>
  <div class="min-h-full bg-surface2 transition-colors duration-300">
    ...
    <header class="bg-surface border-b border-light-c px-4 py-3 sticky top-0 z-30">
      <div class="max-w-6xl mx-auto flex items-center justify-between mb-2">
        <div class="flex items-center gap-2">
          <div
            class="w-8 h-8 rounded-lg bg-red-700 flex items-center justify-center text-white text-base font-bold flex-shrink-0">
            🧯
          </div>
          <div>
            <h1 class="font-bold text-base-c leading-none text-base sm:text-lg">滅火器巡檢管理</h1>
            <p class="text-sm text-hint-c mt-0.5 hidden sm:block">Fire Extinguisher Registry &amp; Inspection
              Records</p>
          </div>
        </div>
        <div class="flex items-center gap-2">
    ...
    <div class="max-w-6xl mx-auto px-3 sm:px-4 lg:px-6 py-4">
      ...
      <template v-if="activeTab === 'registry'">
        <div class="flex flex-wrap items-center gap-2 mb-4">
          ...
          <label
            class="flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-surface2 text-muted-c whitespace-nowrap cursor-pointer"
            style="font-size:14px">
          ...
          <div class="relative ml-auto w-full sm:w-64">
            <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-hint-c" fill="none" stroke="currentColor"
                 viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <template v-else>
            ...
            <div class="bg-surface rounded-2xl border border-light-c shadow-sm p-4">
              ...
              <div class="flex flex-wrap gap-2">
                <span v-for="item in filtered" :key="item.id || item.code" class="badge-stat font-mono">{{ item.code }} · {{
                    item.location
                  }}</span>
              </div>
            </div>
            ...
            <div v-else class="hidden md:block bg-surface rounded-2xl border border-light-c shadow-sm overflow-hidden">
              <table class="w-full">
                ...
                <tr v-for="(item, idx) in filtered" :key="item.id || item.code" class="hover-surface2 transition-colors"
                  ...
                  <td class="px-4 py-2.5 font-mono font-semibold text-base-c" style="font-size:14px">{{
                      item.code
                    }}
                  </td>
                  ...
                  <td class="px-4 py-2.5 text-right whitespace-nowrap">
                    <template v-if="usingSeed">
                      <span class="text-hint-c" style="font-size:13px">請先匯入</span>
                    </template>
                    <template v-else-if="item.active === false">
                      <button class="text-green-700 dark:text-green-400 hover:underline" style="font-size:14px"
                              @click="restoreItem(item)">還原
                      </button>
                    </template>
                    <template v-else>
                      <button class="text-blue-700 dark:text-blue-400 hover:underline mr-3" style="font-size:14px"
                              @click="openModal(item)">編輯
                      </button>
                      <button class="text-red-700 dark:text-red-400 hover:underline" style="font-size:14px"
                              @click="removeItem(item)">停用
                      </button>
                    </template>
                  </td>
                </tr>
              ...
              <div v-for="item in filtered" :key="item.id || item.code"
                ...
                <div class="flex items-start justify-between gap-2">
                  ...
                  <div class="flex flex-col items-end gap-1 flex-shrink-0">
                    ...
                    <template v-else-if="item.active === false">
                      <button class="text-green-700 dark:text-green-400" style="font-size:14px"
                              @click="restoreItem(item)">還原
                      </button>
                    </template>
                    <template v-else>
                      <button class="text-blue-700 dark:text-blue-400" style="font-size:14px" @click="openModal(item)">
                        編輯
                      </button>
                      <button class="text-red-700 dark:text-red-400" style="font-size:14px" @click="removeItem(item)">
                        停用
                      </button>
                    </template>
                  </div>
          ...
          <div v-for="col in kanbanCols" :key="col.key" class="kanban-col">
            ...
            <div v-for="log in col.items" :key="log.id" class="issue-card" :class="'col-' + col.key">
              ...
              <div class="issue-actions">
                <button v-for="next in nextStatuses(col.key)" :key="next" @click="changeStatus(log, next)">{{
                    next
                  }}
                </button>
              </div>
        ...
        <div v-else class="bg-surface rounded-2xl border border-light-c shadow-sm overflow-hidden">
          <table class="w-full">
            ...
            <tr v-for="log in filteredLogs" :key="log.id" class="hover-surface2 transition-colors">
              ...
              <td class="px-4 py-2.5">
                <a v-if="log.photoUrl" :href="log.photoUrl" target="_blank"
                   class="text-blue-700 dark:text-blue-400 hover:underline" style="font-size:14px">查看</a>
            ...
            <tr v-if="!filteredLogs.length">
              <td colspan="6" class="px-4 py-8 text-center text-hint-c" style="font-size:14px">沒有符合的紀錄</td>
            </tr>
            </tbody>
          </table>
        </div>
    ...
    <transition name="fade">
      <div v-if="modal.show" class="fixed inset-0 z-40 flex items-center justify-center bg-black/40 px-4"
           @click.self="modal.show = false">
        <div class="bg-surface rounded-2xl border border-light-c shadow-lg w-full max-w-sm p-5">
          <h2 class="font-bold text-base-c mb-4" style="font-size:17px">{{
          ...
          <div class="flex gap-2 mt-5">
            <button class="flex-1 py-2 rounded-lg border border-light-c text-muted-c hover-surface2 transition-colors"
                    style="font-size:15px" @click="modal.show = false">取消
            </button>
            <button
              class="flex-1 py-2 rounded-lg bg-red-700 text-white font-semibold hover:bg-red-800 transition-colors"
              style="font-size:15px" @click="saveItem">儲存
            </button>
          </div>
        </div>
      </div>
...
.issue-card.col-doing {
  border-left-color: #B4740E;
}

.issue-card.col-replaced {
  border-left-color: #2F7D5C;
}

.issue-card.col-scrapped {
  border-left-color: #8B8E88;
}
...
.badge.muted {
  background: var(--surface2, #f0f1ec);
  color: var(--hint-c, #6b6f6a);
  border-color: var(--border-light-c, #dadedb);
}
</style>
