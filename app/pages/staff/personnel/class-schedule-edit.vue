  ...
  <div class="min-h-screen bg-stone-50 dark:bg-zinc-900 transition-colors duration-300">
    ...
    <header class="bg-white dark:bg-zinc-900 border-b border-stone-200 dark:border-stone-700 px-4 py-3 sticky top-0 z-30">
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="flex items-center gap-2 min-w-0 flex-1">
          ...
          <div class="min-w-0">
            <h1 class="font-bold text-stone-800 dark:text-stone-100 leading-none text-sm sm:text-base truncate">
              員工排假班表
            </h1>
      ...
      <div class="flex items-center gap-2 mt-2.5 pt-2.5 border-t border-stone-100 dark:border-stone-800">
        ...
        >
          圖例
          <svg
            :class="['w-3 h-3 transition-transform', legendOpen ? 'rotate-180' : '']"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      <div
        v-if="legendOpen"
        class="sm:hidden mt-2 pb-1 flex flex-wrap gap-x-3 gap-y-1.5"
      >
        <span
          v-for="l in LEGENDS"
          :key="l.code"
    ...
    <div class="max-w-[1400px] mx-auto px-3 sm:px-4 py-4 sm:py-6">
      ...
      <div v-if="view === 'table'">
        ...
        <div class="sm:hidden space-y-3">
          <div
            v-for="emp in currentDeptEmployees"
            :key="emp.id"
              ...
              <div class="flex min-w-max px-2 py-2 gap-1">
                ...
                >
                  ...
                  <div class="relative flex items-center justify-center w-7 h-7">
                    ...
                    <span
                      v-else
                      class="w-7 h-7 rounded-lg border border-dashed border-stone-200 dark:border-stone-700"
                    />
                    <!-- 附加角標 -->
                    <span
                      v-if="parseCell(emp.schedule[d]).extra"
            ...
            <div class="overflow-x-auto">
              <div class="flex min-w-max px-2 py-2 gap-1">
                ...
                >
                  ...
                  <span
                    v-else
                    class="w-7 h-7 flex items-center justify-center text-[10px] text-stone-300 dark:text-stone-600"
                  >—</span>
                </div>
              </div>
            </div>
          ...
          <!-- 空狀態 -->
          <div
            v-if="currentDeptEmployees.length === 0"
            ...
            <table class="w-full text-xs border-collapse min-w-[700px]">
              <thead>
                <tr class="bg-stone-50 dark:bg-zinc-800 border-b border-stone-200 dark:border-stone-700">
                  <th class="sticky left-0 z-10 bg-stone-50 dark:bg-zinc-800 px-3 py-2 text-left text-stone-500 dark:text-stone-400 font-medium whitespace-nowrap border-r border-stone-200 dark:border-stone-700 min-w-[100px]">
                    姓名
                  </th>
                  <th
                    v-for="d in days"
                  ...
                  <th class="px-2 py-2 text-center text-stone-400 dark:text-stone-500 font-medium whitespace-nowrap border-l border-stone-200 dark:border-stone-700 w-10">
                    應
                  </th>
                  <th class="px-2 py-2 text-center text-stone-400 dark:text-stone-500 font-medium whitespace-nowrap w-10">
                    實
                  </th>
                </tr>
                <tr class="border-b-2 border-stone-200 dark:border-stone-700">
                  ...
                  >
                    ...
                    <div
                      v-if="HOLIDAYS[d]"
                      class="text-[9px] text-pink-400 leading-tight"
                    >
                      {{ HOLIDAYS[d] }}
                    </div>
                  </th>
                  <th
                    class="border-l border-stone-200 dark:border-stone-700"
                    colspan="2"
                  />
                </tr>
              </thead>
              <tbody class="divide-y divide-stone-100 dark:divide-stone-800">
                ...
                >
                  <td class="sticky left-0 z-10 bg-white dark:bg-zinc-900 hover:bg-stone-50 dark:hover:bg-zinc-800/40 border-r border-stone-200 dark:border-stone-700 px-3 py-2 whitespace-nowrap">
                    <div class="font-semibold text-stone-800 dark:text-stone-100 text-xs">
                      {{ emp.name }}
                    </div>
              ...
              <tfoot class="border-t-2 border-stone-200 dark:border-stone-700">
                <tr class="bg-stone-50 dark:bg-zinc-800">
                  <td class="sticky left-0 z-10 bg-stone-50 dark:bg-zinc-800 border-r border-stone-200 dark:border-stone-700 px-3 py-2 whitespace-nowrap">
                    <div class="text-xs font-semibold text-stone-600 dark:text-stone-300">
                      每日人力
                    </div>
                    <div class="text-[10px] text-stone-400 dark:text-stone-500">
                      總人數 {{ totalEmployees }}
                  ...
                  >
                    ...
                    <span
                      v-else
                      class="text-[10px] text-stone-300 dark:text-stone-600"
                    >—</span>
                  </td>
                  <td
                    class="border-l border-stone-200 dark:border-stone-700"
      ...
      <div v-else-if="view === 'calendar'">
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-stone-200 dark:border-stone-700 shadow-sm overflow-hidden">
          ...
          <div class="grid grid-cols-7 divide-x divide-y divide-stone-100 dark:divide-stone-800">
            <div
              v-for="n in calLeadingBlanks"
              :key="'blank'+n"
              class="min-h-[70px] sm:min-h-[100px] bg-stone-50/50 dark:bg-zinc-800/30"
            />
            ...
            >
              ...
              <div class="flex items-start justify-between gap-0.5 mb-0.5 sm:mb-1">
                <div class="flex items-center gap-0.5 sm:gap-1">
                  ...
                  <span
                    v-if="HOLIDAYS[d]"
                    class="sm:hidden w-1.5 h-1.5 rounded-full bg-pink-400 flex-shrink-0 mt-0.5"
                  />
                </div>
                <span
                  v-if="LUNAR[d]"
                  class="text-[8px] sm:text-[9px] text-stone-300 dark:text-stone-600 leading-none mt-0.5"
                >{{ LUNAR[d] }}</span>
              </div>

              <div class="flex flex-col gap-0.5">
                <template
                  v-for="emp in calFilteredEmps"
                  :key="emp.id"
                >
                  <button
                    v-if="emp.schedule[d]"
    ...
    >
      <div class="bg-white dark:bg-zinc-900 rounded-t-3xl sm:rounded-2xl shadow-xl w-full sm:max-w-sm p-5">
        ...
        <div
          v-if="editCode"
          class="mb-4 px-3 py-2 bg-stone-50 dark:bg-zinc-800 rounded-xl flex items-center gap-2 text-xs text-stone-500 dark:text-stone-400"
        >
