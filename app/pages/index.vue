...
const openModal = (item) => {
  modal.isNew = !item
  if (item) {
    Object.assign(form, {...item, ingredients: [...item.ingredients]})
  } else {
    Object.assign(form, {id: '', date: selectedDate.value, name: '', ingredients: [], note: ''})
  }
  modal.show = true
}
...
const fetchAllItems = async () => {
  try {
    const [r, s] = await Promise.all([
      fetch(commonStore.data.main_url + '/holy/restaurant/items/get').then(r => r.json()).catch(() => []),
      fetch(commonStore.data.main_url + '/holy/shop/items/get').then(r => r.json()).catch(() => [])
    ])
    allItems.value = [...r, ...s]
  } catch (e) {
    console.error(e)
