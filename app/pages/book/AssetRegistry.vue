      ...
      <div class="flex items-center justify-between mb-2">
        ...
        <div class="flex items-center gap-2">
          ...
          >
            ...
            >
              <p class="text-xs font-semibold text-stone-500 dark:text-stone-400 mb-2 px-1">
                顯示欄位
              </p>
              ...
              <div class="mt-2 pt-2 border-t border-stone-100 dark:border-stone-700 flex gap-1.5">
                <button
                  class="flex-1 py-1 text-xs text-stone-500 dark:text-stone-400 hover:text-stone-700 transition-colors"
                  @click="resetCols"
                >
                  重設
                </button>
    ...
    <div class="max-w-full px-3 sm:px-4 py-4">
            ...
            <tbody class="divide-y divide-stone-100 dark:divide-stone-700">
              ...
              >
                ...
                <td class="px-2 py-2.5 cursor-grab active:cursor-grabbing">
                  <svg
                    class="w-4 h-4 text-stone-300 dark:text-stone-600 hover:text-stone-400 transition-colors mx-auto"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <circle
                ...
                <td class="px-3 py-2.5">
                  ...
                  <div
                    v-else
                    class="w-10 h-10 rounded-lg bg-stone-100 dark:bg-zinc-800 flex items-center justify-center text-stone-300"
                  >
                    <svg
                      class="w-4 h-4"
                      fill="none"
                ...
                >
                  <span
                    :class="asset.listed ? 'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400' : 'bg-stone-100 text-stone-400 dark:bg-zinc-800'"
                    class="px-2 py-0.5 rounded-full text-xs"
                  >{{ asset.listed ? '✓ 列入' : '不列入' }}</span>
                </td>
                <td
                  v-if="visibleCols.spec"
                  class="px-3 py-2.5 text-stone-600 dark:text-stone-300"
                >
                  {{ asset.spec }}
                </td>
                <td
                ...
                <td
                  v-if="visibleCols.location"
                  class="px-3 py-2.5 text-stone-600 dark:text-stone-300 max-w-36"
                >
                  <div
                    class="truncate"
                ...
                <td
                  v-if="visibleCols.quantity"
                  class="px-3 py-2.5 text-center text-stone-700 dark:text-stone-200 font-medium"
                >
                  {{ asset.quantity }}
                </td>
                <td
                  v-if="visibleCols.note"
                  class="px-3 py-2.5 text-stone-500 dark:text-stone-400 max-w-32"
                >
                  <div
                    class="truncate"
                    :title="asset.note"
                ...
                <td
                  v-if="visibleCols.price"
                  class="px-3 py-2.5 text-right text-stone-700 dark:text-stone-200 font-medium"
                >
                  {{ asset.price ? asset.price.toLocaleString() : '—' }}
                </td>
                <td
                  v-if="visibleCols.lifespan"
                  class="px-3 py-2.5 text-center text-stone-600 dark:text-stone-300"
                >
                  {{ asset.lifespan }}
                </td>
                <td
      ...
      >
        ...
        >
          ...
          <div class="p-4">
            <div class="mb-3">
              <p class="font-semibold text-stone-800 dark:text-stone-100 leading-tight truncate">
                {{ asset.name }}
              </p>
              <p
                v-if="visibleCols.spec || visibleCols.brand"
              ...
              <span
                v-if="visibleCols.org && asset.org"
                class="inline-block mt-1 px-2 py-0.5 rounded-full text-xs bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400"
              >{{ asset.org }}</span>
            </div>
            <div class="grid grid-cols-2 gap-x-3 gap-y-1 text-xs text-stone-600 dark:text-stone-300 mb-3">
              <div
                ...
                class="truncate"
              >
                <span class="text-stone-400">保管人：</span>{{ asset.keeper || '—' }}
              </div>
              <div
                v-if="visibleCols.unit"
                class="truncate"
              >
                <span class="text-stone-400">單位：</span>{{ asset.unit || '—' }}
              </div>
              <div
                v-if="visibleCols.location"
                class="col-span-2 truncate"
              >
                <span class="text-stone-400">位置：</span>{{ asset.location || '—' }}
              </div>
              <div
                v-if="visibleCols.usage"
                class="truncate"
              >
                <span class="text-stone-400">用途：</span>{{ asset.usage || '—' }}
              </div>
              <div
                v-if="visibleCols.issuer"
                class="truncate"
              >
                <span class="text-stone-400">撥發單位：</span>{{ asset.issuer || '—' }}
              </div>
              <div
                v-if="visibleCols.quantity"
                class="truncate"
              >
                <span class="text-stone-400">數量：</span>{{ asset.quantity }}
              </div>
              <div
                v-if="visibleCols.purchaseDate"
                class="truncate"
              >
                <span class="text-stone-400">購置：</span>{{ asset.purchaseDate || '—' }}
              </div>
              <div
                v-if="visibleCols.price"
                class="truncate"
              >
                <span class="text-stone-400">單價：</span>{{ asset.price ? asset.price.toLocaleString() : '—' }}
              </div>
              <div
                v-if="visibleCols.lifespan"
                class="truncate"
              >
                <span class="text-stone-400">年限：</span>{{ asset.lifespan ? asset.lifespan + ' 年' : '—' }}
              </div>
              <div
                v-if="visibleCols.planName"
                class="truncate"
              >
                <span class="text-stone-400">計畫：</span>{{ asset.planName || '—' }}
              </div>
              <div
                v-if="visibleCols.plateNo"
                class="truncate"
              >
                <span class="text-stone-400">車號：</span>{{ asset.plateNo || '—' }}
              </div>
            </div>
            <p
              v-if="visibleCols.note"
              class="text-xs text-stone-400 italic mb-3 truncate"
            >
              {{ asset.note || '—' }}
            </p>
        ...
        >
          <div class="flex items-start gap-3 mb-3">
            ...
            <div class="flex-1 min-w-0 flex items-start justify-between gap-2">
              <div>
                <p class="font-semibold text-stone-800 dark:text-stone-100 leading-tight">
                  {{ asset.name }}
                </p>
                <p
                  v-if="visibleCols.spec || visibleCols.brand"
              ...
              <span
                v-if="visibleCols.org"
                class="px-2 py-0.5 rounded-full text-xs bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400 flex-shrink-0"
              >{{ asset.org }}</span>
            </div>
          </div>
          <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-stone-600 dark:text-stone-300 mb-3">
            <div v-if="visibleCols.keeper">
              <span class="text-stone-400">保管人：</span>{{ asset.keeper || '—' }}
            </div>
            <div v-if="visibleCols.unit">
              <span class="text-stone-400">保管單位：</span>{{ asset.unit || '—' }}
            </div>
            <div v-if="visibleCols.location">
              <span class="text-stone-400">位置：</span>{{ asset.location || '—' }}
            </div>
            <div v-if="visibleCols.usage">
              <span class="text-stone-400">用途：</span>{{ asset.usage || '—' }}
            </div>
            <div v-if="visibleCols.issuer">
              <span class="text-stone-400">撥發單位：</span>{{ asset.issuer || '—' }}
            </div>
            <div v-if="visibleCols.quantity">
              <span class="text-stone-400">數量：</span>{{ asset.quantity }}
            </div>
            <div v-if="visibleCols.purchaseDate">
              <span class="text-stone-400">購置：</span>{{ asset.purchaseDate || '—' }}
            </div>
            <div v-if="visibleCols.price">
              <span class="text-stone-400">單價：</span>{{ asset.price ? asset.price.toLocaleString() : '—' }}
            </div>
            <div v-if="visibleCols.lifespan">
              <span class="text-stone-400">年限：</span>{{ asset.lifespan ? asset.lifespan + ' 年' : '—' }}
            </div>
            <div v-if="visibleCols.planName">
              <span class="text-stone-400">計畫：</span>{{ asset.planName || '—' }}
            </div>
            <div v-if="visibleCols.plateNo">
              <span class="text-stone-400">車號：</span>{{ asset.plateNo || '—' }}
            </div>
          </div>
          <p
            v-if="visibleCols.note"
            class="text-xs text-stone-400 italic mb-3"
          >
            {{ asset.note || '—' }}
          </p>
          <div class="flex gap-2">
        ...
        <div class="px-5 py-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div class="sm:col-span-2">
            ...
            <div class="flex gap-3 items-start">
              <div
                class="flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border-2 border-dashed border-stone-200 dark:border-stone-700 relative bg-stone-50 dark:bg-zinc-800 cursor-pointer"
                @click="triggerImageUpload"
              >
                <img
                  v-if="modal.data.image"
                  :src="imgUrl(modal.data.image)"
          ...
          <div class="sm:col-span-2">
            <label class="flex items-center gap-3 cursor-pointer select-none">
              <div class="relative">
                <input
                  v-model="modal.data.listed"
                  type="checkbox"
                  class="sr-only"
                >
                <div
                  :class="modal.data.listed ? 'bg-teal-600' : 'bg-stone-200 dark:bg-zinc-700'"
                  class="w-10 h-6 rounded-full transition-colors duration-200"
                />
                <div
                  :class="modal.data.listed ? 'translate-x-4' : 'translate-x-0'"
      ...
      <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl w-full max-w-sm p-5">
        ...
        <div class="space-y-1.5 max-h-64 overflow-y-auto">
          <div
            ...
            class="text-center py-6 text-stone-400 text-sm"
          >
            尚無選項
          </div>
          <div
        ...
        <button
          class="mt-4 w-full py-2 text-sm bg-stone-100 dark:bg-zinc-800 text-stone-600 dark:text-stone-300 rounded-xl hover:bg-stone-200 transition-colors"
          @click="showUnitManager = false"
        >
          完成
        </button>
    ...
    >
      <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl w-full max-w-sm p-6 text-center">
        <div v-if="importState.done">
          ...
          /></svg>
          <p class="font-bold text-stone-800 dark:text-stone-100 mb-1">
            匯入完成
          </p>
