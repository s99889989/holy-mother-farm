  ...
  <div class="min-h-screen bg-stone-50 dark:bg-zinc-900 transition-colors duration-300">
    ...
    <header class="bg-white dark:bg-zinc-900 border-b border-stone-200 dark:border-stone-700 px-4 py-3 sticky top-0 z-30">
      <div class="flex items-center justify-between mb-2">
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="hidden md:flex bg-stone-100 dark:bg-zinc-800 rounded-lg p-0.5 gap-0.5">
            <button @click="desktopView = 'table'" :title="'表格'"
            ...
            <button @click="desktopView = 'card'" :title="'卡片'"
          ...
          <div class="relative" ref="colSettingsRef">
            ...
            <div v-if="showColSettings"
              ...
              <div class="mt-2 pt-2 border-t border-stone-100 dark:border-stone-700 flex gap-1.5">
                <button @click="resetCols" class="flex-1 py-1 text-xs text-stone-500 dark:text-stone-400 hover:text-stone-700 transition-colors">重設</button>
                <button @click="showColSettings = false" class="flex-1 py-1 text-xs bg-teal-700 text-white rounded-lg hover:bg-teal-800 transition-colors">完成</button>
              </div>
            </div>
          </div>
          <button @click="exportExcel" title="匯出 Excel"
          ...
          <button @click="triggerImport" title="匯入 Excel"
          ...
          <button @click="openModal(null)" title="新增財產"
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <select v-model="filterOrg" class="px-3 py-1.5 text-sm rounded-lg border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-700 dark:text-stone-200 outline-none focus:ring-2 focus:ring-teal-400">
          <option value="">全部機構</option>
          <option v-for="o in orgOptions" :key="o" :value="o">{{ o }}</option>
        </select>
        <select v-model="filterUnit" class="px-3 py-1.5 text-sm rounded-lg border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-700 dark:text-stone-200 outline-none focus:ring-2 focus:ring-teal-400">
          <option value="">全部單位</option>
          <option v-for="u in managedUnitOptions" :key="u" :value="u">{{ u }}</option>
        </select>
        <select v-model="filterLocation" class="px-3 py-1.5 text-sm rounded-lg border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-700 dark:text-stone-200 outline-none focus:ring-2 focus:ring-teal-400">
          <option value="">全部位置</option>
          <option v-for="l in managedLocationOptions" :key="l" :value="l">{{ l }}</option>
        </select>
        <select v-model="filterListed" class="px-3 py-1.5 text-sm rounded-lg border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-700 dark:text-stone-200 outline-none focus:ring-2 focus:ring-teal-400">
      ...
      <div v-else-if="desktopView === 'table'" class="hidden md:block bg-white dark:bg-zinc-900 rounded-2xl border border-stone-200 dark:border-stone-700 shadow-sm overflow-hidden">
        <div class="px-4 py-2 border-b border-stone-100 dark:border-stone-800 flex items-center gap-2 text-xs text-stone-400 dark:text-stone-500 select-none">
          ...
          <span v-if="reordering" class="ml-auto flex items-center gap-1 text-teal-600 dark:text-teal-400">
            <span class="w-3 h-3 border-2 border-teal-500 border-t-transparent rounded-full animate-spin inline-block"></span>儲存中…
          </span>
        </div>
            ...
            <tr v-for="(asset, index) in filtered" :key="asset.id"
                ...
                :class="[
                  ...
                  dragOverIndex === index && draggingIndex !== index
                    ? 'bg-teal-50 dark:bg-teal-900/20 outline outline-2 outline-teal-400 outline-offset-[-2px]'
    ...
    <div v-if="modal.show" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center z-50">
      <div class="bg-white dark:bg-zinc-900 rounded-t-3xl sm:rounded-2xl shadow-xl w-full sm:max-w-2xl max-h-[92vh] overflow-y-auto">
        ...
        <div class="px-5 py-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div class="sm:col-span-2">
            ...
            <div class="flex gap-3 items-start">
              ...
              <div class="flex-1 space-y-1.5">
                <button v-if="modal.data.image" @click="deleteImage" class="text-xs text-red-400 hover:text-red-500 transition-colors">移除圖片</button>
          ...
          <div>
            <label class="text-xs font-medium text-stone-600 dark:text-stone-300 mb-1 flex items-center justify-between">
              ...
              <button type="button" @click="showUnitManager = true"
                ...
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>管理選項
              </button>
            </label>
          ...
          <div class="sm:col-span-2">
            <label class="flex items-center gap-3 cursor-pointer select-none">
              <div class="relative">
                ...
                <div :class="modal.data.listed ? 'bg-teal-600' : 'bg-stone-200 dark:bg-zinc-700'" class="w-10 h-6 rounded-full transition-colors duration-200"></div>
                <div :class="modal.data.listed ? 'translate-x-4' : 'translate-x-0'" class="absolute top-1 left-1 w-4 h-4 bg-white rounded-full shadow transition-transform duration-200"></div>
              </div>
              <div>
          ...
          <template v-if="!modal.simple">
            <div>
              ...
              <input v-model="modal.data.brand" placeholder="廠牌" class="w-full px-3 py-2 text-sm rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-800 dark:text-stone-100 outline-none focus:ring-2 focus:ring-teal-400" />
            </div>
            <div>
              <label class="text-xs font-medium text-stone-600 dark:text-stone-300 block mb-1">保管人員</label>
              <input v-model="modal.data.keeper" placeholder="輸入姓名" class="w-full px-3 py-2 text-sm rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-800 dark:text-stone-100 outline-none focus:ring-2 focus:ring-teal-400" />
            </div>
            <div>
              <label class="text-xs font-medium text-stone-600 dark:text-stone-300 block mb-1">機構</label>
              <input v-model="modal.data.org" placeholder="例：法人" list="org-list" class="w-full px-3 py-2 text-sm rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-800 dark:text-stone-100 outline-none focus:ring-2 focus:ring-teal-400" />
              <datalist id="org-list"><option v-for="o in orgOptions" :key="o" :value="o" /></datalist>
            </div>
            <div>
              <label class="text-xs font-medium text-stone-600 dark:text-stone-300 block mb-1">用途</label>
              <input v-model="modal.data.usage" placeholder="例：辦公" class="w-full px-3 py-2 text-sm rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-800 dark:text-stone-100 outline-none focus:ring-2 focus:ring-teal-400" />
            </div>
            <div>
              <label class="text-xs font-medium text-stone-600 dark:text-stone-300 block mb-1">撥發單位</label>
              <input v-model="modal.data.issuer" placeholder="請參閱查詢" class="w-full px-3 py-2 text-sm rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-800 dark:text-stone-100 outline-none focus:ring-2 focus:ring-teal-400" />
            </div>
            <div>
              <label class="text-xs font-medium text-stone-600 dark:text-stone-300 block mb-1">撥發數量</label>
              <input v-model.number="modal.data.quantity" type="number" min="1" placeholder="1" class="w-full px-3 py-2 text-sm rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-800 dark:text-stone-100 outline-none focus:ring-2 focus:ring-teal-400" />
            </div>
            <div>
              <label class="text-xs font-medium text-stone-600 dark:text-stone-300 block mb-1">購置日期</label>
              <input v-model="modal.data.purchaseDate" type="date" class="w-full px-3 py-2 text-sm rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-800 dark:text-stone-100 outline-none focus:ring-2 focus:ring-teal-400" />
            </div>
            <div>
              <label class="text-xs font-medium text-stone-600 dark:text-stone-300 block mb-1">單價（元）</label>
              <input v-model.number="modal.data.price" type="number" min="0" placeholder="0" class="w-full px-3 py-2 text-sm rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-800 dark:text-stone-100 outline-none focus:ring-2 focus:ring-teal-400" />
            </div>
            <div>
              <label class="text-xs font-medium text-stone-600 dark:text-stone-300 block mb-1">使用年限（年）</label>
              <input v-model.number="modal.data.lifespan" type="number" min="0" placeholder="0" class="w-full px-3 py-2 text-sm rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-800 dark:text-stone-100 outline-none focus:ring-2 focus:ring-teal-400" />
            </div>
            <div>
              <label class="text-xs font-medium text-stone-600 dark:text-stone-300 block mb-1">計畫名稱（編號）</label>
              <input v-model="modal.data.planName" placeholder="請參閱查詢，填編號" class="w-full px-3 py-2 text-sm rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-800 dark:text-stone-100 outline-none focus:ring-2 focus:ring-teal-400" />
            </div>
            <div>
              <label class="text-xs font-medium text-stone-600 dark:text-stone-300 block mb-1">車號</label>
              <input v-model="modal.data.plateNo" placeholder="車號" class="w-full px-3 py-2 text-sm rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-800 dark:text-stone-100 outline-none focus:ring-2 focus:ring-teal-400" />
            </div>
            <div class="sm:col-span-2">
        ...
        <div class="px-5 py-4 border-t border-stone-100 dark:border-stone-700 flex gap-2 justify-end">
          <button @click="modal.show = false" class="px-4 py-2 text-sm bg-stone-100 dark:bg-zinc-800 text-stone-600 dark:text-stone-300 rounded-xl hover:bg-stone-200 transition-colors">取消</button>
          <button @click="saveAsset" class="px-4 py-2 text-sm bg-teal-700 text-white rounded-xl hover:bg-teal-800 transition-colors">
            {{ modal.isNew ? '新增' : '儲存' }}
          </button>
        </div>
      </div>
    ...
    <div v-if="showUnitManager" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[70]">
      <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl w-full max-w-sm p-5">
        ...
        <div class="flex gap-2 mb-3">
          <input v-model="newUnitInput" placeholder="新增保管單位…" @keydown.enter.prevent="addUnitOption"
        ...
        <button @click="showUnitManager = false" class="mt-4 w-full py-2 text-sm bg-stone-100 dark:bg-zinc-800 text-stone-600 dark:text-stone-300 rounded-xl hover:bg-stone-200 transition-colors">完成</button>
      </div>
    </div>
    ...
    <div v-if="showLocationManager" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[70]">
      <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl w-full max-w-sm p-5">
        ...
        <div class="flex gap-2 mb-3">
          <input v-model="newLocationInput" placeholder="新增放置位置…" @keydown.enter.prevent="addLocationOption"
                 class="flex-1 px-3 py-2 text-sm rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-zinc-800 text-stone-800 dark:text-stone-100 outline-none focus:ring-2 focus:ring-teal-400" />
        ...
        <button @click="showLocationManager = false" class="mt-4 w-full py-2 text-sm bg-stone-100 dark:bg-zinc-800 text-stone-600 dark:text-stone-300 rounded-xl hover:bg-stone-200 transition-colors">完成</button>
      </div>
    </div>
    ...
    <div v-if="importState.show" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[80]">
      <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl w-full max-w-sm p-6 text-center">
        <div v-if="importState.done">
          <svg class="w-12 h-12 text-teal-500 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          ...
          <button @click="importState.show = false" class="px-6 py-2 text-sm bg-teal-700 text-white rounded-xl hover:bg-teal-800 transition-colors">關閉</button>
        </div>
        <div v-else>
          ...
          <div class="w-full bg-stone-100 dark:bg-zinc-800 rounded-full h-2">
            <div class="bg-teal-600 h-2 rounded-full transition-all duration-300"
                 :style="{ width: importState.total ? (importState.current / importState.total * 100) + '%' : '0%' }"></div>
          </div>
        </div>
      </div>
    </div>
...
const fetchAssets = async () => {
  loading.value = true
  try { assets.value = await (await fetch(`${API_BASE.value}/list`)).json() }
  catch (e) { console.error(e) }
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    for (let i=hi+1; i<raw.length; i++) {
      ...
      for (const [xlsCol,apiField] of Object.entries(COL_MAP)) {
        ...
        if (['quantity','price','lifespan'].includes(apiField)) { const n=parseFloat(val); asset[apiField]=isNaN(n)?null:n }
