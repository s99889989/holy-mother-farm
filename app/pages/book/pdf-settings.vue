<template>
  <div class="p-4 max-w-screen-xl mx-auto text-sm text-stone-800 dark:text-stone-200">
    <div class="text-base font-bold text-gray-700 dark:text-stone-300 mb-4">
      PDF 托運單樣式設定
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
      ...
      <div class="space-y-5">
        ...
        <div class="border border-stone-200 dark:border-stone-700 rounded-md overflow-hidden">
          <div class="bg-teal-600 dark:bg-teal-800 text-white px-4 py-2 font-bold text-sm">
            全域設定
          </div>
          <div class="p-4 bg-white dark:bg-zinc-800 space-y-3">
            ...
            <div class="flex items-center gap-3">
              ...
              <input
                v-model.number="settings.barScale"
                type="range"
                min="0.4"
                max="1.2"
                step="0.05"
            ...
            <div class="flex items-center gap-3">
              ...
              <input
                v-model.number="settings.wrapReduce"
                type="range"
                min="0"
                max="30"
                step="1"
          ...
          <div class="p-4 bg-white dark:bg-zinc-800">
            ...
            <div class="space-y-1.5 max-h-80 overflow-y-auto pr-1">
              ...
              >
                ...
                >
                <span
                  class="w-10 text-right font-mono text-xs"
        ...
        <div class="border border-stone-200 dark:border-stone-700 rounded-md overflow-hidden">
          ...
          <div class="p-4 bg-white dark:bg-zinc-800 space-y-3">
            ...
            <div class="flex gap-2 items-center">
              ...
              >
              <button
                class="px-3 py-1 rounded bg-stone-200 dark:bg-zinc-600 hover:bg-stone-300 text-xs text-stone-700 dark:text-stone-200"
                @click="loadWaybills"
              >
                搜尋
              </button>
              ...
              <table class="w-full text-xs">
                  ...
                  <tr>
                    <th class="px-2 py-1 w-6">
                      <input
                        ...
                        @change="toggleAllWaybills"
                      >
                    </th>
                    <th class="px-2 py-1 text-left text-stone-600 dark:text-stone-400">
                      ID
                    </th>
                    <th class="px-2 py-1 text-left text-stone-600 dark:text-stone-400">
                      託運單號
                ...
                <tbody>
                  <tr v-if="!waybills.length">
                    <td
                      colspan="5"
                      class="px-3 py-4 text-center text-stone-400"
                    >
                      {{ waybillsLoading ? '載入中...' : '無資料，請先搜尋' }}
                    </td>
                  </tr>
                  <tr
                    v-for="w in waybills"
                  ...
                  >
                    <td class="px-2 py-1 text-center">
                      <input
                        type="checkbox"
                        :checked="selectedWaybillIds.includes(w.id)"
                        @click.stop="toggleWaybill(w.id)"
                      >
                    </td>
            ...
            <div
              v-else
              class="flex items-center justify-center border border-dashed border-stone-300 dark:border-stone-600 rounded text-stone-400 dark:text-stone-500 text-sm"
              style="height:120px"
            >
              勾選託運單後按「預覽」
            </div>
...
async function loadWaybills() {
  ...
  } catch { waybills.value = [] } finally { waybillsLoading.value = false }
}
