...
<template>
  <div style="min-height:100vh;background:#F5F6F4;color:#1C2321;font-family:'Noto Sans TC','PingFang TC',sans-serif;">
    <div style="height:6px;width:100%;background:repeating-linear-gradient(135deg,#1C2321 0 10px,#B4740E 10px 20px);" />
    ...
    <div style="max-width:420px;margin:0 auto;padding:16px 18px 40px;">
      <div style="display:flex;align-items:center;gap:8px;margin:14px 0 18px;">
        <div style="width:28px;height:28px;border-radius:8px;background:#C1272D;display:flex;align-items:center;justify-content:center;color:#fff;font-size:14px;">
          🧯
        </div>
      ...
      <p
        v-if="loading"
        style="text-align:center;color:#5B615D;padding:40px 0;"
      >
        載入中...
      </p>
      <p
        v-else-if="loadError"
        style="text-align:center;color:#C1272D;padding:40px 0;"
      >
        {{ loadError }}
      </p>
      ...
      <template v-else>
        <div style="display:flex;align-items:center;gap:6px;font-size:13px;color:#2F7D5C;font-weight:500;margin-bottom:16px;">
          <span style="width:7px;height:7px;border-radius:50%;background:#2F7D5C;" />
          已掃描,連結至該滅火器
        </div>
        ...
        <div style="background:#fff;border:1px dashed #DADDD6;border-radius:12px;padding:14px 16px;margin-bottom:18px;font-family:'IBM Plex Mono',monospace;font-size:13px;">
          <div style="display:flex;justify-content:space-between;padding:3px 0;">
            <span style="color:#5B615D;">編號</span><span>{{ extinguisher.code }}</span>
          </div>
          <div style="display:flex;justify-content:space-between;padding:3px 0;">
            <span style="color:#5B615D;">位置</span><span>{{ extinguisher.location }}</span>
          </div>
          <div style="display:flex;justify-content:space-between;padding:3px 0;">
            <span style="color:#5B615D;">批號</span><span>{{ extinguisher.batchNo }}</span>
          </div>
        </div>
        ...
        >
          <button
            :style="{ flex: 1,
                      ...
                      color: activeTab==='history' ? '#1C2321' : '#5B615D' }"
          ...
          <button
            :style="{ flex: 1,
                      ...
                      color: activeTab==='inspect' ? '#1C2321' : '#5B615D' }"
        ...
        <template v-if="activeTab === 'history'">
          <p
            v-if="historyLoading"
            style="text-align:center;color:#5B615D;padding:20px 0;font-size:13px;"
          >
            載入紀錄中...
          </p>
          <p
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
              ...
              <span
                :style="{ fontSize: '12px',
                          ...
                          padding: '2px 10px',
                          borderRadius: '999px',
                          background: statusColor(log.status).bg,
                          color: statusColor(log.status).fg,
                          border: '1px solid '+statusColor(log.status).line }"
              >{{ log.status }}</span>
            </div>
            <p
              v-if="log.note"
              style="font-size:13px;color:#1C2321;white-space:pre-line;margin:6px 0;"
            >
              {{ log.note }}
            </p>
            <a
        ...
        <template v-else-if="!submitted">
          <div style="display:flex;flex-direction:column;align-items:center;margin-bottom:20px;">
            ...
            <div style="font-size:12px;color:#5B615D;">
              已完成檢查項目
            </div>
          </div>

          <div style="font-size:13px;color:#5B615D;margin:0 0 8px;font-weight:500;">
            檢查項目
          </div>
          <div
            v-for="i in items"
          ...
          <div style="font-size:13px;color:#5B615D;margin:14px 0 8px;font-weight:500;">
            拍照佐證
          </div>
          ...
          <p style="font-size:12px;color:#5B615D;text-align:center;margin:-14px 0 14px;">
            檢查人：{{ inspectorName || '（未取得姓名）' }}
          </p>

          <p
            v-if="submitError"
            style="color:#C1272D;font-size:13px;text-align:center;margin-bottom:10px;"
          >
            {{ submitError }}
          </p>
        ...
        <div
          v-else
          style="text-align:center;padding:36px 10px;"
        >
          <div style="width:56px;height:56px;border-radius:50%;background:#E4F1EA;border:1px solid #9FCBB4;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;font-size:26px;color:#2F7D5C;">
            ✓
          </div>
          <h3 style="font-family:'Oswald',sans-serif;font-size:18px;margin:0 0 6px;">
            檢查紀錄已送出
          </h3>
