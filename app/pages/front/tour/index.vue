  ...
  <ClientOnly>
    <div class="preview-page">
      <div class="preview-topbar">
        <span class="topbar-title">聖母健康園區 360 環景導覽</span>
        <select
          v-model="currentSceneId"
          class="scene-select flex-1"
          @change="onSceneSelect(currentSceneId)"
        >
          <optgroup
            v-for="grp in groupedScenes"
            :key="grp.category"
            :label="grp.category"
          >
            <option
              v-for="s in grp.scenes"
              :key="s.id"
              :value="s.id"
            >
              {{ s.name }}
        ...
        >
          ...
          >
            <ellipse
              ...
              rx="17"
              :ry="cursorEllipseRy"
              class="floor-cursor-ring"
            />
            <path
              d="M20 10 L28 24 L20 20 L12 24 Z"
              class="floor-cursor-arrow"
            />
          </svg>
          <span
            v-if="hoverHotspot"
            class="floor-cursor-label"
          >{{ hoverHotspot.label }}</span>
        </div>
...
// 這是給遊客看的公開頁面（front/tour/），不是後台，所以不設 layout: 'staff'、
// 不設 requiredPermission。實際 layout 名稱、要不要隱藏站台導覽列，
// 請依你們前台實際的 layout 慣例調整這裡——目前是空的，會吃 Nuxt 預設的
// 'default' layout。
...
// -----------------------------------------------------------------------
// 記住這台裝置上次看到哪張場景，下次打開直接接著看（存在瀏覽器本機，
// 不是後端）。優先順序：上次看的場景 > 後台設定的「預設進入場景」 >
// 場景清單第一張（order 最小）。
// -----------------------------------------------------------------------
const STORAGE_KEY_SCENE = 'holyMotherFarmTourFront.currentSceneId'

function loadPersistedSceneId() {
  if (typeof window === 'undefined') return null
  try {
    return localStorage.getItem(STORAGE_KEY_SCENE)
  } catch {
    return null
  }
}

function savePersistedSceneId(id) {
  if (typeof window === 'undefined') return
  try {
    if (id) {
      localStorage.setItem(STORAGE_KEY_SCENE, id)
    } else {
...
async function loadData() {
  try {
    const data = await $fetch(`${apiBase.value}/public-data`)
    ...
    if (scenes.value.length) {
      ...
      const persistedExists = persistedId && scenes.value.some(s => s.id === persistedId)
      const startSceneId = settings.value.startSceneId
      const startExists = startSceneId && scenes.value.some(s => s.id === startSceneId)

      currentSceneId.value = persistedExists
        ? persistedId
        : (startExists ? startSceneId : scenes.value[0].id)
    }
  } catch (err) {
...
.preview-page {
  ...
  /* 原本 preview.vue 是被 staff.vue layout 的 #staff-scroll-wrap（已經算好
     100vh − 導覽列高度）包住，才能單純寫 height: 100% 填滿父層。這頁不吃
     staff layout，所以先直接用 100vh 撐滿整個視窗——如果你們前台的
     'default' layout上面還有一條固定導覽列，這裡會需要改成
     `calc(100vh - 導覽列高度)`，不然畫面下緣（含小地圖）可能會被裁掉、
     或多一層捲軸，請依實際 layout 調整這個值。 */
  height: 100vh;
  background: #10171a;
}
