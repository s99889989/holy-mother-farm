<template>
  <div class="p-4 max-w-screen-xl mx-auto text-sm">
    ...
    <div class="flex gap-3 mb-4 items-center">
      <button @click="resetForm"
              class="border px-4 py-1.5 rounded bg-white hover:bg-gray-50">
        ✚ 清空填寫資料
      </button>
    </div>
    ...
    <div class="border rounded-md overflow-hidden mb-4">
      ...
      <div class="p-4 bg-pink-50 space-y-4">
          ...
          <div class="space-y-4">
            ...
            <fieldset class="border rounded p-3 bg-white">
              ...
              <div class="grid grid-cols-3 gap-2 mt-2">
                <div>
                  <label class="text-gray-500 block">代號</label>
                  <input v-model="form.customer_code" type="text" class="border rounded px-2 py-1 w-full" />
                </div>
                <div>
                  <label class="text-gray-500 block">電話</label>
                  <input v-model="form.customer_phone" type="text" class="border rounded px-2 py-1 w-full" />
                </div>
                <div>
                  <label class="text-gray-500 block">手機(ex:0912-345-678)</label>
                  <input v-model="form.customer_mobile" type="text" class="border rounded px-2 py-1 w-full" />
                </div>
              </div>
              <div class="mt-2">
                <label class="text-gray-500 block">地址</label>
                <input v-model="form.customer_address" type="text" class="border rounded px-2 py-1 w-full" />
              </div>
              <div class="mt-2">
                <label class="text-gray-500 block">姓名</label>
                <input v-model="form.customer_name" type="text" class="border rounded px-2 py-1 w-1/2" />
              </div>
            </fieldset>
            ...
            <fieldset class="border rounded p-3 bg-white">
              ...
              <div v-if="form.default_sender" class="space-y-2 mt-2">
                <div class="grid grid-cols-3 gap-2">
                  <div>
                    <label class="text-gray-500 block">代號</label>
                    <input v-model="form.sender_no" type="text" class="border rounded px-2 py-1 w-full" />
                  </div>
                  <div>
                    <label class="text-gray-500 block">電話</label>
                    <input v-model="form.sender_phone" type="text" class="border rounded px-2 py-1 w-full" />
                  </div>
                  <div>
                    <label class="text-gray-500 block">手機(ex:0912-345-678)</label>
                    <input v-model="form.sender_mobile" type="text" class="border rounded px-2 py-1 w-full" />
                  </div>
                </div>
                <div>
                  <label class="text-gray-500 block">地址</label>
                  <input v-model="form.sender_address" type="text" class="border rounded px-2 py-1 w-full" />
                </div>
                <div>
                  <label class="text-gray-500 block">姓名</label>
                  <input v-model="form.sender_name" type="text" class="border rounded px-2 py-1 w-1/2" />
                </div>
              </div>
            </fieldset>
            ...
            <fieldset class="border rounded p-3 bg-white">
              <div>
                ...
                <div class="flex gap-2 mt-1">
                  ...
                  <input v-model="form.production_name" type="text" class="border rounded px-2 py-1 flex-1" />
                </div>
              ...
              <div class="grid grid-cols-2 gap-2 mt-2">
                <div>
                  <label class="text-gray-500 block">訂單編號</label>
                  <input v-model="form.order_no" type="text" class="border rounded px-2 py-1 w-full" />
                </div>
                <div>
                  <label class="text-gray-500 block">備註</label>
                  <input v-model="form.comment" type="text" class="border rounded px-2 py-1 w-full" />
                </div>
              </div>
            </fieldset>
            ...
            <fieldset class="border rounded p-3 bg-white">
              ...
              <div class="grid grid-cols-2 gap-3 mt-2">
                <div>
                  ...
                  <input v-model="form.send_date" type="text" placeholder="YYYY/MM/DD" class="border rounded px-2 py-1 w-full" />
                </div>
                <div>
                  <label class="text-gray-500 block">希望配達日期（週日不配送）</label>
                  <input v-model="form.deliver_date" type="text" placeholder="YYYY/MM/DD" class="border rounded px-2 py-1 w-full" />
                </div>
              </div>
            ...
            <fieldset class="border rounded p-3 bg-white">
              ...
              <div class="space-y-2">
                ...
                <div>
                  ...
                  <input v-model.number="form.insurance" type="number" min="0" class="border rounded px-2 py-1 w-40" />
        ...
        <!-- 錯誤 / 成功訊息 -->
        <div v-if="formError"   class="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded">{{ formError }}</div>
        <div v-if="formSuccess" class="bg-green-100 border border-green-400 text-green-700 px-4 py-2 rounded">{{ formSuccess }}</div>

        <!-- 操作按鈕（對應黑貓四個按鈕） -->
        <div class="flex gap-2 flex-wrap items-center pt-1">
          <!-- 更新（有選擇舊單時顯示） / 新增 -->
          <button v-if="editingId" @click="updateForm" :disabled="submitting"
                  class="bg-yellow-400 hover:bg-yellow-500 text-white font-bold px-5 py-2 rounded disabled:opacity-50">
    ...
    <!-- ════════════════════════ 列表區 ════════════════════════ -->
    <div class="mb-2 text-gray-600">目前契客代號：<strong>{{ form.sender_code }}</strong></div>

    <!-- 篩選列 -->
    <div class="flex gap-3 mb-3 flex-wrap items-center">
      <span class="text-gray-500">依收件人姓名/地址/手機/電話過濾單：</span>
      <input v-model="keyword" type="text" placeholder="姓名 / 電話 / 託運單號"
             class="border rounded px-3 py-1.5 w-60" @keyup.enter="search" />
      <button @click="search"   class="bg-blue-600 text-white px-4 py-1.5 rounded hover:bg-blue-700">過濾</button>
      <button @click="resetSearch" class="bg-gray-300 px-4 py-1.5 rounded hover:bg-gray-400">取消過濾</button>
    </div>

    <!-- 分頁資訊 -->
    <div class="flex gap-2 mb-2 items-center text-sm">
      <button :disabled="page <= 1" @click="page--; refreshList()"
              class="border px-3 py-1 rounded disabled:opacity-40">&lt;&lt; 第一頁</button>
      <button :disabled="page <= 1" @click="page--; refreshList()"
              class="border px-3 py-1 rounded disabled:opacity-40">&lt; 前一頁</button>
      <span>第 {{ page }} 頁 / 共 {{ totalPages }} 頁（{{ listData?.total }} 筆）</span>
      ...
      <button :disabled="page >= totalPages" @click="page = totalPages; refreshList()"
              class="border px-3 py-1 rounded disabled:opacity-40">最後一頁 &gt;&gt;</button>
      <span class="text-gray-400">每頁</span>
      <select v-model="limit" class="border rounded px-2 py-1" @change="page=1; refreshList()">
        <option :value="10">10</option>
        <option :value="20">20</option>
        <option :value="50">50</option>
      </select>
      <span class="text-gray-400">筆</span>
    </div>
    ...
    <div class="overflow-x-auto">
      <table class="w-full border-collapse text-sm">
        ...
        <tr>
          <th class="border border-teal-700 px-2 py-2 w-8">
            <input type="checkbox" @change="toggleAll" :checked="allChecked" />
          </th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">託運單號</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">收件人</th>
          <th class="border border-teal-700 px-3 py-2 text-left">地址</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">郵遞區號</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">配送限制</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">手機</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">電話</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">特殊地點</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">偏遠地址</th>
          <th class="border border-teal-700 px-3 py-2 text-left">品名</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">訂單編號</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">代收貨款</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">收貨日期</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">希望配達時段</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">希望配達日期</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">託運單狀態</th>
          <th class="border border-teal-700 px-3 py-2 text-left whitespace-nowrap">紙張種類</th>
        </tr>
        </thead>
        <tbody>
        <tr v-if="!listData?.rows?.length">
          <td colspan="18" class="border px-4 py-6 text-center text-gray-400">無資料</td>
        </tr>
        <tr
          ...
          :class="{
              'bg-yellow-100 font-semibold': editingId === row.id,
              'bg-yellow-50': selectedIds.includes(row.id) && editingId !== row.id
            }"
          @click="loadRowToForm(row)"
        >
          <td class="border px-2 py-1 text-center" @click.stop>
            <input type="checkbox" :value="row.id" v-model="selectedIds" />
          </td>
          <td class="border px-3 py-1 font-mono text-blue-600 whitespace-nowrap">{{ row.tracking_no }}</td>
          <td class="border px-3 py-1 whitespace-nowrap">{{ row.customer_name }}</td>
          <td class="border px-3 py-1 whitespace-nowrap max-w-[180px] overflow-hidden text-ellipsis" :title="row.customer_address">{{ row.customer_address }}</td>
          <td class="border px-3 py-1 whitespace-nowrap">{{ row.customer_postcode }}</td>
          <td class="border px-3 py-1"></td>
          <td class="border px-3 py-1 whitespace-nowrap">{{ row.customer_mobile }}</td>
          <td class="border px-3 py-1 whitespace-nowrap">{{ row.customer_phone }}</td>
          <td class="border px-3 py-1"></td>
          <td class="border px-3 py-1"></td>
          <td class="border px-3 py-1 whitespace-nowrap max-w-[120px] overflow-hidden text-ellipsis" :title="row.production_name">{{ row.production_name }}</td>
          <td class="border px-3 py-1 whitespace-nowrap">{{ row.order_no }}</td>
          <td class="border px-3 py-1 whitespace-nowrap">{{ row.price }}</td>
          <td class="border px-3 py-1 whitespace-nowrap">{{ row.send_date }}</td>
          <td class="border px-3 py-1 whitespace-nowrap">{{ deliverTimeLabel(row.deliver_time) }}</td>
          <td class="border px-3 py-1 whitespace-nowrap">{{ row.deliver_date }}</td>
          <td class="border px-3 py-1 whitespace-nowrap">{{ row.state }}</td>
          <td class="border px-3 py-1 whitespace-nowrap">{{ paperName(row.paper_id) }}</td>
        </tr>
        </tbody>
      </table>
    </div>
