    ...
    <header class="bg-surface border-b border-light-c px-4 py-3 sticky top-0 z-30">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <div class="w-8 h-8 rounded-lg bg-surface2 flex items-center justify-center text-white text-sm font-bold flex-shrink-0">設</div>
          <div>
            <h1 class="font-bold text-base-c leading-none text-sm sm:text-base">設定管理</h1>
            <p class="text-xs text-hint-c mt-0.5 hidden sm:block">common.yml — 共用設定</p>
          </div>
        </div>
    ...
    <div class="max-w-5xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="bg-surface rounded-2xl border border-light-c shadow-sm p-4">
          <h3 class="text-sm font-semibold text-muted-c mb-3">品項類別</h3>
          <div class="space-y-2">
            <div v-for="(cat, key) in commonConfig.categories" :key="key" class="flex items-center gap-2">
              <span class="text-xs text-hint-c w-5 flex-shrink-0">{{ key }}</span>
              <input v-model="cat.name" type="text" placeholder="中文名稱"
                class="flex-1 bg-surface border border-light-c text-base-c rounded-lg px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-400 min-w-0" />
              <input v-model="cat.en" type="text" placeholder="英文 key" maxlength="20"
                class="w-24 bg-surface border border-green-300 dark:border-green-700 text-green-700 dark:text-green-300 rounded-lg px-2 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-green-400 flex-shrink-0" />
        ...
        <!-- 單位 -->
        <div class="bg-surface rounded-2xl border border-light-c shadow-sm p-4">
          <h3 class="text-sm font-semibold text-muted-c mb-3">單位</h3>
          <div class="space-y-2">
            <div v-for="(unit, key) in commonConfig.units" :key="key" class="flex items-center gap-2">
              <span class="text-xs text-hint-c w-5 flex-shrink-0">{{ key }}</span>
              <input v-model="unit.name" type="text"
                class="flex-1 bg-surface border border-light-c text-base-c rounded-lg px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-400" />
        ...
        <!-- 製造商 / 供應商 -->
        <div class="bg-surface rounded-2xl border border-light-c shadow-sm p-4">
          <h3 class="text-sm font-semibold text-muted-c mb-3">製造商 / 供應商</h3>
          <div class="space-y-2">
            <div v-for="(make, key) in commonConfig.makes" :key="key" class="flex items-center gap-2">
              ...
              <select v-model="make.role"
                class="border border-light-c bg-surface text-base-c rounded-lg px-1.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-400 flex-shrink-0">
