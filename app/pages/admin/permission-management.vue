  ...
  <div class="min-h-screen bg-surface2 transition-colors duration-300">
    ...
    <header class="bg-surface border-b border-light-c px-4 py-3 sticky top-0 z-30">
      <div class="flex items-center gap-2">
        ...
        <div>
          <h1 class="font-bold text-base-c text-sm sm:text-base leading-none">權限組</h1>
          <p class="text-xs text-hint-c mt-0.5 hidden sm:block">Permission Groups</p>
        </div>
      </div>
    </header>
      ...
      <div class="bg-surface rounded-2xl shadow-xl w-full max-w-2xl p-6 my-auto">
        ...
        <div class="space-y-4">
          ...
          <div class="flex gap-2 pt-1">
            <button class="flex-1 py-2.5 text-sm bg-surface2 text-muted-c rounded-xl transition-colors" @click="groupModal.open = false">取消</button>
    ...
    <div v-if="deleteGroupTarget" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4">
      <div class="bg-surface rounded-2xl shadow-xl w-full max-w-sm p-6">
        ...
        <div class="flex gap-2">
          <button class="flex-1 py-2.5 text-sm bg-surface2 text-muted-c rounded-xl transition-colors" @click="deleteGroupTarget = null">取消</button>
          <button :disabled="saving"
...
const setAllPerms = (val) => {
  allKeys.value.forEach(k => { groupModal.data.permissions[k] = val })
}
const setSectionPerms = (sectionKeys, val) => {
  sectionKeys.forEach(kd => { groupModal.data.permissions[kd.key] = val })
}
