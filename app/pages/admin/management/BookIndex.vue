...
const openRecurModal = (rule) => {
  ...
  if (rule) {
    Object.assign(recurForm, {
      id: rule.id,
      name: rule.name,
      type: rule.type || 'booking',
      time: rule.time || '12:00',
      guests: rule.guests || 2,
      note: rule.note || '',
          ...
          <div v-if="selectedDate" class="mt-3 space-y-2">
            <div
              ...
              <p class="text-sm text-green-700 dark:text-green-300">
                ...
                <span class="font-semibold">{{
                    bookings.reduce((s, b) => s + (b.meatQty || 0) + (b.fullVegQty || 0) + (b.eggVegQty || 0) + (b.spiceVegQty || 0), 0) + recurBookingGuests
                  }}</span> 人
            ...
            <div
              ...
              <p class="text-sm text-orange-700 dark:text-orange-300">
                ...
                葷 <span class="font-semibold">{{ totalMeat }}</span> / 素 <span class="font-semibold">{{
                  totalVeg
                }}</span>
              </p>
            </div>
                    ...
                    <div class="flex-1 px-3 py-2.5 flex items-center justify-between gap-2">
                      <div>
                        ...
                        <div class="flex flex-wrap gap-2 mt-1 text-xs text-stone-500 dark:text-stone-400">
                          <span v-if="booking.meatQty > 0">🍖 葷 {{ booking.meatQty }}</span>
                          <span v-if="booking.fullVegQty > 0">🌿 全素 {{ booking.fullVegQty }}</span>
                          <span v-if="booking.eggVegQty > 0">🥚 蛋奶素 {{ booking.eggVegQty }}</span>
                          <span v-if="booking.spiceVegQty > 0">🧄 五辛素 {{ booking.spiceVegQty }}</span>
