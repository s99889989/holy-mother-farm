...
// customer 變成 null 不一定是真的登出——localStorage（例如被 iOS
// Safari 的 ITP 機制清掉）跟 session cookie 是各自獨立的儲存機制，
// 過去只要 customerStore.isLoggedIn 是 false，就直接判定沒登入、
// 一律踢回登入頁，從來沒有機會去問後端「session cookie 其實是不是
// 還有效」。
//
// 這個函式在 customer 是 null 時主動打一次 /me，如果後端說 session
// 其實還有效，就把 customer 補回來，不用整個踢回登入頁重新登入。
