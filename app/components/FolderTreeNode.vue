<template>
 ...
 selectedPath === node.path ? 'bg-indigo-50 dark:bg-indigo-900/30' : 'hover-surface2']"
      ...
      <!-- 展開/收合 -->
      <button class="p-0.5 mr-0.5 text-hint-c flex-shrink-0 w-4" @click.stop="expanded = !expanded">
 ...
 : 'text-muted-c']"
      ...
      <div class="opacity-0 group-hover:opacity-100 flex-shrink-0 relative">
        <button class="p-1 text-hint-c hover:text-muted-c transition-colors" @click.stop="menuOpen = !menuOpen">
          <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
            <circle cx="5" cy="12" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="19" cy="12" r="1.5"/>
          </svg>
        </button>
        <div v-if="menuOpen"
             class="absolute right-0 top-7 z-50 bg-surface border border-light-c rounded-xl shadow-lg py-1 min-w-32">
          <button class="w-full text-left px-3 py-1.5 text-xs text-teal-600 hover-surface2" @click.stop="$emit('moveup', node.path); menuOpen=false">↑ 上移</button>
          <button class="w-full text-left px-3 py-1.5 text-xs text-teal-600 hover-surface2" @click.stop="$emit('movedown', node.path); menuOpen=false">↓ 下移</button>
          <div class="border-t border-light-c my-1"></div>
          <button class="w-full text-left px-3 py-1.5 text-xs text-indigo-600 hover-surface2" @click.stop="$emit('add', node.path); menuOpen=false">＋ 新增子資料夾</button>
          <button class="w-full text-left px-3 py-1.5 text-xs text-amber-600 hover-surface2" @click.stop="$emit('rename', node.path, node.name); menuOpen=false">✏ 重新命名</button>
          <div class="border-t border-light-c my-1"></div>
          <button class="w-full text-left px-3 py-1.5 text-xs text-red-500 hover-surface2" @click.stop="$emit('delete', node.path); menuOpen=false">🗑 刪除</button>
        </div>
      </div>
    </div>
