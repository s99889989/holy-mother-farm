...
export default defineEventHandler((event) => {
  ...
  let sql = `
    SELECT
      ...
      production_name, price, state, created_at
    FROM waybills
    WHERE 1=1
  `
  const params: any[] = []
  ...
  if (keyword) {
    sql += ` AND (customer_name LIKE ? OR customer_phone LIKE ? OR tracking_no LIKE ? OR order_no LIKE ?)`
  ...
  if (keyword) {
    countSql += ` AND (customer_name LIKE ? OR customer_phone LIKE ? OR tracking_no LIKE ? OR order_no LIKE ?)`
